...
All sixteen are hidden from the generic settings page on Atlas Cloud, so a workspace admin cannot read or write any of them. Five are **workspace-scoped** — meaning each can hold a different value per workspace, set by a platform admin: `ATLAS_BRAIN_AUDIENCE_SYNC_ENABLED`, `ATLAS_BRAIN_COVERAGE_SNAPSHOT_ENABLED`, `ATLAS_BRAIN_WAREHOUSE_CADENCE_ENABLED` and the two alias auto-approval keys. The other eleven are one value for the whole deployment.
