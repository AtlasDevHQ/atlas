...
export const SEARCH_BRAIN_TOOL_DESCRIPTION = `Search the company brain — a trust-labeled read over three stores. \`tier: "fact"\` is human-reviewed; \`tier: "raw-episode"\` is its source record; \`tier: "document"\` is hosted knowledge — ${KNOWLEDGE_TRUST_FRAMING}. When \`provenance.attribution\` is withheld (\`visible: false\`), say attribution is restricted; never call the claim anonymous, undated, unsourced, or infer the author. Retracted claims are excluded, drafts outside developer mode. Unextracted episodes carry \`extraction: "pending"\`. Facts carry age metadata (\`validFrom\`, \`corroborationCount\`, \`decay\`): present a stale fact's age, never assert it as current or drop it. Conflicting facts surface unranked \`tensions\`: report every side, never pick a winner. \`unavailable\` means the search failed, not "nothing known". Example: \`{ "query": "who owns billing" }\`.
