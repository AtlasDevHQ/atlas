...
The isolation controls still apply in full: the sandbox's network policy is deny-all (code inside cannot make outbound connections) and its filesystem is ephemeral — nothing persists at Vercel after the tool call completes. The US processing is disclosed in the [sub-processor list](https://www.useatlas.dev/sub-processors) referenced by the DPA (Vercel entry: semantic-layer content and query results).

Workspaces that need in-region tool execution today can connect their own sandbox on a region-controlled provider — see [Sandbox — SaaS Mode](/guides/sandbox). A workspace-connected (BYOC) sandbox always takes priority over the platform sandbox and fails closed rather than falling back to it. BYOC currently covers `explore` fully; Python support on the BYOC providers is in progress.
