...
- [x] **1.4.4 — Multi-environment semantic layer** (#45, 11 + 17 audit issues + #2429 follow-up) — connection groups foundation + admin merge-into-group wizard + Phase 4 archive cascade, group-scoped semantic / dashboards / scheduled tasks / approvals / PII, group-aware chat routing + per-turn override, application-layer FK gate for cross-org `connection_group_id` writes (#2424), legacy `connection_id` dropped (#2346 + 0069), `@useatlas/types` → 0.1.x. PRD #2336, audit #2407, mop-up #2442. Path-A real route-level e2e deferred to #2441. **Code-complete; manual verification gating GH milestone close at #2443.**
...
- [⏳] **Internal-team dogfood pass — 2026-05-16** — shipped: shared `<EmptyAskHero>` + `<AskComposer>` + `<ConnectDataPrompt>` primitives behind both chat (`/`) and notebook (`/notebook`) empty states (#2451 / #2452), admin sidebar Collapsible switched to controlled state + billing page no longer lies "Self-Hosted" on SaaS + `useDeployMode` hostname-based default (#2457). Spawned two follow-on PRDs from prod findings: **#2458** (admin data-layer IA reshape, 4 pages → 2; 2/5 slices shipped via #2471 / #2472) and **#2464** (SaaS plan + trial onboarding; 2/4 slices shipped via #2469 / #2470). Root-cause investigation for the prod `/api/v1/billing` 404 (#2455) tied into the trial work and closed alongside.
