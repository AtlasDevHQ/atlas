...
<Callout type="warn" title="Deprecated: the `cache:` config block">
A `cache:` block in `atlas.config.ts` is **ignored** and logs a boot warning. Its values no longer apply — use the settings above. The block is scheduled for removal in a future release.
</Callout>
