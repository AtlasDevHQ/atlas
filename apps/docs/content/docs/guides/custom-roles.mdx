...
| `dashboards:write` | Can create, edit, share and publish dashboards |
...
- **No enterprise license?** The legacy `admin`/`member`/`owner` roles continue to work exactly as before. `admin` and `owner` get all permissions; `member` gets the query pair plus both `dashboards:*` flags.
