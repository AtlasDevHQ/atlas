...
| Control | Purpose |
|---------|---------|
| Per-source rate limiting | Atomic concurrency + QPM check via `acquireSourceSlot(connId)` before the query reaches the database. Rejects with a rate-limit error (and an optional `retryAfterMs` hint) when the source is at capacity. Configurable via `rateLimit` in `atlas.config.ts` datasource entries |
