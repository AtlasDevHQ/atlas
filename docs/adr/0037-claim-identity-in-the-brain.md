...
- **Every drop is counted and reported.** An expected `entity:` drop, a tag the two regions disagree about, and a legacy surface that normalizes away are otherwise one `200` with healthy counts. The importer separates `storeLocal` (the rule), `unreadable` (**drift — evidence lost, not deferred**) and `unkeyable`, and warns once per import.
