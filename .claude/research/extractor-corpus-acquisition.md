...
The [ADR-0044](../../docs/adr/0044-fact-content-never-enters-model-weights.md) prerequisite for the distilled extractor ([issue 5337](https://github.com/AtlasDevHQ/atlas/issues/5337)): ADR-0044's Consequences section, verbatim — *"A corpus-acquisition step is now a prerequisite for #5337 rather than an afterthought, since 'just use our episodes' is closed."* This document names the candidate corpora with their licence terms verified against primary sources, frames the two decisions only a human can make, and states what the corpus may and may not be used for. It decides neither human question.
...
|---|---|---|---|---|---|
| **Apache mailing-list archives** (lists.apache.org) | Strong — async project mail: ownership, status, decisions, deprecations | ASF public-archives policy + Apache License 2.0 §1's Contribution definition | **Yes, on the ALv2 theory** (note 1) — the ASF's publication statement and its note that *"numerous third parties independently archive nearly all of our public mailing lists"* support the practice, but neither is itself a copyright grant | **Yes, on the ALv2 theory** (attribution/notice obligations per ALv2 §4; see ambiguity note 1 and open question 3) | [apache.org/foundation/public-archives.html](https://www.apache.org/foundation/public-archives.html): posts are *"deemed to be published by the sender of that communication and made public without conditions"*. [ALv2 §1](https://www.apache.org/licenses/LICENSE-2.0): "submitted" includes *"communication on electronic mailing lists … managed by, or on behalf of, the Licensor … but excluding communication that is conspicuously marked … as 'Not a Contribution.'"* |
...
| **GitHub threads at large** (GH Archive / GitHub-wide) | Strongest structurally | **None that covers the use.** GH Archive states no dataset licence at all (verified: [gharchive.org](https://www.gharchive.org/) carries no licence statement). GitHub ToS D.5 confirms publicly posted content *"may be viewed by others"* and grants other users a licence to use and reproduce it **through the GitHub Service** — on-platform use and forking, not our bucket (paraphrase; D.5's full text would not extract verbatim through our tooling, so only the short phrase is quoted). The AUP research allowance is conditioned: *"use public, non-personal information from the Service for research purposes, only if any publications resulting from that research are open access"* — a condition shipped commercial weights do not meet | **Not licensed** — issue text is its authors' copyright; no grant reaches off-platform storage | **No written basis** | [GitHub ToS](https://docs.github.com/en/site-policy/github-terms/github-terms-of-service) §D.5; [AUP](https://docs.github.com/en/site-policy/acceptable-use-policies/github-acceptable-use-policies) |
| **LKML via lore.kernel.org** | Strong — same profile as Apache lists | **Ambiguous.** Archives are clone-by-design (public-inbox: *"Git clone URLs are provided at the bottom of each page"*, and grokmirror can *"continuously mirror the entire mailing list archive collection"* — [kernel.org/lore.html](https://www.kernel.org/lore.html)) — but **no reuse licence is stated anywhere on the archive pages** (verified 2026-08-28: neither that page nor [korg.docs.kernel.org/lore.html](https://korg.docs.kernel.org/lore.html) states one), so the discussion **prose** remains its authors' copyright by default. Patches are GPLv2 via the DCO; the prose around them — the part this corpus wants — is not | **Yes in practice** (cloning is the designed access path), **unlicensed in writing** | **No written basis** — implied-licence/fair-use arguments only | [kernel.org/lore.html](https://www.kernel.org/lore.html); [korg.docs.kernel.org/lore.html](https://korg.docs.kernel.org/lore.html) |
...
| **Stack Exchange data dumps** | Moderate (Q&A, not async decision threads) | **Ruled out at the acquisition point.** Content is CC BY-SA 4.0, but since 2024 the dump download sits behind a login and an agreement whose affirmation the cited article quotes: the downloader affirms *"the file is being provided to me for my own use and for projects that do not include training a large language model"* — and CC BY-SA's ShareAlike interacting with shipped commercial weights is additionally unsettled (ambiguity note 2) | Dump agreement refuses the use | **Refused by the access agreement; SA question unsettled besides** | [devclass.com 2024-07-30](https://devclass.com/2024/07/30/stack-exchange-restricts-access-to-dump-of-user-contributed-data-as-critics-complain-license-permits-reuse-for-any-purpose/) |
...
1. **The ALv2 "Contribution" theory is a reading, not settled law — and the stronger objection is in §2, not §1.** ALv2 §1 licenses what is *"intentionally submitted to Licensor for inclusion in the Work"*, with "submitted" defined to include mailing-list and issue-tracker communication *"for the purpose of discussing and improving the Work."* Off-topic list chatter arguably falls outside "for inclusion in the Work"; the ASF's separate *"made public without conditions"* sentence is a publication statement, not a formal copyright grant. The sharper counter-argument: §2's copyright grant flows from a **"Contributor"**, defined in §1 as one whose Contribution *"has been received by Licensor and subsequently incorporated within the Work"*, and the grant's object is *"the Work and such Derivative Works"* — ordinary list/issue discussion is never incorporated into the Work, so arguably no §2 grant ever attaches to the raw archive text at all, whatever the submitter's intent. This is still the strongest written basis any conversational corpus offers — and it is still an argument, with a real counter-argument, and card 2 should treat it as exactly that.
...
### Data protection is a separate axis from copyright — and this table has no column for it

A corpus can be cleanly licensed and still privacy-encumbered, and every conversational corpus above carries **personal data** — names, email addresses, sometimes personal content — of, among others, EU/UK residents. Copyright answers "may we copy it"; it does not answer "may we hold and process these people's data in `atlas-training-corpora`, and what do we do with an erasure request." Three specifics found in this research's own sources:

- The [ASF public-archives page](https://www.apache.org/foundation/public-archives.html) this document cites states no GDPR posture and **explicitly refuses to remove contact information** from the archives — the ASF's position on its archive does not transfer any position to our copy of it.
- GitHub's AUP research allowance — quoted in the GitHub-wide row — is limited to *"public, **non-personal** information"*. Issue and PR text carries usernames and often real names/emails; whether that text is "non-personal" is exactly the kind of question this document must not quietly resolve by quoting past it. The `apache/*` rows rest on ALv2, not on the AUP allowance, but the personal-data character of the text is the same.
- ADR-0044's own core argument — *weights cannot unlearn; a checkpoint sits outside `purge-scope.ts`* — applies with equal force to a **third party's** erasure request as to a customer's. A list poster who asks to be removed from our training corpus can be removed from the bucket; they cannot be removed from shipped weights. The informed-consent clause card 1 states for our own colleagues has no analogue for the strangers in the public corpora.

**This is a question for the human and, plausibly, counsel — not one this document resolves.** Practical mitigations exist (pseudonymizing headers/handles before labelling, minimizing retained fields, a documented erasure procedure for the bucket, recording the lawful-basis reasoning per corpus) and should be priced into card 2 — but choosing among them is part of the sign-off, and card 2's counsel option now includes this axis explicitly.

---

## Decision card 1 — our own workspace Slack (human decision; not made here)

ADR-0044: *"Ours to consent to, and the consent is real rather than construed"* — and [issue 5381](https://github.com/AtlasDevHQ/atlas/issues/5381) states the same requirement as its own acceptance criterion: *"A recorded, explicit decision on using our own workspace's messages, made by a person rather than inferred from ADR-0044."*
...
| **C. Counsel review first** | A professional read on notes 1 and 2 **and the data-protection axis** (the section following the ambiguity notes) before any download | Time — and note 2 is unsettled industry-wide, so counsel can bound the risk, not eliminate it; procurement stalls meanwhile, and this lane is the one item that *"does not compress by writing faster"* |
...
- **Teacher:** a frontier-class extraction model — functionally, the same class the production extractor resolves today. The exact model is deliberately **not pinned in this document**: the pin lives in the label-batch manifest below, which records the precise teacher model id per batch, so the document cannot silently drift from what a run actually used. Run against the **deployed contract verbatim**: `EXTRACTION_SYSTEM_PROMPT` as the system turn, `extractionPrompt`/`extractionExcerpt` shaping (including the `MAX_BODY_CHARS` cap and strip-then-truncate order), and `ExtractionSchema` (as `EXTRACTION_JSON_SCHEMA` on a batch wire) as the output shape — all from `packages/api/src/lib/brain/extract-contract.ts`. The student must learn the contract the product runs, not a variant of it; any drift between labelling prompt and production prompt is a silent domain shift.
...
3. **Attribution mechanics for shipped weights** (ambiguity note 2) — where the AMI/ICSI CC-BY attribution rides (model card, NOTICE file, both) can be decided cheaply now; whether it is *required* stays unsettled industry-wide. The same decision should cover **ALv2 §4** for the Apache-derived rows: a redistributed derived artifact must carry a copy of the licence and retain attribution notices — the model-card-plus-NOTICE mitigation plausibly satisfies both families at once, and deciding that here is cheaper than discovering it at ship time.
