...
1. Click the **"What if?"** button (branch icon) on any query cell that has output
...
When a notebook has branches, a **branch selector** appears at the top showing all branches. Click any branch to switch between them — each branch maintains its own independent cell history. You can **rename** a branch by clicking the pencil icon in the branch selector, or **delete** a branch via the trash icon (with a confirmation dialog).

Cells that have branches show a small **gutter indicator** (branch icon) on the left edge. Hover over it to see how many branches originate from that cell.
