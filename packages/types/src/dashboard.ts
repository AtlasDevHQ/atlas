...
// The per-user destructive-op staging wire types (#2365 `StagedChange` /
// `StageKind` / `StageStatus` / `StagePayload`) were retired with the stage
// tracker (ADR-0034 Decision 2, #4555). Destructive bound-editor ops now apply
// straight to the caller's draft with an inline undo — see `DashboardCard` +
// the `/draft/undo` route.
