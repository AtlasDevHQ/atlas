...
4. **Disagreement survives.** Two people in the company contradict each other in writing. Atlas surfaces both, attributed, and has picked neither — a week later it has still picked neither, and nobody had to intervene to keep it that way.
   ...
   ✅ **DEMONSTRATED 2026-09-02 ([#5425](https://github.com/AtlasDevHQ/atlas/issues/5425)).** The week ran 2026-08-25T19:36:30Z → 2026-09-01T19:36:30Z and elapsed clean. Edge `fa9935b7` intact with `created_at` unmoved, both claims live (`valid_to` and `invalidated_at` NULL on each), no `supersedes` edge between them, and `object_cmp` NULL on both so the pair sits in the abstain band by design. Non-intervention is established from append-only tables rather than from a mutable field: two human acts in the window, both against *other* facts; zero extraction cycles that wrote anything; two episodes ingested, both being those same two corrections. ⚠️ **What this does NOT retire is the paragraph below it** — recognition is still partial by construction, the recognition here was retroactive rather than at ingest ([#5449](https://github.com/AtlasDevHQ/atlas/issues/5449) is still owed), and a human both authored the cardinality entry and ran the sweep. *"Nobody had to intervene to keep it that way"* is what holds; *"nobody had to intervene to make it happen"* was never the claim.
...
>
> **Condition 4 closed 2026-09-02** — the week elapsed with no intervention and the verdict is on [#5425](https://github.com/AtlasDevHQ/atlas/issues/5425); the row below rating it *"**Yes**, for claims that are recognized as contradicting"* was right about the qualifier and wrong to state it as met, since at the time nothing had run the week. Five of the eight now hold (2, 4, 5, 6, 7). The three open are **1** (cold start, gated on a real customer), **3** (tiers unmistakable) and **8** (self-hoster, inheriting the other seven).
>
