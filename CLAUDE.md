...
- [ ] **Encrypted at rest** — Bearer secrets in the internal DB go through one of two `encryptSecret` / `decryptSecret` helper pairs before write; both produce identical `enc:v<N>:iv:authTag:ciphertext` AES-256-GCM ciphertext, but their plaintext-detection differs and current call sites are split: (1) **`db/internal.ts`** (URL-aware passthrough on read — accepts `scheme://…` plaintext as-is, plus a 3-colon legacy fallback) is reserved for the two surviving legacy non-`workspace_plugins` columns: `workspace_model_config.api_key_encrypted` (including the bedrock JSON cred bundle) and `sso_providers.config.clientSecret`. **No new call sites** — the URL-aware passthrough was a back-compat affordance for the pre-encryption `connections.url` column, which was dropped by 0096 (post-#2744 / [ADR-0007](docs/adr/0007-unified-install-pipeline.md) cutover). (2) **`db/secret-encryption.ts`** (versioned-prefix-only — anything without `enc:v<N>:` is treated as opaque plaintext) is the default for everything else: every F-41 integration credential column (bot tokens, API keys, email/sandbox JSON blobs) AND every datasource install credential — datasource URLs now live inside `workspace_plugins.config->>'url'` and go through selective-field encryption (`encryptSecretFields` / `decryptSecretFields`) keyed on the catalog row's `config_schema` `secret: true` flag, same pattern as every other plugin config secret. Reads call `decryptSecret(<col>_encrypted)` directly — no plaintext fallback survives post-#1832. Adding a new integration credential table is a one-line addition to `INTEGRATION_TABLES` in `packages/api/src/lib/db/integration-tables.ts` (consumed by both rotation + the F-42 residue audit) plus an `_encrypted` column in the migration. SaaS regions can opt into `ATLAS_STRICT_PLUGIN_SECRETS=true` to reject plugin admin writes whose schema is corrupt or carries secret/passthrough drift. The `encryptUrl` / `decryptUrl` deprecated re-exports were removed in 1.5.3 closeout (#2755) per the original #2285 schedule — use `encryptSecret` / `decryptSecret` from `db/internal.ts` directly when touching one of the two legacy columns. Key derivation: `ATLAS_ENCRYPTION_KEYS` (versioned, F-47) → `ATLAS_ENCRYPTION_KEY` (legacy single-key, treated as v1) → `BETTER_AUTH_SECRET` (deprecated under SaaS — startup warns)
...
- [ ] **Tests are self-contained** — No top-level `process.env.X = ...` or `process.chdir(...)` at module scope. The hoisted `??=` pattern IS permitted when an import-time env read requires the var to be set before the file's first import (see `actions.test.ts` for the template); the gate (`scripts/check-test-discipline.sh`, drift CI job) treats `??=` and `=` differently — only unconditional `=` is blocked. For path-typed test-owned vars (`ATLAS_SEMANTIC_ROOT = tmpRoot`), unconditional `=` is REQUIRED so a parent-env value doesn't break hermetic isolation. `mock.module()` does NOT need a paired `mock.restore()` — the 1.5.4 slice 5a empirical fixture (`packages/api/src/__tests__/_bun-isolation-experiment/`, see #2801) proved bun's `--isolate` resets module mocks between files in the same worker. The custom `scripts/test-isolated.ts` subprocess-per-file runner is still in use (slice 6 / #2802 cutover blocked on #2811/#2812); when it lands, `--parallel` reuses workers across files so OS-level state (env, cwd, file handles, signal listeners) leaks between files in the same worker
...
### Plugin migrations
- [ ] **Update the chat-plugin × Atlas contract doc when the boundary changes** — Any PR that adds, removes, or reshapes a field at the `@useatlas/chat` / `@chat-adapter/*` boundary updates the table in `docs/architecture/chat-plugin-atlas-contract.md` in the same commit. Covers: new Atlas-extension fields on `chat_cache.value`; new host-callback wirings under `chatPlugin({ proactive: { ... } })`; new platforms moving from ○ pending to ✓ verified; changes to a read site's fail-loud behaviour. **Why:** the 1.5.2 chat-plugin migration shipped three back-to-back hotfixes (#2628, #2630, #2676) where each ported the happy path but silently dropped an Atlas-side state contract — see #2677 for the audit response. **How to apply:** before opening a PR that touches `plugins/chat/src/` or `packages/api/src/lib/slack/` or `packages/api/src/lib/integrations/install/*-oauth-handler.ts`, diff the contract table against the change set and edit the doc to match. Open ⚠ rows are blockers for milestone closeout
