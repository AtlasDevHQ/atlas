...
- **Glossary disambiguation** — `revenue`, `status`, `price` are `status: ambiguous` and must trigger the disambiguation contract documented in [the typed MCP tools](/guides/mcp#searchglossary), never resolve to a single mapping
