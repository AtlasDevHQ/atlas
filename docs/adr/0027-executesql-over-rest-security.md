...
- **A workspace-admin off-switch for raw SQL** (gate-1 analog) — deferred to the Architecture Backlog as [#4095](https://github.com/AtlasDevHQ/atlas/issues/4095), shaped to slot into the gate-1 seam. Not built now: Atlas is pre-customer and raw SQL is already contained by member-floor + whitelist + RLS + approval. Default would be enabled (no behavior change). ⚠️ **2026-08-18: the "Atlas is pre-customer" half of that rationale has expired** — the clean-break window closed (docs/contexts/deployment-posture/CONTEXT.md). The containment half still holds and is what the deferral now rests on entirely, so [#4095](https://github.com/AtlasDevHQ/atlas/issues/4095) is worth re-reading against a with-customers threat model rather than inheriting this deferral unexamined.
