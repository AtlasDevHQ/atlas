...
No tag in flight — the train is between tags. `v0.0.6` was tagged + released 2026-06-02 (a patch rollup — webhook delivery unification + multi-tenant ConnectionRegistry correctness; see [History](#history)); the next tag's scope firms up as work begins. The **Staging environment** track ([Planned tags](#planned-tags)) continues in parallel, independent of the tag train.

Nothing banked yet since `v0.0.6`.
