...
3. nsjail explicit (when `ATLAS_SANDBOX=nsjail` is set) — **hard fail**: if nsjail init fails the API does not boot. Remove `ATLAS_SANDBOX` to fall through to auto-detect.
