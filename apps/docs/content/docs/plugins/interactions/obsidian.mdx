...
Both the modal and code blocks display query results as HTML tables. Tables are capped at **100 rows** with an overflow indicator. The full dataset is available when you insert into a note (as a Markdown table).

## How It Works

The plugin uses a lightweight SSE streaming client (`src/client.ts`) that connects to the Atlas `/api/chat` endpoint. It parses the Vercel AI SDK data stream protocol and emits typed events:
