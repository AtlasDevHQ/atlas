...
| 4 | #4907 | `botUserId` in `chat_cache` — Atlas's OAuth handler replaced the adapter's own callback, which was the field's only writer | `isMessageFromSelf` false for every self-post; Atlas answered its own messages in an unbounded prod loop |

Row 4 is the same shape as row 2, inverted: there, Atlas wrote a placeholder the adapter needed to be real; here, Atlas wrote nothing where the adapter needed a value. Both come from Atlas owning a write path the adapter also owns — and in both, the adapter degraded silently rather than announcing the missing input. **When Atlas takes over one of the adapter's write paths, it inherits every field that path wrote, not just the ones Atlas reads.** `botUserId` was invisible to Atlas code and still load-bearing.
