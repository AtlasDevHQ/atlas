...
<Callout type="info" title="Read-only surface, plus a curated read-safe set">
  ...
  return data without mutating workspace state: `POST /api/v1/query`,
  ...
  guarantee (SQL on these paths runs through the SELECT-only validation pipeline; explore
  is read-only by sandbox isolation). Every other write — including
  `POST /api/v1/chat` — and every admin/platform operation is hard-blocked from grant and
  execution and hidden from discovery. The write approval-strength policy below is the
  enforcement each capability *carries for when true writes become reachable* — it is not
  a live write path yet.
</Callout>
