...
Atlas supports workspace-level model routing as an enterprise feature. Each workspace can configure its own LLM provider and API key, overriding the platform default. This enables enterprise customers to use their own Anthropic, OpenAI, Azure OpenAI, or custom OpenAI-compatible endpoints.
...
Model routing requires an enterprise license. Set `ATLAS_ENTERPRISE_ENABLED=true` and configure `ATLAS_ENTERPRISE_LICENSE_KEY`, or set `enterprise.enabled` and `enterprise.licenseKey` in `atlas.config.ts`.
