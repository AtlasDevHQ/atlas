...
Rate limiting is **disabled by default**. When enabled, all API routes are covered — `/api/chat`, `/api/v1/query`, `/api/v1/conversations`, admin endpoints, and more.
...
The `retryAfterSeconds` field is clamped to `[0, 300]` — the maximum wait the server will ask for is 5 minutes.

---

## SDK Error Handling

The `@useatlas/sdk` surfaces rate limit errors as `AtlasError` instances with the `rate_limited` code and a `retryAfterSeconds` property.
...
try {
  ...
  if (error instanceof AtlasError && error.code === "rate_limited") {
    ...
    console.log(`Rate limited — retry in ${error.retryAfterSeconds}s`);
  }
}
```
...
const result = await withRetry(
  (signal) => atlas.query("Revenue by region"),
  { maxRetries: 3 },
);
```
</Tab>
...
3. The countdown ticks down each second and disappears when it reaches zero
...
# Review audit log for rate-limited queries (logged with "Rate limited" error)
# In the admin console: Admin > Audit Log, filter by failed queries
```

You can also check the API logs — rate limit rejections are logged at `warn` level with the rate limit key and retry delay:

```bash
# Set debug logging to see rate limit decisions
