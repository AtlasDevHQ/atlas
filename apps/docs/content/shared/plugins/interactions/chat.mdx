...
|----------|-------------|
| Slack | [Slack adapter options](#environment-variables) / [Dedicated guide](/plugins/interactions/slack) |
| Microsoft Teams | [Teams adapter options](#environment-variables) / [Dedicated guide](/plugins/interactions/teams) |
| Discord | [Discord adapter options](#environment-variables) |
| Google Chat | [Google Chat adapter options](#environment-variables) / [Dedicated guide](/plugins/interactions/gchat) |
| Telegram | [Telegram adapter options](#environment-variables) / [Dedicated guide](/plugins/interactions/telegram) |
| GitHub | [GitHub adapter options](#environment-variables) / [Dedicated guide](/plugins/interactions/github) |
| Linear | [Linear adapter options](#environment-variables) / [Dedicated guide](/plugins/interactions/linear) |
| WhatsApp | [WhatsApp adapter options](#environment-variables) / [Dedicated guide](/plugins/interactions/whatsapp) |
...
| Migration from deprecated plugins | [Migrating from @useatlas/slack](#migrating-from-useatlasslack) / [Migrating from @useatlas/teams](#migrating-from-useatlasteams) |
...
| `onBridgeReady` | `function` | No | Called at the end of `initialize()` with a narrow `ChatResumeBridge` capability (and with `null` on `teardown()`), so the host can post into threads outside the request cycle — e.g. delivering a parked turn's continued answer once its approval resolves. The plugin works unchanged when omitted. |
| `observeMessage` | `function` | No | Async observer invoked for every inbound channel message the bridge dispatches, ahead of the handler that owns it (never-throw / never-block contract). When omitted, no observer is registered. Atlas wires it to the Company Brain chat webhook fast-path. |
