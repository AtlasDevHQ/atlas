...
Introspection moves from static functions on the `connection` namespace (`connection.listObjects(url, config)` / `connection.profile(url, config)`) onto the **built connection** returned by `create` / `createFromConfig` — `PluginDBConnection` gains optional `listObjects?()` / `profile?()` bound to the creds that built it (option shapes `PluginConnectionListObjectsOptions` / `PluginConnectionProfileOptions` — no `url`/`config`). `@useatlas/plugin-sdk` 0.0.12 → 0.0.13, under the publish-before-ref-bump discipline (consumer refs bump in a follow-up after publish).

The four MCP-provisionable plugins (ClickHouse / Snowflake / BigQuery / Elasticsearch) expose the relocated introspection on the connection `createFromConfig` builds, and `resolveLiveConnection` consumes the built connection's `profile` with **no host shim** (a built connection without `profile` is an actionable `unsupported`). The plugins keep their namespace `listObjects`/`profile` exports for the CLI direct-export path and the in-product wizard — the only remaining namespace consumers — whose convergence onto the built connection (and the namespace removal + DuckDB / dormant-Salesforce relocation) is the tracked follow-up; neither is reached by `resolveLiveConnection` over MCP. Mirror-drift discipline (above) now also covers `PluginConnection*Options` ↔ the host's `LiveConnection*Options`.
...
## Amendment (#3670 closeout): wizard + CLI convergence — one profiler home

**Status:** Accepted (#3670). Completes the wizard + CLI half deferred above: there is now exactly ONE profiler implementation per datasource type, reached identically by MCP, the in-product wizard, and the CLI — the built connection's introspection. The parallel homes, the namespace exports, and the native adapter shims are deleted.

- **Wizard → one context resolver.** `api/routes/wizard.ts`'s copy-pasted resolve→detect-dbType→capability prologue (#3657) collapses into `resolveWizardConnection` (`lib/datasources/wizard-connection.ts`), built on the SAME `resolveLiveConnection` MCP uses. `/profile` reads `connection.listObjects`, `/generate` + `/enrich` read `connection.profile` — off the resolved live connection, no url/config threading. SaaS (workspace_plugins) is the primary path; the `ATLAS_DATASOURCE_URL` `default`/`__demo__` fast path is a registry-gated, native-only self-hosted byproduct. `resolveWizardProfiler` is deleted. `resolveLiveConnection`'s `ok` result now carries `defaultSchema` so the wizard reports the effective schema without re-deriving resolution.
- **CLI → plugin introspection.** `packages/cli/lib/profilers/*` (the ClickHouse profiler, the duplicate DuckDB profiler, the Salesforce positional shim, the Elasticsearch re-export) is deleted. `atlas init` / `diff` / `improve` consume each plugin's `src/profiler` module directly (the one home); CSV/Parquet ingestion moves to `lib/duckdb-ingest.ts` (a write path, not profiling). Native pg/mysql stay core.
- **Namespace exports deleted.** All six datasource plugins drop `connection.listObjects` / `connection.profile`; the SDK `AtlasDatasourcePlugin.connection` type, the `definePlugin` validation, and the structural `DatasourceConnectionShape` drop them too. Introspection lives ONLY on the built connection (`PluginDBConnection.listObjects` / `.profile`). DuckDB + Salesforce relocate their introspection onto `createFromConfig` (the two stragglers this ADR named).
- **Native profilers in the options shape at source.** `profilePostgres` / `profileMySQL` / `listPostgresObjects` / `listMySQLObjects` take a single-options object, so a native profiler IS a `DatasourceProfiler` — `resolveProfiler` returns it directly, no positional→options shim at any call site (#3666). The dead `resolveWizardProfiler` native branch and its capability shims went with that module.
- **`resolveProfileCapabilityByDbType` (the create_datasource hint proxy)** no longer reads the deleted namespace `profile`; since profiling rides the built connection, profilability is connectability — it checks `createFromConfig`, the same predicate provisioning uses, and drops the unused `profileFn` from `ProfileCapability`.
- **Enforcement (AC #1).** `datasources/__tests__/one-profiler-home.test.ts` asserts the negatives — no plugin exposes a connection-namespace `listObjects`/`profile`, and `packages/cli/lib/profilers/` is gone — alongside `universal-profiling-enforcement.test.ts`'s positive (every connectable type is profilable via the one resolver). A new datasource can't reintroduce a second home without going red.

## Amendment (#4197): profileTable convergence + one native dispatch home

**Status:** Accepted (#4197). Finishes arch-win #96's convergence (`.claude/research/architecture-wins.md` §96, "one profiler home" — not GitHub issue #96): the agent's `profileTable` tool was the last profiling surface off the seam — a hand-rolled, native-only sampling path (its own count/colinfo/distinct/top-values queries over the registry pool, with its own `dbType === "mysql"` branches, untestable and broken for plugin dbTypes).

- **`profileTable` rides the one resolver.** The tool resolves `resolveProfilingConnection(connectionId, orgId)` — `lib/datasources/wizard-connection.ts` renamed to `lib/datasources/profiling-connection.ts` (and `resolveWizardConnection` → `resolveProfilingConnection`) now that the wizard routes AND the tool share it — and reads the connection's bound `profile({ selectedTables: [table] })`. Plugin dbTypes and OAuth datasources profile for free; the executeSQL-mirroring whitelist + mode gates still run first, and the tool `close()`s the connection it resolved. Its output maps the unified `TableProfile` (sampleValues replace the bespoke topValues/min/max sampling).
- **One pg-vs-mysql dispatch.** The per-dialect dispatch (duplicated across `resolveProfiler`, `resolveLiveConnection`'s native branch, and the wizard env-var byproduct) collapses into `nativeProfilerFor(dbType)`; the twice-hand-built native `LiveDatasourceConnection` shape collapses into `buildNativeLiveConnection` (both in `lib/datasources/native-live-connection.ts`, kept light so consumers don't drag the `mcp-lifecycle` graph into their static load path).
- **ADR-0019 untouched:** the profiler output shape consumed by the semantic generators is unchanged — this consolidates profiling *introspection*, not generation.
