...
**Note on just-bash:** The just-bash backend shares the same process as the API server. While secrets are not passed to sandbox commands, a sophisticated attack could theoretically access parent process memory. For multi-tenant deployments, use a higher tier.
...
| 5 | **just-bash** | Fallback. OverlayFS + path-traversal protection only |

Key behaviors:

- **Sidecar skips nsjail auto-detection.** When `ATLAS_SANDBOX_URL` is set, the sidecar is the intended backend (e.g. Railway). nsjail auto-detection is skipped entirely to avoid noisy namespace warnings on platforms that block `clone()`.
