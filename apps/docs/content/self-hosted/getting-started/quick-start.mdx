...
For most people, scaffolding a new Atlas app is the right starting point. Demo data seeds by default:
...
The scaffold creates a runnable Atlas project with the canonical NovaMart e-commerce dataset (52 tables, ~480K rows) pre-seeded and a semantic layer auto-generated. It defaults to SQLite + Anthropic — pass `--defaults` for non-interactive setup, or follow the prompts to pick PostgreSQL / OpenAI / Bedrock / Ollama / Gateway.
...
- **[Bun](https://bun.sh/) v1.3.14+** — check with `bun --version`
...
Restart your terminal after installing. Verify with `bun --version` — you need v1.3.14 or later.
</Accordion>
</Accordions>
