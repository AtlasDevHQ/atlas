...
The 1.5.1 arc deliberately picked Effect's `Context.Tag` over a bare boolean flag because every subsystem already needed multiple methods (residency: 8; SLA: 6; backups: 5). A `Tag` lets the AGPL core express "I yield* this enterprise capability" without committing to whether `@atlas/ee` is on disk. The Noop default closes the loop: self-hosted runs the no-op implementation; SaaS overlays `ee/src/layers.ts:EELayer` via the lazy import in `ConditionalEELayer`.
