...
| `perOrg.warmupProbes` | `number` | `2` | Warmup probes when an org pool is first created |
| `perOrg.drainThreshold` | `number` | `5` | Consecutive query errors before auto-drain of an org pool |

<Callout type="info">
Per-org pooling requires multi-tenancy (Better Auth organization plugin). When not configured, all orgs share the global pool. See [Multi-Tenancy](/guides/multi-tenancy) for the full setup.
</Callout>
