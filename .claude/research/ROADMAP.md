...
- [ ] Hosted MCP endpoint with SaaS auth (#2024) — OAuth 2.1 + DCR + `bunx … init --hosted` loopback flow shipped end-to-end across PRs #2054/#2056/#2057/#2059; Settings → OAuth Clients UI and `mcp.useatlas.dev` DNS still open. (Sub-issue #2028 closed.)
- [x] Decide standalone `bunx @useatlas/mcp serve` story — vendored demo runtime vs hosted-only (#2052, PR #2060) — path A (hosted-only) recorded; serve error and docs now point users at `bunx @useatlas/mcp init --hosted` instead of the (now-shipped) #2024 tracker.
...
- [x] List Atlas in MCP registries (#2027, PR #2061) — `io.github.AtlasDevHQ/atlas` v0.0.2 live on `registry.modelcontextprotocol.io` (auto-published via OIDC on every `mcp-v*` tag); mcp.so submitted as `chatmcp/mcpso#2228`. Claude Desktop catalog deferred — Anthropic-curated form, not auto-pulled from the registry.
