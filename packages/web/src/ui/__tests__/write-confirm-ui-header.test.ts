...
describe("web chat surfaces declare the confirm-before-write banner (#5495)", () => {
  ...
  it("the widget package deliberately does NOT — it has no confirm card to render", () => {
    // If this ever fails, `@useatlas/react` grew the header. That is only
    // correct alongside a real confirm card in its own `tool-part.tsx`; adding
    // the header alone re-opens #5495 for widget users.
    const widget = readFileSync(
      resolve(WEB_ROOT, "../react/src/hooks/use-atlas-chat.ts"),
      "utf8",
    );
    expect(widget).not.toContain(HEADER);
  });
});
