...
- **`executeSQL`** -- SQL query execution. Args: `{ sql: string, explanation: string, connectionId?: string }`. Result: `{ success: boolean, columns: string[], rows: Record<string, unknown>[], truncated?: boolean }`.
...
Each tool invocation appears as a message part with these fields:

```typescript
// Tool call part (while the tool is running or after it completes)
{
  type: "tool-invocation",    // always "tool-invocation"
  toolInvocationId: string,   // unique ID for this invocation
  toolName: string,           // "explore" | "executeSQL"
  state: string,              // "partial-call" | "call" | "output-available"
  input: { ... },             // tool arguments (shape depends on toolName)
  output: unknown             // tool result (only present when state is "output-available")
}
```

The `state` field progresses through the lifecycle: `"partial-call"` (arguments streaming in), `"call"` (arguments complete, execution started), `"output-available"` (result ready). You typically only need to render `"call"` (show a loading state) and `"output-available"` (show the result).
...
3. **Tool result rendering** -- detect `type: "tool-invocation"` parts and render SQL results as a table
