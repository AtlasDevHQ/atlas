...
For most people, scaffolding a new Atlas app is the right starting point. One command, demo data pre-seeded:
...
The `--demo` flag scaffolds a runnable Atlas project with the canonical NovaMart e-commerce dataset (52 tables, ~480K rows) seeded into a local Docker Postgres. The scaffold walks you through picking an LLM provider and wiring its API key into `.env`.

Once it's running, jump to [Ask your first question](#ask-your-first-question) below.
