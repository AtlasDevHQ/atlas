...
> the prose below is that file's `## Lead source (CRM acquisition)` section verbatim — only the relative links are repathed for the new depth — and it is no longer there.
