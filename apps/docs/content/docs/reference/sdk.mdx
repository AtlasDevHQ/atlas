...
for await (const event of atlas.streamQuery("Revenue by region last quarter")) {
  switch (event.type) {
    ...
    case "result":
      ...
      break;
    case "finish":
      // Stream complete — reason is "stop", "length", or "tool-calls"
      console.log(`\nDone (${event.reason})`);
      break;
  }
}
...
try {
  for await (const event of atlas.streamQuery("...", {
    signal: controller.signal, // Pass the signal to enable cancellation
  })) {
...
// Soft-delete the task (preserved in audit trail)
...
const stats = await atlas.admin.semantic.stats(); // Coverage percentages
