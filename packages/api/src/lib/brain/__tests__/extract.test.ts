 ...
 *   4. **The drain's backing-off exclusion.** `extract-reconcile-pg.test.ts`
 *      proves a poisoned head does not stall the queue against real Postgres,
 *      but it SKIPS without `TEST_DATABASE_URL` — and the one failure it cannot
 *      catch anywhere is the cast: `id <> ALL($2::text[])` against a `uuid`
 *      column fails only when Postgres sees it. So the two assertions that need
 *      no DB live here: the SQL says `uuid`, and the exclusion set is the
 *      quarantined population and nothing else.
 */
...
// Static: a `type` binding inside the dynamic import's object pattern is a syntax error.
import type { QuarantineEntry } from "@atlas/api/lib/brain/extract";
...
const entry = (overrides: Partial<QuarantineEntry> = {}): QuarantineEntry => ({
