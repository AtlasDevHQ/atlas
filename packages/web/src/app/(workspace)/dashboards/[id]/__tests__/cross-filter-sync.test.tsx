...
// ---------------------------------------------------------------------------
// Merged from drilldown-sync.test.tsx (#3212) — the same `../search-params`
// URL-state seam driven through the same real nuqs adapter. `REGION`, the
// `seeded()` wrapper and `afterEach(cleanup)` above were verbatim copies;
// only the one-parameter harness travels, as `DrilldownHarness`. It writes
// via `withOverride` where the suite above writes via `toggleOverride`, and
// it is the only place the bar's own Reset button is exercised.
// ---------------------------------------------------------------------------
...
function DrilldownHarness({ onChange }: { onChange: (o: ParameterValues) => void }) {
  const [raw, setRaw] = useQueryState(DASHBOARD_PARAMS_KEY, dashboardParamsParser);
  return (
    <>
...
describe("drilldown ↔ parameter bar URL sync (#3212)", () => {
  test("a drilldown write reflects in the bar and emits the bound override", async () => {
    const onChange = mock((_: ParameterValues) => {});
    render(<DrilldownHarness onChange={onChange} />, { wrapper: seeded() });
  ...
  test("Reset clears the active override back to defaults", async () => {
    ...
    render(<DrilldownHarness onChange={onChange} />, { wrapper: seeded({ region: "us" }) });
