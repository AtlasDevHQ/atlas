...
<Callout type="warn">
**Telegram / Microsoft Teams / Google Chat / WhatsApp connect routes are removed ([#2994](https://github.com/AtlasDevHQ/atlas/issues/2994)).** Their legacy connect endpoints (`POST /telegram`, `POST /teams/byot`, `POST /gchat`, `POST /whatsapp`) wrote only a per-platform credential store and **no `workspace_plugins` row**, so they bypassed the per-tier chat-integration cap ([#2953](https://github.com/AtlasDevHQ/atlas/issues/2953)) and never produced a runtime-routable install (the chat runtime routes purely off `workspace_plugins.config`). These four static-bot platforms are not installable until the unified ADR-0007 static-bot install (routing-identifier capture + cap-gated `workspace_plugins` write, like Discord) is wired — tracked in the [#2994](https://github.com/AtlasDevHQ/atlas/issues/2994) sub-issues. The `DELETE` disconnect routes remain so existing tenants can clean up.
</Callout>
