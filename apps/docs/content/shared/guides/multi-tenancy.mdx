...
Cache behavior is tuned at runtime via the settings registry — **Admin → the
Cache page** or the `ATLAS_CACHE_*` env vars (`ATLAS_CACHE_ENABLED`,
`ATLAS_CACHE_TTL`, `ATLAS_CACHE_MAX_SIZE`) — with no redeploy. A `cache:`
block in `atlas.config.ts` is no longer supported and fails validation at
boot. See <AudienceLink selfHosted="/self-hosted/deployment/cache-configuration">Cache Configuration</AudienceLink> for details.
