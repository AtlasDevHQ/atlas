...
- A user with the `admin` role in your workspace — managed auth and the internal database are built into the hosted platform
</Callout>
</WhenSaaS>
...
A **Delivery Channels** card at the bottom summarizes all available notification channels (email, Slack, webhook) for use with <AudienceLink saas="/guides/scheduled-tasks">scheduled tasks</AudienceLink>.

---

## Install models

Each catalog entry advertises one **install model** that drives how the Connect button behaves. The unified install router (`/api/v1/integrations/:platform/{install,callback,install-form}`) dispatches by `plugin_catalog.install_model`:
...
The unified install pipeline splits the install responsibilities between two roles. This separation is what makes the SaaS deploy at `app.useatlas.dev` self-serve without per-tenant operator work.

<WhenSaaS>
On the hosted platform, **Atlas is the operator** — the App Registrations and credentials described below are already configured for every OAuth platform. Your part is the customer-admin half.
</WhenSaaS>

<WhenSelfHosted>
On a self-hosted deployment, **you are the operator** — you create each provider App Registration under your own account and wire its credentials into the API service, once per platform.
</WhenSelfHosted>

**Operator (once per Platform — at deploy time)**
...
<WhenSaaS>
The Slack App Registration is already configured on the hosted platform — there is no operator step. Click **Connect to Slack** on the Slack card in `/admin/integrations`. Atlas redirects to Slack's OAuth flow; on return the bot token is persisted via the chat-adapter store and the install metadata lands in `workspace_plugins` keyed to your workspace.
</WhenSaaS>
...
OAuth is always the path on the hosted platform. If for some reason OAuth isn't configured on the deploy, the card falls back to the BYOT token form (the platform always has an internal database). The card only shows **Not Available** when neither OAuth nor BYOT is possible.
</WhenSaaS>
...
Workspace admins see a **Connect to Teams** button if the platform has Teams configured — it initiates the Azure AD admin-consent flow, and the callback captures the Azure-verified tenant id. See the [Microsoft Teams integration guide](/integrations/teams) for the full per-workspace flow (manifest distribution + admin consent). If the platform doesn't have Teams configured, the card falls back to the BYOT credentials form. The card only shows **Not Available** when neither OAuth nor BYOT is possible.
</WhenSaaS>
...
1. **Platform bot install (recommended)** — Set `DISCORD_BOT_TOKEN` and `DISCORD_CLIENT_ID` (plus `DISCORD_PUBLIC_KEY` to verify inbound webhook signatures) as environment variables. The admin UI **Connect to Discord** button appears once `DISCORD_CLIENT_ID` is detected and starts the cap-gated bot-install flow at `/api/v1/integrations/discord/install`, which adds the operator's bot to the selected server and records the install through the chat-integration cap.
...
Workspace admins see a **Connect to Discord** button when the platform has the Discord bot install configured. Clicking it starts the cap-gated bot-install flow at `/api/v1/integrations/discord/install`, which adds the platform's bot to the selected server and records the install through the chat-integration cap. If the platform bot install isn't configured, the card falls back to the BYOT credentials form. The card only shows **Not Available** when neither the platform bot install nor BYOT is possible.
</WhenSaaS>

<Callout type="warn">
The legacy OAuth connect flow — `/api/v1/discord/install` and `/callback`, gated by `DISCORD_CLIENT_SECRET` — was retired in #3145 and now returns **410 Gone**. It created uncapped installs that bypassed the chat-integration cap. Use the bot-install flow above; `DISCORD_CLIENT_SECRET` is no longer read.
</Callout>
...
Telegram integration requires an internal database — always present on the hosted platform, `DATABASE_URL` on a self-hosted deployment.
...
Google Chat integration requires an internal database — always present on the hosted platform, `DATABASE_URL` on a self-hosted deployment. It is always BYOT — workspace admins provide their own service account credentials.
...
GitHub integration requires an internal database — always present on the hosted platform, `DATABASE_URL` on a self-hosted deployment. It is always BYOT — workspace admins provide their own PAT.
...
Linear integration requires an internal database — always present on the hosted platform, `DATABASE_URL` on a self-hosted deployment. It is always BYOT — workspace admins provide their own API key.
...
WhatsApp integration requires an internal database — always present on the hosted platform, `DATABASE_URL` on a self-hosted deployment. It is always BYOT — workspace admins provide their own Cloud API credentials.
...
Email integration requires an internal database — always present on the hosted platform, `DATABASE_URL` on a self-hosted deployment. Each workspace admin configures their own email delivery provider.
...
### Agent tool — `sendEmail`

Once installed, Atlas's agent loop can deliver messages through the workspace's SMTP transport via the `sendEmail` tool. The agent resolves the active workspace at execute time and routes through the same encrypted credentials the form persisted — no operator env vars are required. If a workspace has no Email install, `sendEmail` returns an actionable error pointing the user back to `/admin/integrations` instead of failing with a generic 500.

The Resend-based `sendEmailReport` tool (operator-env-configured plugin under `plugins/email/`) remains available and uses a different tool name, so both can coexist in the same deploy — pick whichever credentials your workspace has set up.
...
<WhenSaaS>
Email delivery reverts to the platform's Resend baseline until you reconnect a provider.
</WhenSaaS>

<WhenSelfHosted>
Email delivery will fall back to environment variables (`ATLAS_SMTP_URL` or `RESEND_API_KEY`) or be disabled until reconnected.
</WhenSelfHosted>
...
The webhook card shows the count of active webhook delivery tasks. Webhooks are created via <AudienceLink saas="/guides/scheduled-tasks">Scheduled Tasks</AudienceLink> — the Integrations page provides a summary view.

<WhenSaaS>
With webhook support enabled, a **Create Webhook** link navigates to the Scheduled Tasks page.
</WhenSaaS>
...
| `DELETE` | `/api/v1/admin/integrations/slack` | Legacy Slack disconnect. Prefer `DELETE /api/v1/integrations/slack` (above) for new clients. |
| `DELETE` | `/api/v1/admin/integrations/discord` | Legacy Discord disconnect — tears down the self-hosted BYOT `discord_installations` row. |
...
<Callout type="warn">
**Telegram / Microsoft Teams / Google Chat / WhatsApp now install via the cap-gated static-bot flow** ([#3141](https://github.com/AtlasDevHQ/atlas/issues/3141)–[#3144](https://github.com/AtlasDevHQ/atlas/issues/3144), umbrella [#2994](https://github.com/AtlasDevHQ/atlas/issues/2994)): the admin supplies a routing identifier (chat_id / tenant_id / customer-id / phone_number_id) and `confirmInstall` writes a cap-gated, runtime-routable `workspace_plugins` row. **Disconnect goes through the unified `DELETE /api/v1/integrations/:slug`** — their legacy per-platform `DELETE` endpoints (and the backing `*_installations` tables) were removed in [#3161](https://github.com/AtlasDevHQ/atlas/issues/3161). Discord keeps its legacy `DELETE` because `discord_installations` still backs the self-hosted BYOT bot-token path.
</Callout>

---

## Jira (Lazy OAuth — #2659)
...
<WhenSaaS>
On the hosted platform the Atlassian OAuth app is already registered and configured — there is no operator setup. Skip to the admin flow below.
</WhenSaaS>
...
<WhenSaaS>
On the hosted platform the Salesforce Connected App is already registered and configured — there is no operator setup. Skip to the admin flow below.
</WhenSaaS>
...
Every catalog entry declares a minimum plan tier via `min_plan` in
`atlas.config.ts`. The vocabulary matches the workspace `plan_tier`
column on `organization`:

| Tier      | Notes                                                          |
| --------- | -------------------------------------------------------------- |
| `free`    | Accessible to every workspace.                                 |
| `trial`   | Trial workspaces and above.                                    |
| `starter` | Paid Starter and above. Slack ships at this tier on the hosted platform. |
| `pro`     | Pro and above.                                                 |
| `business`| Business / premium integrations (Salesforce ships here on the hosted platform). |

<WhenSaaS>
The tiers map to Atlas's paid plans — see <AudienceLink saas="/guides/billing-and-plans">Billing &amp; Plans</AudienceLink> for what each plan includes.
</WhenSaaS>

<WhenSelfHosted>
On a self-hosted deployment you declare the catalog — and each entry's
`min_plan` — in your own `atlas.config.ts`. **An entry that omits
`min_plan` defaults to `starter`**, and without billing configured every
workspace sits on the default `free` tier, so set `min_plan: "free"` on
entries every workspace should be able to install. The tier gate only
distinguishes workspaces if you wire up your own Stripe billing — see
[Self-Hosted Billing](/self-hosted/guides/self-hosted-billing).
</WhenSelfHosted>

The admin Integrations page renders a card per entry; a workspace
sitting below the entry's `min_plan` sees the card in a **locked**
visual state with a "Premium — requires &lt;plan&gt;" badge and a
disabled Upgrade CTA. The install / install-form / OAuth-callback
endpoints all return `403 plan_upgrade_required` when the workspace
plan does not admit the integration, with `{ required_plan, current_plan }`
in the response body for the UI to surface in an upgrade prompt.

Disconnect is intentionally **not** plan-checked: a workspace that
downgrades after installing an integration always sees a
"Configured but inactive — plan downgrade" banner with a working
Disconnect button so credentials can be cleaned up.

### Operator workspaces

The `organization.is_operator_workspace` boolean flag bypasses every
plan-tier check for the operator's own dogfood orgs — on the hosted
platform, Atlas's own per-region operator orgs. The flag is
operator-only — never exposed in customer-facing UI or billing
surfaces.
...
- <AudienceLink saas="/guides/scheduled-tasks">Scheduled Tasks</AudienceLink> — Create webhook delivery tasks
- <AudienceLink saas="/guides/admin-console">Admin Console</AudienceLink> — Overview of all admin pages
