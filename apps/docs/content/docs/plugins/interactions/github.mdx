...
The `adapters:` config field was removed in [#2650](https://github.com/AtlasDevHQ/atlas/pull/2650). Adapter activation is driven by `catalog` rows + per-Platform env vars; `chatPlugin()` itself takes no credentials. GitHub ships two install models — the OAuth-style App row landed in [#2818](https://github.com/AtlasDevHQ/atlas/pull/2818) and the static-bot PAT row landed alongside it ([#2807](https://github.com/AtlasDevHQ/atlas/pull/2807)). Pick exactly one row per workspace. See the [chat plugin docs](/plugins/interactions/chat#configuration) for the canonical shape.
</Callout>
