...
Metrics are computed over a configurable time window (default: 24 hours).

## Alerting

SLA alerts fire when workspace metrics exceed configured thresholds. Three alert types are supported:
...
| **Downtime** | 15 minutes | Consecutive downtime exceeds threshold |
...
|----------|---------|-------------|
| `ATLAS_ENTERPRISE_ENABLED` | `false` | Enable enterprise features including SLA monitoring |
| `ATLAS_ENTERPRISE_LICENSE_KEY` | — | Enterprise license key |
...
| `ATLAS_SLA_DOWNTIME_MINUTES` | `15` | Default downtime alert threshold (minutes) |
...
| `GET` | `/:workspaceId` | Per-workspace detail with time-series |
| `GET` | `/alerts` | List alerts (optional `?status=firing`) |
