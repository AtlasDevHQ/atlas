...
| `allowedCallbackHosts` | `string[]` | No | — | Extra hosts (`host[:port]`) a per-request `callbackUrl` override may target, in addition to the channel `callbackUrl`'s own host (#3347 SSRF allowlist). |
| `rateLimitRpm` | `number` | No | `60` | Per-channel requests-per-minute cap. Excess returns `429` with a `Retry-After` header. |
| `concurrencyLimit` | `number` | No | `3` | Per-channel concurrent in-flight cap. Excess returns `429` with a `Retry-After` header. |
| `requireTimestamp` | `boolean` | No | `false` | For `api-key` channels only: require the `X-Webhook-Timestamp` header and enforce the 5-minute window. HMAC channels always require it (subject to the legacy flag). |
...
**HMAC mode** (`authType: "hmac"`): Compute an HMAC-SHA256 of `${timestamp}:${body}` using the shared secret. Send the hex digest in the `X-Webhook-Signature` header AND send the same Unix-second timestamp in the `X-Webhook-Timestamp` header. Requests outside a 5-minute window or with a previously-seen signature are rejected.

```bash
TS=$(date +%s)
BODY='{"query": "How many active users last month?"}'
SIGNATURE=$(printf "%s:%s" "$TS" "$BODY" | openssl dgst -sha256 -hmac "your-secret" | cut -d' ' -f2)
...
<Callout type="warn">
**Wire-format change in v0.0.7.** The HMAC signing input is now `${timestamp}:${body}` (previously just the body). Upstream senders that haven't updated yet can be staged through the upgrade by setting `ATLAS_WEBHOOK_REPLAY_LEGACY=true`, which makes a missing `X-Webhook-Timestamp` header soft-fail with a warning log instead of a 401. Plan to flip the env var off within a week — default is fail-closed.
</Callout>
...
| `callbackUrl` | `string` | No | Overrides the channel-level callback URL for this request. Must be a valid HTTP/HTTPS URL (internal/private IPs are rejected), and its host must match the channel `callbackUrl`'s host or an `allowedCallbackHosts` entry — without a channel-level anchor, overrides are rejected outright. |
...
| `401` | Invalid API key, HMAC signature, missing/stale timestamp, or in-window replay |
...
5. For HMAC auth, use a **Code** node to compute the signature over `${timestamp}:${body}`, then set both `X-Webhook-Signature` and `X-Webhook-Timestamp` (Unix seconds).
