...
Atlas is a headless API. `@atlas/web` is the Next.js reference client that lives inside the Atlas monorepo at `packages/web/` — it ships with the repo and powers [app.useatlas.dev](https://app.useatlas.dev), but it is **not published to npm**. Any frontend that can make HTTP requests and consume a streaming response can replace it. The sections below describe its features so you know what to port; the source under `packages/web/src/ui/` is the canonical reference.
...
- Streams chat responses using the [Vercel AI SDK UI Message Stream Protocol](https://sdk.vercel.ai/docs/ai-sdk-ui/stream-protocol#ui-message-stream-protocol)
...
|-----------|-------|----------------|
| Nuxt (Vue) | [Nuxt](/self-hosted/frameworks/nuxt) | `@ai-sdk/vue` |
| SvelteKit | [SvelteKit](/self-hosted/frameworks/sveltekit) | `@ai-sdk/svelte` |
| React (Vite) | [React/Vite](/self-hosted/frameworks/react-vite) | `@ai-sdk/react` |
| TanStack Start | [TanStack Start](/self-hosted/frameworks/tanstack-start) | plain `fetch` / TanStack Query |
...
[
  ...
  { "type": "text", "text": "Now I'll query the data." },
  {
    ...
    "state": "output-available",
    "input": { "sql": "SELECT id, total_cents FROM orders LIMIT 5", "explanation": "Recent orders" },
    "output": {
      "success": true,
...
| | `POST /api/v1/chat` | `POST /api/v1/query` |
|---|---|---|
...
1. **A chat loop** -- `POST /api/v1/chat` with `{ messages }`, parse the Data Stream response (or use `useChat` from an AI SDK adapter)
...
Everything else -- conversation persistence, chart auto-detection, managed auth UI, markdown rendering -- is optional. The `packages/web/` reference client in the Atlas monorepo provides all of these, but none are required. If you just want a chatbot that queries your database, the three items above are sufficient.

## What the reference client (packages/web) adds

The `packages/web/` reference client (the Next.js app behind [app.useatlas.dev](https://app.useatlas.dev), monorepo-only — not on npm) adds these features on top of `@ai-sdk/react`. You can port any of them by reading the source in `packages/web/src/ui/`:
