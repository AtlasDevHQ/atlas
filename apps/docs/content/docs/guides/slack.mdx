...
When the [action framework](/guides/actions) is enabled and a query produces pending actions, Atlas posts approval buttons visible only to the requesting user. Clicking **Approve** or **Deny** triggers `POST /api/slack/interactions`.
...
**OAuth callback fails** -- Ensure the Redirect URL in your Slack app settings matches `https://your-api-host/api/slack/callback` exactly. Check that `DATABASE_URL` is set (required for storing OAuth tokens).
