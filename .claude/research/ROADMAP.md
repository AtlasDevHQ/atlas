...
## Shipped Milestones (0.6.0 → 1.4.2)
...
- [x] **1.4.4 — Multi-environment semantic layer** (#45, 11 + 17 audit + #2429 + 2026-05-16 dogfood follow-on) — connection groups foundation + admin merge-into-group wizard + Phase 4 archive cascade, group-scoped semantic / dashboards / scheduled tasks / approvals / PII, group-aware chat routing + per-turn override, application-layer FK gate for cross-org `connection_group_id` writes (#2424), legacy `connection_id` dropped (#2346 + 0069), `@useatlas/types` → 0.1.x. PRD #2336, audit #2407, mop-up #2442. Path-A real route-level e2e deferred to #2441. **Dogfood follow-on**: chat / notebook empty-state primitives (#2451 / #2452 / #2457), admin data-layer IA reshape PRD #2458 (all 5 slices shipped — `/admin/schema-diff` deleted via #2480 since pre-customer), SaaS plan + trial onboarding PRD #2464 (all 4 slices shipped), `/admin/connections` cache-poisoning root cause (#2444 / PR #2478). **Verification-pass batch (2026-05-16)** — 10/10 of #2481–#2490 shipped: first wave (8 parallel agents, ~1h) closed chat empty-state DB overlay (#2481), ConnectionRegistry boot-hydrate (#2482), SaaS demo-conn leak fix (#2483), Add Connection env field + 429 surfacing (#2484/#2485), admin MFA gate consistency (#2486), post-signup landing race (#2487), stale-bundle cache headers (#2488); second wave (2 parallel) closed `/admin` Overview platform/org split (#2489) + `/admin/connections` live-count parity (#2490); third wave (5 parallel) from a 2026-05-16 PM browser-driven verification pass closed useAdminFetch empty-path CORS (#2502), entity-count drift across admin surfaces (#2503), missing chat env picker (#2504), agent `default`-leak on SaaS (#2505), orphan empty env group (#2506); #2501 (Cloudflare CSP) awaits allowlist-vs-strip decision. **Code-work shipped; manual verification #2443 + the #2501 CSP decision are the gates on GH milestone close.**

---

## Active

_No active code-work milestone — 1.4.4 is code-complete pending the manual verification gate at #2443. 1.4.5 boots once #2443 closes._

## Planned

Two queued milestones — next-up decision lives in `/next`.

- **1.4.5 — Chat as dashboard editor** ([milestone #46](https://github.com/AtlasDevHQ/atlas/milestones/46), 8 issues) — PRD #2362 + tracer-bullet drawer (#2363), per-user drafts + Publish flow (#2364), stage tracker + ghost overlays (#2365), `screenshotDashboard` vision tool (#2366 spike → #2367), archived chat sessions (#2368), `createDashboard` reframe of the #2265 spike (#2369).
- **1.5.0 — Proactive Chat** ([milestone #43](https://github.com/AtlasDevHQ/atlas/milestones/43), 11 issues) — `/ee`-gated paid tier, Slack-first. PRD #2291 + reaction-first tracer (#2292), reaction-to-answer flow (#2293), admin opt-in (#2294), three-layer kill switch (#2295), meter + audit instrumentation (#2296), public-dataset surface for non-linked askers (#2297, HITL), feedback (#2298), sensitivity presets (#2299), one-time activation (#2300), monthly quota cap (#2301).
