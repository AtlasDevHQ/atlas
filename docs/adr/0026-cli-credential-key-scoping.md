...
ADR-0026 establishes the **credential spine** for workspace self-service — the **key-scoping detail under [ADR-0025](0025-workspace-self-service-surfaces.md)** (the umbrella: a workspace operates Atlas through one gated core; the MCP and a workspace CLI are two transports separated by **credential**, not by package). The default human credential is minted by `atlas login` via the OAuth 2.0 **device authorization grant** (RFC 8628, the `gh auth login` / `railway login` model); it resolves **live** to `{orgId, role}` through the same actor-resolution and governance rails the MCP uses, and is audited as `origin=cli`. This is the HITL **key-scoping** grill point: how a portable, file-stored credential maps to exactly one workspace at exactly the caller's role, and how role/membership changes clear immediately. Decided in the grill of issue #4043; this slice is the tracer the rest of the ADR-0025 surface depends on.
...
2. **Vanilla `getSession` resolves the *user-level* role.** `managed.ts` reads `effectiveRole` (stamped by `customSession` via `resolveEffectiveRole(user.role, userId, activeOrganizationId)`, which derives the member role internally), so a `platform_admin` user's session-class bearer would resolve to `platform_admin`. Left unchecked, a portable CLI bearer would carry cross-tenant god-mode. The key-scoping decisions exist to close exactly this gap.
...
- **`origin=cli`'s trust resolution + ceiling** are proven by focused tests: the new `cli` arm of `resolveMcpActorRole` (resolves org-role-only, withholds `platform_admin`); the `origin=cli` enum value plus both `origin IN (...)` CHECK constraints (so a `cli`-scoped approval rule / `cli`-origin queue row validates). Full dispatch-gate-chain coverage for a `cli` actor (clears RBAC at its org role, subject to approval like `origin=mcp`) is **deferred to the slice that wires the cli bearer into MCP dispatch** — this spine's runtime path is the REST read, where the gate chain does not fire. The **origin ceiling holds**: `origin=cli` can *tighten* (governance scoped to `cli` or `any` applies) but never *lower* governance, and it carries no authority the bound org role doesn't grant.
