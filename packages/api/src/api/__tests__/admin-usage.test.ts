...
const mockCurrentUsage = {
  queryCount: 42,
  tokenCount: 10000,
  // Internal-only field the rollup carries but the GET / wire contract must NOT
  // expose: costUsd (#4036 — Atlas's per-period gateway COGS). Present here so a
  // regression that spread `...usage` instead of picking explicit fields would
  // leak it and trip the assertion below.
  costUsd: 4.56,
  activeUsers: 3,
...
describe("Admin Usage API", () => {
  ...
  describe("GET /api/v1/admin/usage", () => {
    it("returns current period usage", async () => {
      ...
      // margin) must NOT reach a workspace admin. The handler picks explicit
      // fields rather than spreading the rollup; this pins that boundary so a
      // revert to `...usage` fails here.
