...
<Callout type="warn">
Set `secret_token` to the same value as the `TELEGRAM_WEBHOOK_SECRET` env var. When set, Telegram includes it in the `x-telegram-bot-api-secret-token` header on every webhook request, and the Chat SDK adapter verifies it automatically. On Atlas this secret is **required** — the AdapterRegistry refuses to build the Telegram adapter without `TELEGRAM_WEBHOOK_SECRET`, so an unsigned (forgeable) webhook can't reach the agent. See the [Telegram integration guide](/integrations/telegram#2-set-the-env-vars).
</Callout>
