...
describe("migrateAuthTables", () => {
  it("runs internal DB migration when DATABASE_URL is set", async () => {
    ...
    // migrateInternalDB: 3 audit_log + 4 conversations/messages + 2 starred column + 3 sharing columns + 3 slack + 5 action_log + 2 source tracking + 7 scheduled_tasks + 1 delivery_status ALTER + 1 connections + 3 token_usage + 5 invitations + 1 plugin_settings + 1 settings = 41 queries
