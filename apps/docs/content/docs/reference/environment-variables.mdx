...
Action defaults (approval mode, timeout, max per conversation) are configured via `atlas.config.ts` under `actions.defaults`, not environment variables. See the [declarative config guide](/docs/deployment/configuration) for details.
