...
- The three historical aliases — the YAML `connection:` field, the CLI `--source` flag, the admin/API `source` — are **deprecated and unified** to `group`. (See [Semantic layer scoping](../contexts/semantic-layer-scoping/CONTEXT.md) and [ADR-0012](../adr/0012-group-scoped-semantic-layer-directories.md).)
...
*As implemented (#3236):* the wizard's Review step is the two-phase surface. Phase 1 is the existing `/wizard/generate` (mechanical, no LLM — immediately saveable). Phase 2 is a new **per-table** `POST /wizard/enrich` route: it re-profiles the one table for fresh DB grounding, then runs the shared engine's in-memory `enrichEntityYaml` (extracted from the file-based `enrichEntity`, #3233, so CLI and web share the prompt + merge rules). The frontend fires one request per chosen table with bounded concurrency (`runWithConcurrency`) and upgrades each row the instant it settles — "streaming" without a streaming wire protocol; partial completion is safe because each table is independent. **Enrich all** is gated behind a confirm dialog that states the table count; **Enrich selected** enriches only checked rows; the **Ignore** list is pre-seeded from the profiler's `possibly_abandoned` flag (`seedIgnoredTables`) and excludes those tables from both enrichment and the final save. The route fails fast with a 503 (`getMissingModelConfig`) when no LLM provider is configured, so a key-less self-host shows one actionable banner instead of N per-table auth errors and the baseline stays saveable.
