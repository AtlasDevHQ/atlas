...
For **Organization only** shares, note that a *cross-site* iframe can't carry the viewer's session cookie (`SameSite`), so the embed shows a members-only sign-in notice instead of the transcript. Embed org-scoped shares same-site, or use a public share for third-party embedding.
