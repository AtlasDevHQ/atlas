...
function isSyncedSource(source: KnowledgeSource): boolean {
  return (
    ...
    source === "helpscout" ||
    source === "freshdesk" ||
    // #4770 / #4965: a brain source has no documents, but it DOES have sync
...
const CollectionListResponseSchema = z.object({
  collections: z.array(
    z.object({
      slug: z.string(),
      source: z.enum(["upload", "bundle-sync", "notion", "confluence", "confluence-datacenter", "gitbook", "zendesk", "salesforce-knowledge", "intercom", "front", "helpscout", "freshdesk", "slack-history", "zoom-transcripts"]),
