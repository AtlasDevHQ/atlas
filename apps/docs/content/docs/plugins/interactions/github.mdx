...
The App row covers both multi-tenant (auto-detect installation from webhook payloads) and single-tenant (fixed `GITHUB_APP_INSTALLATION_ID`) modes — set the env var to lock to one installation, or leave it unset for multi-tenant.
...
App ID, slug, private key, OAuth client credentials, and webhook secret come from env vars — see [Environment Variables](#environment-variables) below. Single-tenant mode is selected by also setting `GITHUB_APP_INSTALLATION_ID` to a fixed installation; leave it unset for multi-tenant.
...
|-----|--------|-----------------|-----------|-------|
| GitHub App | `"github"` | `"oauth"` | App ID + private key | Recommended; covers single- and multi-tenant via env-var presence of `GITHUB_APP_INSTALLATION_ID`. |
...
| `GITHUB_APP_INSTALLATION_ID` | Single-tenant App (`github-single-tenant`) | The operator's baked-in installation ID. Its presence selects single-tenant mode; leave unset for multi-tenant auto-detection. |
