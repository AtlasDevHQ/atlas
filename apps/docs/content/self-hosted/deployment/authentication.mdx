...
Full user management with email/password login, sessions, and role-based access. Built on [Better Auth](https://www.better-auth.com/). Requires an internal Postgres database (`DATABASE_URL`). Supports [social login providers](/guides/social-providers) (Google, GitHub, Microsoft) alongside email/password.
...
The recovery email is sent through whichever delivery backend you have configured for Atlas — see the [Email integration setup](/guides/integrations#email) for the workspace-level provider, or wire up `ATLAS_EMAIL_PROVIDER` / `RESEND_API_KEY` at the platform level. Resolution order:
...
- [Social Login Providers](/guides/social-providers) — configure Google, GitHub, and Microsoft sign-in
- [Choosing an Integration](/guides/choosing-an-integration) — compare auth modes side-by-side with setup examples
- [Row-Level Security](/security/row-level-security) — automatic data isolation per tenant using JWT claims
- [Actions Framework](/guides/actions) — approval-gated write operations (requires auth)
- [Troubleshooting](/guides/troubleshooting#auth-issues-operator) — common auth errors and fixes
