...
| **Visual query builder** | No (natural language) | Yes |
| **Dashboards** | No (chat + dashboards for exploratory analysis) | Yes (core feature) |
...
| **Data residency** | US-hosted, with EU and APAC on request | Cloud regions |
...
**Atlas** is an AI agent you embed in other applications or use via [Atlas Cloud](https://app.useatlas.dev). It does one thing -- lets users query data in natural language -- and does it as a composable component, not a standalone product. It also provides 8 integrations via Chat SDK (6 chat platforms plus GitHub and Linear), a plugin marketplace, and enterprise features (SSO/SCIM, PII masking, data residency).
