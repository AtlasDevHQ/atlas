...
When you have multiple connections sharing a logical schema (e.g. a `prod` group with `us-int`, `eu`, and `apac` replicas), each entity appears as **one row** in the tree — not three. A quiet trailing badge (e.g. `prod`) names the [environment](/semantic-layer/multi-env#connection-groups) the entity belongs to.
