 ...
 * at the auth gate. The F-09 rejection matrix (#1751) lives in the last
 * describe of this file; see security-audit-1-2-3.md F-09 for rationale.
 */
  ...
  describe("GET /api/v1/admin/abuse/:id/detail", () => {
    ...
    it("detail falls back to null AND surfaces a warning when name resolution rejects (#1640, #1751)", async () => {
      mockGetAbuseDetail.mockImplementation(async () => ({
        ...
        triggerCounters: null,
        thresholds: { queryRateLimit: 200, queryRateWindowSeconds: 300, errorRateThreshold: asRatio(0.5), uniqueTablesLimit: 50, throttleDelayMs: 2000, escalationCooldownMs: 60_000 },
        currentInstance: createAbuseInstance([]),
        priorInstances: [],
        eventsStatus: "ok",
      }));
...
// ===========================================================================
/**
 * F-09 platform-admin gate for /api/v1/admin/abuse/** — the subtree provides cross-tenant abuse
