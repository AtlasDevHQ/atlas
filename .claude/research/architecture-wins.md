...
- `matchScopeAcrossAliases({ leftAlias, rightAlias, column? })` produces the cross-alias equality used by every draft/published join: `COALESCE(d.connection_id, '__default__') = COALESCE(p.connection_id, '__default__')`. Five join sites collapse to one expression.
- `withGroupScope(orgId, scopeId)` returns a `GroupScope` with `.param` (null-normalised bind value) and `.match(paramIndex)` (the COALESCE'd equality clause against `$N`). One call site today (`deleteDraftEntity`), but the factory captures `orgId` so future expansion to a fused `org_id = $A AND <scope match>` clause stays additive.

**What got unbundled:**
- **Ten inline copies of `'__default__'` → one constant.** Changing the sentinel now requires editing one literal, not ten. The migration smoke test (`migrate-pg.test.ts`) still pins the index-builder SQL to the literal `__default__`, so drift between the helper and the migration would surface there immediately rather than as silent natural-key duplication at runtime.
- **The "five joins are all the same shape" invariant moved from convention to call site.** Pre-refactor, a contributor adding a sixth draft/published join could reasonably reach for an `IS NOT DISTINCT FROM` or an `IS NULL OR ... =` rewrite; the inline literals offered no guidance. Post-refactor, `matchScopeAcrossAliases` is the named pattern, and the four-line cross-alias JSDoc explains why COALESCE is used (partial unique index over the COALESCE'd expression — must match the index's expression exactly to hit the planner's index path).
- **The eventual `connection_id` → `connection_group_id` rename becomes parametric.** When the multi-env schema migration lands, the call sites pass `{ column: "connection_group_id" }` (or the helper's default flips), the ten consumers update in lockstep automatically, and the diff stays bounded to this module + a single migration. Without this win, the same rename is a ten-file sweep that has to be reviewed for "did each site swap consistently."

**Scope note — the audit overcounted by five.** The original audit (`.claude/research/multi-env-semantic-layer.md`) listed six tables (`semantic_entities`, `dashboard_cards`, `scheduled_tasks`, `conversations`, `approval_queue`, `pii_column_classifications`) as candidates. Verifying via grep showed that **only `semantic_entities` actually inlines the COALESCE-with-sentinel pattern** — its `connection_id` is nullable AND part of a partial unique key, forcing the COALESCE form. The other five tables filter by `connection_id` with plain equality (either `?? null` on insert or `connection_id = $X` on select); they have nothing to refactor at this stage. They'll adopt the helper when their schema migrates to `connection_group_id` and they need to participate in the same draft/published natural-key shape — which is the same migration that makes #2336's atomic publish across groups possible. The architecture-wins entry documents this explicitly so the next reader doesn't expect a six-file refactor and find a four-file one.

**Impact:**
- **10 inline COALESCE sites → 0** in non-test code (`semantic/entities.ts` × 6, `admin-publish-preview.ts` × 2, `mode.ts` × 1, `content-mode/tables.ts` × 1). The only remaining `'__default__'` references in non-test source are inside `with-group-scope.ts` itself (the helper body), `schema.ts:400` (the comment on the partial unique index documenting the migration-0028 invariant), and the `with-group-scope.ts` JSDoc body referencing the migration. The test mock in `admin-publish.test.ts` intentionally keeps the inline SQL strings — they are independent fixtures that the helper's output happens to match byte-for-byte, so the test stays green without modification.
- **17 new unit tests** in `with-group-scope.test.ts`, all pure (no DB, no I/O). Covers the four boundary cases the issue called out (null sentinel, single-member resolution, missing scope, sentinel-coalesce semantics) plus column/alias overrides for forward-compat.
- **All 35 affected isolated tests pass without modification.** Behaviour is byte-identical: the helper's output strings match the inlined SQL the call sites used previously, verified by every existing test in the semantic-entity / publish / mode / content-mode pipeline staying green.
