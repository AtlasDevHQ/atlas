...
Connect and manage external platform integrations — Slack, Microsoft Teams, and webhooks — from a single page. Each integration shows its connection status and provides connect/disconnect actions.
...
Configure which sandbox backend the explore and Python tools use. Supports per-workspace overrides in multi-tenant deployments.
...
| `GET` | `/sandbox/status` | Get sandbox configuration and available backends |
