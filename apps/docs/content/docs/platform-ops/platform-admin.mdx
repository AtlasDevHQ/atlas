...
- **Nothing is filtered silently.** A table absent from the region's schema, a table with no scope column of its own (`messages` and the eight others the purge reaches through a parent), and a scope column of an uncomparable type are each reported with their own reason, because each implies a different next move.
- **EXECUTE is triple-gated**: `ATLAS_RESIDUE_OK=1`, `--confirm`, and `--pg-dump <path>` naming a backup that exists and is non-empty. There is no undo, and a path that is merely recorded is a string that can lie.

Deletes are per (table, scope column) and not atomic across tables — a residue sweep is a cleanup taken against a backup, and one table's foreign key must not roll back the tables that succeeded. Statements that a `RESTRICT` blocks are retried until nothing more can progress; whatever is still failing is printed with its Postgres message and the command exits non-zero.
