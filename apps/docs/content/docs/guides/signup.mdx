...
Atlas is built for three regions and serves from one. The United States is available at signup; Europe and Asia Pacific are built and parked, and we bring one online on request (the signup picker offers a Request access link):
...
- **Data residency** — Workspace databases (identity included) live in the selected region. This is critical for organizations subject to GDPR, CCPA, or other data sovereignty regulations. One documented exception: agent sandbox tool execution runs in the US regardless of region — see [Sandbox Execution Locality](/platform-ops/data-residency#sandbox-execution-locality)
