...
Elasticsearch / OpenSearch datasources authenticate via `ATLAS_ES_*` environment variables — credentials are never carried in the URL. See [Environment Variables](/reference/environment-variables#elasticsearch--opensearch-datasource). The same applies to `diff`.
...
End-to-end verification of the demo → Twenty CRM lead-capture pipeline. It injects fixture personas **below** the form/Turnstile layer (straight into `crm_outbox` via `enqueue`), waits for the lead-outbox flusher to drain, then lists Twenty over REST and diffs the resulting Persons + Notes against what the fixture declared. Because it makes live Twenty network calls it is **not** part of per-PR CI, but it is not local-only either: an operator runs it ad-hoc, **and** it is the post-deploy **Staging Smoke** gate (.github/workflows/staging-smoke.yml (deleted)), which fires after a staging deploy to prove the flusher path is alive against the staging DB + Twenty. Changing or removing this subcommand will break that gate.
...
<Callout type="warn" title="Destructive — deletes orphaned-workspace rows from a region's internal DB">
Deletes **orphaned-workspace residue** — rows whose organization row is already gone, left behind by a purge that predated the table entering the purge path ([#5185](https://github.com/AtlasDevHQ/atlas/issues/5185)). The normal purge cannot reach them: it requires the workspace to exist and be soft-deleted. Candidate tables are derived from `lib/db/purge-scope.ts` (decision: `purged` only), and sentinel scope values (`_default`, `<atlas-operator>`, empty) are never deleted. **DRY RUN by default**; executing requires a **triple** gate: `ATLAS_RESIDUE_OK=1` in the env, `--confirm` on the command line, **and** `--pg-dump <path>` naming a real, non-empty backup file **less than 6 hours old**.
</Callout>

```bash
# Preview (dry run — reports the residue that would be deleted, deletes nothing)
bun run atlas-operator -- ops sweep-residue --region us

# Execute (triple-gated: env + --confirm + a fresh backup)
pg_dump "$ATLAS_REGION_US_DB_URL" > backup.sql
ATLAS_RESIDUE_OK=1 bun run atlas-operator -- ops sweep-residue \
  --region us --pg-dump ./backup.sql --confirm
```

| Flag | Description |
|------|-------------|
| `--region <us\|eu\|apac>` | Selects the region DB by resolving `ATLAS_REGION_<R>_DB_URL`. |
| `--database-url <url>` | Explicit region DB URL (escape hatch). **There is no `DATABASE_URL` fallback** — one of `--region` / `--database-url` is required. |
| `--pg-dump <path>` | Required to execute. Must name a real, non-empty backup file less than 6 hours old — the third gate. |
| `--confirm` | Required to execute. Pairs with `ATLAS_RESIDUE_OK=1` in the env. |
| `--dry-run` | Force preview even when the execute gates are satisfied. |

**Exit codes:** non-zero if anything survived the sweep or could not be checked — a scripted cleanup fails loudly rather than reporting a clean success.

### ops gate-export

<Callout type="warn" title="Exfiltration-gated — cuts tenant review decisions into a file">
Cuts an **EVALUATION bundle** of one workspace's review-gate decisions — (episode, decision, fact?) triples, with positives (published), rejected (retracted via the retract verb, which stamps `invalidated_at`, never `status`) and negatives (an extracted episode that yielded no claim) distinguishable. Warehouse observations are excluded: the publish gate refuses them, so no human ever ruled on one. ⚠️ **EVALUATION ONLY — never a training corpus** ([ADR-0044](https://github.com/AtlasDevHQ/atlas/blob/main/docs/adr/0044-fact-content-never-enters-model-weights.md), [#5339](https://github.com/AtlasDevHQ/atlas/issues/5339)). A bundle lives **outside** `purge-scope.ts` — cut it for a named evaluation and destroy it after. **DRY RUN by default**; executing requires **both** `ATLAS_GATE_EXPORT_OK=1` **and** `--confirm`.
</Callout>

```bash
# Preview (dry run — runs the identical query, reports exact counts, writes nothing)
bun run atlas-operator -- ops gate-export --workspace org_abc123 --region us

# Execute (double-gated)
ATLAS_GATE_EXPORT_OK=1 bun run atlas-operator -- ops gate-export \
  --workspace org_abc123 --region us --output ./gate-eval.jsonl --confirm
```

| Flag | Description |
|------|-------------|
| `--workspace <orgId>` | Required. The single workspace whose gate decisions are exported. |
| `--region <us\|eu\|apac>` | Selects the region DB by resolving `ATLAS_REGION_<R>_DB_URL`. The command **refuses to cross a region boundary**. |
| `--database-url <url>` | Explicit region DB URL (escape hatch). **There is no `DATABASE_URL` fallback** — one of `--region` / `--database-url` is required. |
| `--output <path>` | Bundle output path. |
| `--confirm` | Required to execute. Pairs with `ATLAS_GATE_EXPORT_OK=1` in the env. |
| `--dry-run` | Force preview even when the execute gate is satisfied. |

**Safety:** refuses a workspace carrying a grant token outside the ACL grammar. Every run — refusals and dry runs included — lands in `admin_action_log`.
