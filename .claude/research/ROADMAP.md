...
## Shipped Milestones (0.6.0 → 1.5.3)
...
- [ ] **1.6.0 — CRM & Lead Capture** ([milestone #52](https://github.com/AtlasDevHQ/atlas/milestone/52), [PRD #2726](https://github.com/AtlasDevHQ/atlas/issues/2726)) — Demo lead → Twenty CRM person, signup hook, talk-to-sales form, durable outbox, admin override + retry surface. Twenty-as-Atlas-datasource slice deferred to a future "generic REST datasources" milestone (#2728 closed with reasoning).
  ...
  - [x] Slice 8 (#2734 / PR #2844) — `crm_outbox` depth gauges + threshold-warn so ops sees backlogs before they snowball; depth + oldest-pending-age sampled on each flusher tick.
  - [x] Slice 9 (#2735 / PR #2845) — platform-operator UI at `/platform/crm-outbox` for `crm_outbox` inspection + manual `retry` / `mark-dead`. SaaS-gated through the `SaasCrm` Tag's `available` flag (self-hosted hides the nav link via the new `saasOnly` and falls through to the no-op layer's 404 envelope on direct access). `retry` preserves `attempts` so the deterministic backoff resumes from where it stopped — no foot-gun infinite-retry path. Both mutations audit-logged via new `ADMIN_ACTIONS.crm_outbox.{retry, markDead}`.
  - [x] Slice 10 (#2736 / PR #2846) — idempotent `atlas ops backfill-crm-leads` enqueues every existing `demo_leads` row into `crm_outbox` so historical demo signups also dispatch to Twenty; deduped via the per-source idempotency key.
  - [x] Slice 7 (#2732 / PR #2847) — admin UI at `/admin/integrations/twenty` to override the per-workspace Twenty plugin config; apiKey written via `workspace_plugins.config` selective-field encryption.
  - [x] Slice 11 (#2737 / PR #2848) — Stripe `customer.subscription.created` webhook enqueues a `conversion` lead for the Better Auth signup-stamped Person; trial-gated and idempotent on `stripe_customer_id`.
  - [x] Closeout follow-up (#2850 / PRs #2852 + #2854) — split Twenty credential resolver: `ee/saas-crm/` reads `TWENTY_API_KEY` env for Atlas's own lead pipeline, `plugins/twenty/` reads per-workspace DB config; `scripts/check-twenty-resolver-imports.sh` locks the seam.
