...
**Deep-module surface metrics:**
- **Interface:** `paginate` (driver) + `PaginatorRegistry.resolve` + `detectPaginationConfig` + the page-cache trio (`derivePageCacheKey` / `isPageFresh` / `invalidateInstallCache`), over one `PaginationStrategy.next` SPI. A strategy file's whole contract is `{ name, itemsPath, next }`.
- **Implementation:** four dialects, RFC 8288 `Link` parsing, no-progress/short-page/total-aware termination, SHA-256 canonical-params hashing (key-order-insensitive), watermark + TTL freshness, maxPages/maxItems safety bounds, merge across pages — all behind the driver.
...
- **30 tests, no live HTTP.** Table-driven `next()` per strategy (fixed request + response → expected next), the headline ~5,000-lead Twenty cursor walk merged through `executeOperationPaged` over the *real* operation graph with a paged `fetch` stub, and the cache lifecycle (per-page hit, 5-min TTL expiry, watermark flush, writes-never-cached, non-2xx-never-cached).
