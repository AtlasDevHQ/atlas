...
export default defineConfig({
  ...
  plugins: [
    ...
    snowflakePlugin({ url: process.env.SNOWFLAKE_URL! }),
  ],
});
```
...
Each sandbox plugin bakes in its own priority — E2B at 90, Daytona at 85 — so registration order doesn't matter. Atlas tries E2B first (priority 90); if `create()` throws it tries Daytona (priority 85); if both fail, the built-in chain takes over.
