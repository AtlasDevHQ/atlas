...
**Cons:** Lower throughput than vLLM, no continuous batching, limited serving options.
...
<Callout type="info">
**CPU-only inference** is possible with Ollama for 7-8B models (Q4 quantization) but is 10-50× slower than GPU. Not recommended for interactive use. Set `ATLAS_QUERY_TIMEOUT=120000` if using CPU inference to avoid timeouts.
</Callout>
...
For CPU-only: remove the `deploy.resources.reservations` block from the `ollama` service in the compose file.
...
Tested on a single A100 80GB GPU with Q8 quantization, 10-table demo schema.
...
Keep temperature low for SQL generation. Higher temperatures increase SQL syntax errors.

```bash
# In atlas.config.ts or via the provider's configuration
# Most self-hosted models work best at temperature 0.0–0.3 for SQL
```

Atlas uses the AI SDK's default temperature. For self-hosted models, the model server's default is typically fine. If you see inconsistent SQL output, reduce the temperature in your inference server config.
...
3. Increase `ATLAS_QUERY_TIMEOUT` for slow models: `ATLAS_QUERY_TIMEOUT=120000`
