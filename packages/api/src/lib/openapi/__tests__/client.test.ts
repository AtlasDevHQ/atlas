...
import { executeOperation, executeOperationPaged, parseRetryAfterMs } from "../client";
...
// ═══════════════════════════════════════════════════════════════════════════
//  Client x vendor-quirk integration (v0.0.2 slice 6a, #3028).
//  Formerly client-quirk.test.ts. Uses stubbed fetch impls (no loopback
//  server), so it is independent of the http fixture above.
// ═══════════════════════════════════════════════════════════════════════════
/**
 * Proves the generic `executeOperation` / `executeOperationPaged` apply a
 * declarative {@link VendorQuirk} through the existing header + query seams —
 * required headers and query param-shaping (Stripe `expand[]`) — with NO
 * per-vendor code branch, and the headline AC: a Stripe-style cursor walk merges
 * across pages using the SAME generic cursor strategy (last-item-id dialect),
 * expand[] shaped on every page, cursor (`starting_after`) advancing by the last
