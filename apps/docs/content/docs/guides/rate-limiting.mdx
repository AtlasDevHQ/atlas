...
| `ATLAS_RATE_LIMIT_RPM_WORKSPACE` | `max(60, RPM)` | Permission-gated non-admin ceiling — `/api/v1/dashboards/*` today (#5191). Separate so a 20-card dashboard's render burst depletes neither the cheap-read allowance nor the admin one |
...
Rate limiting is **disabled by default**. When enabled, it covers the primary API routes: `/api/v1/chat`, `/api/v1/query`, `/api/v1/conversations`, `/api/v1/admin/*`, `/api/v1/dashboards/*`, `/api/v1/semantic/*`, `/api/v1/scheduled-tasks`, and `/api/v1/actions`. Health, auth, widget, and OpenAPI spec endpoints are exempt.

### Per-bucket isolation

Each caller has four independent sliding windows keyed on the same identity (user ID or IP). Hitting one bucket's ceiling never throttles the others:
...
| `workspace` | `/api/v1/dashboards/*` | Opening a 20-card dashboard fires one render per card, and refresh-all fires as many again. Separate from `default` so that burst doesn't deplete the same budget as chat, and separate from `admin` so it can't deplete the budget an operator needs to *fix* the workspace (#5191) |

Both the `admin` and `workspace` buckets default to `max(60, ATLAS_RATE_LIMIT_RPM)` — at least one request per second, and tracking the base RPM above that — so neither an interactive admin session nor a dashboard load runs out of budget on a deployment with a tight base limit. Tune `ATLAS_RATE_LIMIT_RPM_ADMIN` / `ATLAS_RATE_LIMIT_RPM_WORKSPACE` directly if your workspace has different needs.
...
- [Error Codes](/reference/error-codes) — Full catalog of `rate_limited` and other error codes
