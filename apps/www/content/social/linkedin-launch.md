...
Atlas ships with 8 database adapters (PostgreSQL, MySQL, BigQuery, ClickHouse, DuckDB, Snowflake, Salesforce, Elasticsearch/OpenSearch), 24 plugins (Slack, Teams, Discord, MCP server, webhooks), an embeddable React widget, TypeScript SDK, admin console, and enterprise features like SSO/SCIM, custom roles, and audit log export. It's built in TypeScript end-to-end (Hono + Next.js + Effect.ts + bun) and deploys with Docker, Railway, or Vercel. The core is AGPL-3.0 licensed — self-host the full product for free, every feature, no artificial limits. Atlas Cloud (app.useatlas.dev) is the managed option for teams that don't want to run infrastructure.
