...
"The process is the region" has one known exception at launch: **the agent's sandboxed tool execution**. On Atlas Cloud, `explore` and `executePython` run in Vercel Sandbox, and Vercel Sandbox provisions exclusively in `iad1` (US East) with no region primitive — verified conclusively 2026-07-16 against `@vercel/sandbox` 2.7.0 (no create-time region parameter; `region` exists only as a read-only instance getter), Vercel's docs (sandboxes "automatically provision in `iad1`"), the absence of any project- or team-level placement setting, and nothing publicly roadmapped. All three regional API processes share one Vercel team/project (`deploy/api/atlas.config.ts`), so EU/APAC workspaces' sandbox payloads execute in the US.

**What transits is customer data, not metadata:** the semantic-layer files copied into the sandbox for `explore` embed sampled column values from the customer warehouse, and `executePython` writes actual SQL query-result rows into the sandbox. Counterweights: the sandbox's network policy is deny-all (code inside cannot make outbound connections), its filesystem is ephemeral (nothing persists at Vercel after the tool call), and the DPA sub-processor annex has disclosed the US processing since 2026-07-10 (Vercel entry: "semantic-layer content and query results") — the legal floor is covered; this amendment brings the product promise in line.
...
- [#4665](https://github.com/AtlasDevHQ/atlas/issues/4665) — finish Python support on the e2b/daytona BYOC sandbox providers. BYOC already routes above the platform `vercel-sandbox` pin and fails closed, so a workspace-connected sandbox on a region-controlled provider gives real in-region execution today for `explore`; only `createPython` is missing.
