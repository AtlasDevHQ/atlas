---
title: Query Caching
description: Cache query results for faster repeated queries with tenant-isolated, claim-scoped cache keys.
---
...
Claims are sorted alphabetically before hashing, so key order doesn't affect the cache key.
...
If the plugin backend fails at runtime, Atlas logs the error and falls back to the built-in LRU cache.
