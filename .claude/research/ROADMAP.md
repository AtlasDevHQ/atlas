...
- [x] Phase 1 — Audit Better Auth config + middleware coverage (#1720, PR #1734). All P0/P1/P2 findings fixed: F-01 → PR #1738 (+ #1742 for dashboards twin #1736, + #1749 for DB invariant #1737 which also revokes drifted share_tokens); F-02 → PR #1735; F-03 → PR #1744; F-04 → PR #1748; F-05 bundled into F-06; F-06 → PR #1739; F-07 → PR #1747. Follow-ups in milestone: share-token redaction #1743 (hardening on F-01 cluster), Better Auth rate-limit integration test #1741 (coverage on F-06)
...
- [ ] Phase 3 — Audit SQL validation edges + fuzz the 4-layer validator (#1722). Audit shipped (PR #1775, findings F-17..F-21). F-17/F-18/F-19 fixes in follow-up PR on `security/1.2.3-phase-3-validator-fixes`: F-17 `unwrapMysqlExecutableComments()` unwraps `/*!NNNNN ... */` before validation layers (Option A, 8 regression pins covering variant matrix + PG no-op); F-18 AST guard rejects `stmt.into?.type === "into"` when `keyword !== "var"` (excludes MySQL `@var` session assignment); F-19 `FORBIDDEN_PATTERNS` extended to `/\bINTO\s+(?:OUTFILE|DUMPFILE)\b/i`. Closes #1772 / #1773 / #1774. F-20 (quoted-identifier case-fold) + F-21 (dialect dangerous functions) stay in P3 tail
