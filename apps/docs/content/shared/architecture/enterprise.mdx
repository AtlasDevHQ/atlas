...
| `ee-stub-build` fails at `bun run type` | Core code references an EE-specific type via a structural path that the stub doesn't expose | Type-only imports from `@useatlas/types` or `@atlas/api/lib/effect/services` are fine; concrete imports from `@atlas/ee/...` are not. Move the type into a shared module |
| `ee-stub-build` fails at `bun run build` | A consumer call site assumes EE-only side effects (e.g. `Effect.provide(EELayer)` instead of `EnterpriseLayer`) | Use `runEnterprise(program)` / the Hono bridge's `runHandler` — they thread `EnterpriseLayer`, which composes Noop defaults under the conditional EE overlay |
