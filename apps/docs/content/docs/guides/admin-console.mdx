...
**Schema drift:** Drift between the live database schema and the semantic layer is surfaced inline on `/admin/semantic` — drifted entities are flagged in the tree, and the drawer shows added/removed columns and type mismatches with one-click reconcile actions. The drift-detection API (`GET /api/v1/admin/semantic/diff?connection=<id>`) returns structured JSON with `newTables`, `removedTables`, `tableDiffs`, `unchangedCount`, and a `summary` object.
