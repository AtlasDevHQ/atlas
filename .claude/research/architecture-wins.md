...
A second `/prod-audit` pass surfaced two more SaaS misconfig classes that fit the precedent without modification:
  ...
  7. **Rate limiting silently disabled in SaaS.** `auth/middleware.ts:38` parses `ATLAS_RATE_LIMIT_RPM` via `Number(raw)` and treats unset / empty / `0` / non-numeric as "rate limiting disabled". A SaaS region without an explicit env var ran without a per-user RPM ceiling — a DDoS hole that was discoverable only by traffic inspection. Identical shape to `InternalDbGuardLive` (env-direct check, fail with a tagged error in SaaS, no-op self-hosted) — `RateLimitGuardLive` lands in ~35 lines including the parser-mirror logic that catches typos.
