...
- `fork-pr-gate` — `.github/workflows/fork-pr-gate.yml`. Runs on **every** PR via `pull_request_target` and reports unconditionally (passes immediately for same-repo PRs), so it never leaves the merge BLOCKED-forever on internal PRs. For a PR from a **fork** it auto-applies the `external-fork` label and fails until a maintainer applies `external-approved` by hand. See [The fork-PR gate](#the-fork-pr-gate) below
