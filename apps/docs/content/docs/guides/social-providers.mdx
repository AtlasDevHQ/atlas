...
<Callout type="warn" title="Self-Hosted Only">
  Configuring social login providers requires editing the `betterAuth()` call in your Atlas source code. This applies to self-hosted deployments only. On [app.useatlas.dev](https://app.useatlas.dev), social login providers are managed by the Atlas platform — go to **Admin &gt; Authentication** to enable them for your workspace.
</Callout>
