...
- **Operator surface (`atlas.config.ts`)** declares **capability** — the set of Platforms and integrations that an Atlas deployment knows how to do, plus per-Platform App Registration credentials (one Atlas-owned App Registration per Platform per deployment, sourced from env vars).
- **Plugin Catalog** is the runtime registry. Seeded from `atlas.config.ts` on every boot in an idempotent pass. Holds plan-tier gating (`min_plan`), runtime enable/disable (`enabled`), and config schema per entry.
- **Customer surface (`/admin/integrations` page + per-Platform OAuth flow)** is where Workspace admins install / disconnect Platforms and integrations. Each install creates a `workspace_plugins` row referencing the catalog entry; per-Platform credentials persist to the adapter's native store (`chat_cache` for chat Platforms; per-plugin store for lazy integrations).
...
After this milestone, adding a new chat Platform for a customer becomes a self-serve admin action. Adding a new Platform *type* to the deployment is a one-time operator task (App Registration + env vars + catalog entry in `atlas.config.ts`), done once per Platform per deployment forever.
...
- **`PlatformOAuthHandler` interface (new) + one implementation per Platform (Slack first; Teams/Discord/etc. follow as 1-day PRs after this milestone closes)** — Methods: `startInstall(workspaceId): { redirectUrl, stateToken }` and `handleCallback(code, stateToken): { installRecord, credentialWritten }`. Implementations live under `packages/api/src/lib/integrations/oauth/<platform>.ts`. Slack implementation is the lift of the current `slack.ts` OAuth path.
...
- **New Platform adapters beyond the 8 already in `plugins/chat/src/adapters/`.** Teams / Discord / gchat / Telegram / GitHub / Linear / WhatsApp adapters already exist as code; this milestone enables them at the SaaS wiring layer, but landing them as fully working integrations (Teams Bot Framework subtleties, Discord intents, etc.) is per-adapter follow-up work. Each becomes a 1-day PR after this milestone closes.
