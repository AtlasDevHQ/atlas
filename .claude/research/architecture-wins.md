...
- **Four-layer enforcement at the API.** `/integrations/catalog` adds `accessible` + `upgradeRequired` to the per-entry response. `POST /install` and `POST /install-form` return `403 { error: "plan_upgrade_required", required_plan, current_plan }` on plan mismatch. `GET /callback` defensively re-checks at install-record-write time (downgrade-mid-OAuth case). `DELETE /:platform` intentionally NOT plan-checked so a downgraded customer can always clean up an existing install.
