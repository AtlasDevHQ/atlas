...
- **Cross-surface cache**: pinning in chat immediately updates the widget (and any other same-origin surface) without a re-fetch. The TanStack Query cache shares across surfaces.
