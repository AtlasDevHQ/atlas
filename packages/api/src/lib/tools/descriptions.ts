...
export const SEARCH_BRAIN_TOOL_DESCRIPTION = `Search the company brain — a trust-labeled read over three stores. \`tier: "fact"\` is human-reviewed; \`tier: "raw-episode"\` is its source record; \`tier: "document"\` is hosted knowledge — ${KNOWLEDGE_TRUST_FRAMING}. When a fact's \`provenance.attribution\` is \`{ "visible": false }\` its author, source id and timestamp are withheld: use the claim, say attribution is restricted, never call it anonymous, undated or unsourced, never infer the author. Retracted claims are excluded, drafts outside developer mode — see \`status\`. Unextracted episodes are tagged \`extraction: "pending"\`. Conflicting facts surface their unranked conflict cluster in \`tensions\`: each visible counterpart carries its claim and provenance — report every side with its evidence, never pick a winner — and a \`{ "visible": false, "withheldCount": N }\` entry means N conflicting claims are withheld from this reader: treat the claim as contested, never as settled. \`unavailable\` means the search failed — never report that as "nothing known". Example: \`{ "query": "who owns billing" }\`.
