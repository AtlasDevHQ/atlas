...
mutate "$DOC" "$DOC_BACKUP" 's/Seven are \*\*workspace-scoped\*\*/Seven of them are **workspace-scoped**/'
...
mutate "$DOC" "$DOC_BACKUP" 's/ Seven are \*\*workspace-scoped\*\* — meaning each can hold a different value per workspace, set by a platform admin: `ATLAS_BRAIN_AUDIENCE_SYNC_ENABLED`, `ATLAS_BRAIN_COVERAGE_SNAPSHOT_ENABLED`, `ATLAS_BRAIN_WAREHOUSE_CADENCE_ENABLED`, `ATLAS_BRAIN_SUGGESTER_ENABLED`, `ATLAS_BRAIN_EXTRACTION_MODEL` and the two alias auto-approval keys.//'
...
mutate "$DOC" "$DOC_BACKUP" 's/Seven are \*\*workspace-scoped\*\*/Five are **workspace-scoped**/'
...
mutate "$DOC" "$DOC_BACKUP" 's/Seven are \*\*workspace-scoped\*\*/7 are **workspace-scoped**/'
check pass "W-DIGITS: a digit count is read, not silently skipped"
restore_doc

mutate "$DOC" "$DOC_BACKUP" 's/Seven are \*\*workspace-scoped\*\*/8 are **workspace-scoped**/'
