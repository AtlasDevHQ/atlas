...
**Three carve-outs, all recorded here because the rule requires it.** Beyond the promotion adapter itself (`lib/content-mode/adapters/brain-facts.ts`, which *is* `promoteBrainFacts`), the allowlist names exactly three writers, and each has to argue that it is not making a new gate decision:
...
- **the alias decide transaction** (`lib/brain/vocabulary-decide.ts`, #5023 → #5024) — the *identity* arm's sanctioned writer. This carve-out was registered as a **pre-registration** in #5023, when the file wrote no gated column; **#5024 landed the write it reserved** and it is now an ordinary live entry. What it writes is `subject_key` / `predicate_key` / `object_key`, and only those, through one statement per slot position (`REKEY_DRIFTED_FACTS_SQL`) — ADR-0037 §7's drift re-key, in TypeScript at request time rather than in another migration, because the re-key is triggered *by* the decision, needs the workspace advisory lock the edge write already takes, and is the one place the vocabulary version that authorized it is known. The gate's own remediation text names it (*"Re-keying because the vocabulary changed? That is the alias-approval decide transaction"*). It is a **recomputation of a derived column**, not a promotion: it moves neither a claim's content nor its review state, which is also why it must not touch `updated_at`. The consequence the gate exists to prevent — *a re-key changes what a claim collides with, and the collision is what stamps `valid_to`* — is closed by the same slice rather than inherited: publish and the decide seam now share one advisory namespace (`IDENTITY_MUTATION_LOCK_NAMESPACE`, 5024) and `SUPERSEDE_STAMP_SQL` re-checks the collision join. **Its cost, recorded because the allowlist has no per-column granularity:** an allowlist entry exempts a *file*, so this one also exempts the decide seam for `status`, `visible_to` and `valid_to` — columns it has no business writing and does not write. What holds that in place is this register plus a **column-scoped assertion** in `vocabulary-decide-pg.test.ts`, which replaced the "the seam names no `brain_facts` at all" tripwire #5023 shipped. That tripwire could not survive #5024 by construction; it was retired deliberately and narrowed, not dropped. A gated column beyond those three is still a new argument, not an inherited one.
