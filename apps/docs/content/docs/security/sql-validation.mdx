...
description: How Atlas enforces read-only SQL execution through a multi-layer defense-in-depth pipeline.
---
...
| — | Per-source rate limiting | Atomic concurrency + QPM check before execution |
...
Layers 0–3 run in `validateSQL()` before execution. Between validation and execution, `acquireSourceSlot(connId)` checks per-source concurrency and queries-per-minute limits. If the source is at capacity, the query is rejected with a rate-limit error (and an optional `retryAfterMs` hint) before any database work begins. Per-source limits are configurable via `rateLimit` in `atlas.config.ts` datasource entries. Layers 4–6 are applied during execution.
...
RLS can be configured via [environment variables](/docs/reference/environment-variables#row-level-security) (single-policy shorthand) or declaratively in `atlas.config.ts` with the `rls` key (supports multiple policies targeting different tables and claims).
