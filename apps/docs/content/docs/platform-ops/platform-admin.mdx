...
- **It refuses a database it cannot reason about.** If `organization` has zero rows, every workspace-scoped row matches the orphan predicate and the "residue" is the whole tenant dataset — so the sweep aborts instead. `organization` is a Better Auth table and is *not* created by Atlas migrations, so a complete Atlas schema with no orgs is a real state: a partial restore, a scratch DB, a not-yet-provisioned region, a mistyped `--database-url`. An EXECUTE naming more than 50 distinct workspace ids is refused for the same reason — residue is the tail of past purges, not a population.
- **Nothing is filtered silently, and "not in scope" is reported separately from "could not be read".** A table absent from the schema, a table with no scope column of its own (`messages` and the ten others the purge reaches through a parent or an expression), and a scope column of an uncomparable type are *never candidates* — benign. A table the sweep could not read (no `SELECT` privilege, a lock timeout, an ambiguous schema) has an **UNKNOWN** residue state, is printed under its own heading, and **exits non-zero**. Presence is probed with `to_regclass` rather than `information_schema`, which is privilege-filtered and would report a permission problem as "run your migrations".
...
Deletes are per (table, scope column) and not atomic across tables — a residue sweep is a cleanup taken against a backup, and one table's foreign key must not roll back the tables that succeeded. Each `DELETE` re-asserts the orphan predicate itself, so a workspace re-created between the preview and the execute is spared. Statements that a `RESTRICT` blocks are retried until nothing more can progress; whatever is still failing is printed with its Postgres message and the row count that therefore survives.

The command exits non-zero on **any** of: a failed delete, an unreadable table, a blast-radius refusal, or a delete that removed more rows than the report listed. A scripted `for region in us eu apac` loop reads that exit code, so "we could not look" must never script as "it was clean".
