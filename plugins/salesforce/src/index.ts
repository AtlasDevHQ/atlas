 ...
 * adapter-only health branch. This module supplies the Salesforce substance:
 * the schema pair, the connection builders, the introspection bindings, the
 * SOQL validator, the describeGlobal health probe, and the static-mode
 * querySalesforce tool registration.
 *
 ...
 * ```
 */
export const salesforcePlugin = createDatasourcePlugin<
  SalesforcePluginConfig,
  z.infer<typeof SalesforceConnectionConfigSchema>,
...
>({
  ...
  connectionConfigSchema: SalesforceConnectionConfigSchema,
  ...
  // static and adapter-only modes. Replaces the standard SQL pipeline, so
  // there is no parserDialect / forbiddenPatterns here.
  ...
  // Mode-aware guidance: the dedicated `querySalesforce` tool is only
  // registered in static mode (see onInitialize). The adapter-only branch
  // describes the DORMANT credential-form bridge path (#3302 option a /
  // ADR-0014); because the bridge skips `salesforce`, it is unreachable in any
  // current Atlas deployment. SaaS per-workspace Salesforce is OAuth-installed
  // and its connection is built via the `LazyPluginLoader` from the OAuth
  // tokens — not `executeSQL` through this adapter.
  dialect: (hasStaticConfig) =>
    [
  ...
  // jsforce sessions are stateful, so the static connection is a cached
  // singleton: `connection.create()`, the health probe, the querySalesforce
  // tool, and the factory-emitted teardown share it. The static url is only
  // parsed on first use — never on the adapter-only build path.
  cacheStaticConnection: true,
  createStaticConnection: (rt) =>
    createSalesforceConnection(parseSalesforceURL(rt.config.url!), rt.logger),
  ...
  // credential-form path (#3302 option a) and self-host wiring. The runtime
  // url is parsed here (never at build time) — surfaces parser errors as a
  // thrown error for the datasource bridge to handle.
  buildConnection: (parsed, rt) =>
    createSalesforceConnection(parseSalesforceURL(parsed.url), rt.logger),
  ...
  healthProbe: async (rt) => {
    try {
      const conn = rt.staticConnection();
      ...
      rt.logger?.warn(`Health check failed: ${message}`);
 ...
 */
export const buildSalesforcePlugin = salesforcePlugin.build;
