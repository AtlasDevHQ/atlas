...
Atlas uses a split license: the server core (`@atlas/api`, `@atlas/cli`, `@atlas/web`) is **AGPL-3.0**, while the client libraries (`@useatlas/sdk`, `@useatlas/react`, `@useatlas/types`) and all plugins are **MIT**. All features are available in the open-source version — there are no proprietary tiers.
