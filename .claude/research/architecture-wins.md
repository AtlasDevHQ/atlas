...
**Update 2026-05-02 (#1988):** the boot-guard family was extended again, this time to close the medium-severity tail surfaced by the same `/prod-audit` pass. Three more siblings landed in the same shape:

- `RegionGuardLive` (#1988 C7) — SaaS instance booted with `ATLAS_API_REGION` (or `residency.defaultRegion`) pointing at a region that doesn't exist in `config.residency.regions`, or whose entry has a malformed `databaseUrl`. Without it, the misrouting middleware treated every request as misrouted; in strict mode it 421'd legitimate traffic. `RegionMisconfiguredError` carries the claimed region + available keys so the typo class (`eu` vs `eu-west`) is diagnosable from the boot log alone.
- `PluginConfigGuardLive` (#1988 C8) — at boot, validate every stored `workspace_plugins.config` row against its plugin's current `getConfigSchema()` (renamed keys, missing required fields, type drift). The validation function lives in a new `lib/plugins/validation.ts` module so it stays unit-testable independent of the Layer DAG; the Layer is a thin wrapper that lazy-imports the registry + InternalDB. Reuses the existing `ATLAS_STRICT_PLUGIN_SECRETS=true` knob — same flag operators already use for F-42 secret-residue strict mode — rather than introducing a parallel switch. Without strict mode, issues log at `warn`; with it, the boot Layer fails with the per-row issue list attached.
- `MigrationGuardLive` (#1988 C9) — promotes `MigrationLive`'s soft failure to fatal in SaaS. The legacy non-fatal behavior is correct for self-hosted (operators may run stateless instances with no internal DB); in SaaS it silently downgraded `loadSettings()` to env-vars-only via the `42P01 / does not exist` branch, which then bypassed admin overrides every later guard relied on. Lives in `lib/effect/layers.ts` next to `DpaGuardLive` because it directly yields the `Migration` Tag — the tagged error (`MigrationsRequiredError`) stays in `saas-guards.ts` next to its siblings so the family's failure shapes remain co-located for grep / docs.

A coupled change in the same PR addressed B5 — the agent's preflight `loadOrgWhitelist`/`getOrgSemanticIndex`/`buildLearnedPatternsSection` `Effect.catchAll` previously dropped semantic context silently and prepended a vague string to the system prompt. The user got a degraded answer with no UI affordance. `runAgent` now accepts a `contextWarnings: ChatContextWarning[]` out-array that the chat route serializes as SSE `data-context-warning` frames before merging the model stream. The new `ChatContextWarning` type in `@useatlas/types` is shaped as a sibling of `ChatErrorInfo` (the #1980 frame) but discriminated by `severity: "warning"` so a single client parser can route warnings and hard errors without misclassifying a degradation as a failure.

The pattern is now stable enough that adding another sibling guard (or another `data-context-warning` code) is a 30-minute task: define the tagged error in `saas-guards.ts`, add the guard Layer, wire it into `buildAppLayer`, write the SaaS-fails / self-hosted-ok pair, and add a row to the deploy.mdx table. The `EnterpriseGuardLive` env-vs-config split remains the only architectural decision; everything else is mechanical reuse.
