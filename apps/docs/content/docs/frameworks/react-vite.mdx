...
See the [Data Stream Protocol](/frameworks/overview#tool-call-parts-ai-sdk-v6) section in the overview for the full part structure.
...
See [Bring Your Own Frontend](/frameworks/overview) for what the `packages/web/` reference client (monorepo-only — not on npm) adds on top of `@ai-sdk/react` (conversation sidebar, chart detection, managed auth, etc.).
