...
| Managed | `Cookie: atlas.session_token=<token>` (browser) or `Authorization: Bearer <session-token>` |
...
The response streams as SSE events. The `x-conversation-id` response header contains the conversation ID for follow-up messages (only present when `DATABASE_URL` is configured). The `x-run-id` response header carries the identifier of the turn (a single agent run). If durable sessions are enabled and a turn is interrupted (deploy, crash, timeout), the client can re-attach to it via `POST /api/v1/chat/{conversationId}/resume` using the conversation ID and run ID — the agent continues from its last checkpoint rather than restarting, so completed tool calls are not re-executed. A second concurrent resume of the same run is rejected with `409`.
...
| `no_datasource` | 400 / 503 | No analytics datasource URL configured. 400 from chat, `/query`, and `/demo`; 503 from the SQL pipeline (`/execute-sql`, `/metrics`, dashboards) |
| `no_capability` | 400 | Workspace has no datasource, Knowledge Base collections, or Company Brain content |
...
For cross-origin managed auth deployments, see the <AudienceLink selfHosted="/self-hosted/deployment/authentication#cross-origin-deployment-self-hosted">cross-origin section</AudienceLink> in the authentication guide.
