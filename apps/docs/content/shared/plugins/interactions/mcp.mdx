...
<Callout type="info">
**Plugins can contribute MCP tools.** Any plugin can implement `mcpTools()` to register first-class tools that show up in `tools/list` alongside Atlas's own (`executeSQL`, `explore`, the typed semantic tools). See the [Plugin MCP Tools guide](/plugins/sdk/mcp-tools) for the contract, the description rubric (which plugin tools share with native tools — drift fails CI), and a complete example. The reference implementation lives in [`@useatlas/yaml-context`](https://github.com/AtlasDevHQ/atlas/blob/main/plugins/yaml-context/src/index.ts).
</Callout>
