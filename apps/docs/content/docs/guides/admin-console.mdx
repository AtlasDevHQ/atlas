...
### Group by Type / Environment toggle (1.4.4)
...
- **By Environment** — connections cluster under their **connection group** (`prod`, `staging`, `eu-only`, …). This is the new view 1.4.4 introduced and is the right default for multi-environment workspaces — it makes "which replicas does `prod` actually have?" answerable at a glance
...
Drift between the live database schema and the semantic layer is surfaced inline on `/admin/semantic`. As of 1.4.4, the legacy `/admin/schema-diff` page is retired — drift now lives next to the entities it describes, so you no longer have to context-switch between two routes to reconcile schema changes.
...
<Callout type="info">
**If you're looking for `/admin/schema-diff`** — the page was retired in 1.4.4. Drift is now inline on `/admin/semantic` per the walkthrough above. There was no public `/api/v1/admin/schema-diff` REST endpoint; the new inline drift drawer uses `/api/v1/admin/semantic/diff` (documented above).
</Callout>
