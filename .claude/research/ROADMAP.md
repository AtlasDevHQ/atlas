...
- [ ] Phase-1 architecture tails (breaking-change cluster, needs a second types publish cycle): `ProviderConfig` tagged union (#1542), unify `ActionStatus` / `ActionDisplayStatus` (#1591), ApprovalRule discriminated union (#1660), `RegionMigration` discriminated union (#1696)
- [ ] Open-question tail: `z.enum()` vs `z.string()` in admin-schemas — architectural discussion (#1643)
- [ ] Bugs surfaced during the schema-consolidation audit: `custom_domains` Drizzle schema drift — missing DNS TXT verification columns (#1707). Follow-up tests: e2e admin-cache browser coverage (#1708)
