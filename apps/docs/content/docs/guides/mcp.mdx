...
> Hosted mode against `app.useatlas.dev` is partially live: mint a workspace token via `/api/v1/admin/mcp-tokens` and point any MCP client at the regional URL. See the [hosted endpoint reference](/guides/mcp-hosted) for the URL pattern, bearer model, and operational notes. The Settings → MCP Tokens UI and the `bunx @useatlas/mcp init --hosted` flag are in flight.
