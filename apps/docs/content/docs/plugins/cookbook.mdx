...
```typescript
import { definePlugin } from "@useatlas/plugin-sdk";
import type { AtlasPluginContext, PluginLogger } from "@useatlas/plugin-sdk";
  ...
  hooks: {
    beforeQuery: [{
      handler: (ctx) => {
        ...
        // For production use, consider a proper SQL AST rewriter.
...
### Data Masking in Results

Redact sensitive columns in query results before they reach the user:
...
export default definePlugin({
  ...
  hooks: {
    afterQuery: [{
      handler: (ctx) => {
        const columnsToMask = ctx.result.columns.filter((c) =>
          MASKED_COLUMNS.has(c.toLowerCase()),
        );
        if (columnsToMask.length === 0) return;
...
### Request Authentication via `onRequest`

Validate custom headers on incoming HTTP requests:
  ...
  hooks: {
    beforeQuery: [{
      handler: async (ctx) => {
        ...
        // Check cache (simple hash — use a proper hash in production)
