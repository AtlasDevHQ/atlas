...
<Callout type="info">
Requires managed auth mode (the internal `user`/`session` tables). User-management operations (list, ban/unban, delete, revoke sessions) are platform-admin-gated server-side. Not available in simple-key, BYOT, or no-auth modes.
</Callout>
