...
The semantic layer is a set of YAML files that describe your database schema, business terminology, and key metrics. The agent reads these files before writing SQL -- they are the primary mechanism for grounding the agent in your semantic layer. For definitions of every term used on this page, see [Semantic Layer Concepts](/getting-started/concepts).
...
See the [Schema Evolution guide](/guides/schema-evolution) for a complete workflow on detecting drift, updating YAMLs, and preserving manual enrichments. See [CLI Reference](/reference/cli) for all options.
