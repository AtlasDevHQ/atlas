...
- This checkout lives in WSL. When Codex runs from Windows, prefer WSL-native commands with `wsl -d Ubuntu --cd /home/msywu/oss/atlas/3 bash -lc '...'` instead of running Git/Bun against the `\\wsl.localhost\...` UNC path.
...
- Example: `wsl -d Ubuntu --cd /home/msywu/oss/atlas/3 bash -lc 'bun install'`.
...
- [ ] **Encrypted at rest** — Bearer secrets in the internal DB go through `encryptSecret` (AES-256-GCM, `enc:v<N>:iv:authTag:ciphertext`) before write. Two helper pairs exist: (1) `db/internal.ts` is reserved for the two legacy non-`workspace_plugins` columns (`workspace_model_config.api_key_encrypted`, `sso_providers.config.clientSecret`) — no new call sites; (2) `db/secret-encryption.ts` is the default for everything else (every F-41 integration credential column — bot tokens, API keys, email/sandbox JSON blobs). Datasource credentials live inside `workspace_plugins.config` JSONB and go through selective-field encryption (`encryptSecretFields` / `decryptSecretFields`) keyed on the catalog row's `config_schema` `secret: true` flag — same pattern as every plugin config secret (post-#2744 / [ADR-0007](docs/adr/0007-unified-install-pipeline.md) cutover dropped `connections.url`). Reads call `decryptSecret(<col>_encrypted)` directly — no plaintext fallback survives post-#1832. Adding a new integration credential table is a one-line addition to `INTEGRATION_TABLES` in `packages/api/src/lib/db/integration-tables.ts` (consumed by both rotation + the F-42 residue audit) plus an `_encrypted` column in the migration. SaaS regions can opt into `ATLAS_STRICT_PLUGIN_SECRETS=true` to reject plugin admin writes whose schema is corrupt or carries secret/passthrough drift. Key derivation: `ATLAS_ENCRYPTION_KEYS` (versioned, F-47) → `ATLAS_ENCRYPTION_KEY` (legacy single-key, treated as v1) → `BETTER_AUTH_SECRET` (deprecated under SaaS — startup warns)
...
### Merge discipline
- [ ] **Branch protection is on for `main`** — Required status checks: `ci` (umbrella over lint/type/build/tests incl. the `api-tests` shards + `ee-stub-build`), `Deploy Validation` (umbrella over Scaffold + Standalone + Config), `Image Scan`, `Analyze (javascript-typescript)`, `fork-pr-gate`. `strict: true` (branch must be up to date). Force-push and deletion blocked. `enforce_admins: false` so the override below works in genuine emergencies. The umbrella exists because GitHub emits the un-substituted matrix template name (`Scaffold (${{ matrix.platform }})`) when an entire matrix job is skipped, breaking required-context name matching — `deploy-validation-required` (`if: always()`) collapses it. See `docs/development/branch-protection.md`
...
- [ ] **Required reviews are intentionally off** — Solo developer + parallel-Codex workflow. Don't enable `required_pull_request_reviews` without rethinking the model
