...
description: Drop the legacy `connectionId` field — the multi-environment slice (#2336) replaces it with `connectionGroupId` on every wire shape it touched.
---
...
The multi-environment semantic layer release (#2336) made every piece of
content (semantic entities, PII rules, dashboards, scheduled tasks,
approvals) scope to a **connection group**
