...
- **Live vs Restart** — Each setting shows whether changes take effect immediately (**Live**) or require a server restart (**Requires restart**). Query Limits and Rate Limiting are live; Security, Agent, and provider settings require a restart
