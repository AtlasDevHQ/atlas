...
Both properties are consumed during SQL validation: `parserDialect` sets the AST parser mode used in layer 2, and `forbiddenPatterns` are checked as additional regex guards in layer 1. See [SQL Validation Pipeline](/docs/security/sql-validation) for the full layer breakdown.
