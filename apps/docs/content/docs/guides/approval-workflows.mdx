...
Atlas supports approval workflows as an enterprise feature. Admins define rules that intercept sensitive queries before execution, requiring sign-off from designated approvers. This adds a governance layer for compliance-sensitive environments.
...
- Verify enterprise is enabled (`ATLAS_ENTERPRISE_ENABLED=true`)
