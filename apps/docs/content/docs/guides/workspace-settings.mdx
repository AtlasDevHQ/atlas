...
<Callout type="info" title="Platform settings">
  Global settings that affect all workspaces (security, LLM provider, deploy mode, branding) are managed separately on the [Platform Settings](/guides/admin-console#platform-settings) page at `/platform/settings`. Only platform admins can access platform settings.
</Callout>
...
| **Default Answer Style** | Analyst (surface default) | The workspace "house voice" — Analyst, Plain English, or Executive. Applies to web conversations without a per-conversation pick and to SDK/MCP/API calls that send no style. A per-conversation pick from the chat header always wins; Slack chooses its own voice per turn. Reset to fall back to the built-in default |
