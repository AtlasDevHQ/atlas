...
```bash
curl -X POST "https://discord.com/api/v10/applications/$DISCORD_CLIENT_ID/commands" \
...
The `adapters:` config field was removed in [#2650](https://github.com/AtlasDevHQ/atlas/pull/2650). Adapter activation is driven by `catalog` rows + per-Platform env vars; `chatPlugin()` itself takes no credentials. Discord's static-bot install handler landed in [#2749](https://github.com/AtlasDevHQ/atlas/pull/2749) — the catalog row carries `install_model: "static-bot"`, and bot credentials are read from `DISCORD_BOT_TOKEN`, `DISCORD_CLIENT_ID`, and `DISCORD_PUBLIC_KEY`. See the [chat plugin docs](/plugins/interactions/chat#configuration) for the canonical shape.
</Callout>
...
| `DISCORD_CLIENT_ID` | Discord application ID — labeled "Application ID" on the Developer Portal's General Information page (identical to the OAuth2 client ID). Read by the adapter as `applicationId` |
