...
The **Admin → Learned Patterns** cockpit surfaces each pattern's evidence so its payoff is observable rather than asserted: its status (and whether a human or the machine approved it), its confidence, how many times it's been seen, its average latency, and its **injection count over the trailing 30 days** — how often the pattern actually made it into the agent's prompt, the most direct "is this pattern earning its keep now" measure. Sort or filter by any of these to find, for example, an approved pattern with zero recent injections — a candidate to refine its description or retire.
