...
| Variable | Default | Description |
|----------|---------|-------------|
| `ATLAS_DATASOURCE_URL` | — | Analytics datasource connection string (PostgreSQL or MySQL). Falls back to `DATABASE_URL_UNPOOLED` / `DATABASE_URL` when `ATLAS_DEMO_DATA=true`. Optional when datasources are defined in `atlas.config.ts` |
...
`ATLAS_RLS_CLAIMS` is a JSON string. Used with simple-key auth to provide static claims for row-level security filtering:
...
| `ATLAS_ACTION_APPROVAL` | `manual` | Default approval mode for actions: `auto`, `manual`, `admin-only` |
