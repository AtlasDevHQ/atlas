...
All thirteen are hidden from the generic settings page on Atlas Cloud, so a workspace admin cannot read or write any of them. Four are **workspace-scoped** — meaning each can hold a different value per workspace, set by a platform admin: `ATLAS_BRAIN_AUDIENCE_SYNC_ENABLED`, `ATLAS_BRAIN_COVERAGE_SNAPSHOT_ENABLED` and the two alias auto-approval keys. The other nine are one value for the whole deployment.
