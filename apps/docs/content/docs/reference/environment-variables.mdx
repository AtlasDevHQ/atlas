...
| `TEAMS_APP_PASSWORD` | — | Platform-shared Teams app client secret used by the platform OAuth flow. In BYOT mode the app password is stored per-tenant in the internal database instead, and this env var is unused. Source: `packages/api/src/lib/teams/store.ts` |

```bash
# Teams integration via Azure AD admin consent
TEAMS_APP_ID=your-teams-app-id
# Platform OAuth flow only — leave unset for BYOT
TEAMS_APP_PASSWORD=your-teams-app-secret
```

> In BYOT mode the Teams app password (client secret) is **not** read from `TEAMS_APP_PASSWORD`. Per-org credentials are stored encrypted in the internal database and configured via the admin integrations UI — see [Teams integration](/guides/integrations#microsoft-teams).
...
WhatsApp (Meta Business) static-bot install (`/api/v1/integrations/whatsapp/install`). Both variables gate registration — an enabled `whatsapp` catalog row with either unset returns `501` and skips the adapter. Source: `packages/api/src/lib/integrations/install/register.ts`.
