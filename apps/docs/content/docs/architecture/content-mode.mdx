...
> Design doc for the `ContentModeRegistry` library (#1515) that drives the developer/published mode system. Covers the static-tuple architecture, the type derivation, and the end-to-end flow for adding a new mode-participating table.

## Problem

The original mode system worked but was shallow. Four content tables participated — `connections`, `prompt_collections`, `query_suggestions`, and `semantic_entities` — and each surface (read filter, draft counts, publish path) had its own hand-written policy:
