...
- **`internal/backfill-chat-installations.ts` is gone** — the bandage isn't needed when there's only one store to fill. Future ops walk-throughs ("how do I bring an install back online?") collapse from "decrypt + re-encrypt + insert into chat_cache" to "reinstall via OAuth, the callback writes straight to chat_cache."
