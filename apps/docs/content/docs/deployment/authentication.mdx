...
Set `ATLAS_AUTH_MODE` explicitly to override auto-detection: `none`, `api-key`, `managed`, or `byot`.
...
- Subsequent signups get the `analyst` role
...
4. Sessions last 7 days. The session expiry is extended when a request arrives at least 24 hours after the last session update
...
> **Cookie cache:** Better Auth is configured with a 5-minute cookie cache (`cookieCache.maxAge = 300s`). This means role changes or session revocations may take up to 5 minutes to take effect. The cache improves performance by avoiding a database lookup on every request, but be aware of this delay when revoking access or changing user roles.
