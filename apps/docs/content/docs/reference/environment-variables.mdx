...
| `ATLAS_MEMORY_MAX_SLOTS` | `64` | **Durable working-memory** slot cap (#3757). Maximum number of named memory slots a single session may hold. A write that would add a **new** slot past this cap is rejected (surfaced to the caller), never truncated; overwriting an existing slot never counts against it. Workspace-scoped, hot-reloadable |
| `ATLAS_MEMORY_MAX_VALUE_BYTES` | `16384` (16 KiB) | **Durable working-memory** per-value size cap (#3757). Maximum serialized size (JSON/UTF-8 bytes) of a single memory slot value; a larger write is rejected before persistence, never truncated. Keeps memory a small fact store, not a bulk data sink. Workspace-scoped, hot-reloadable |
