...
1. The agent reads entity YAMLs via the `explore` tool to understand the data model
...
Atlas runs background health checks with a 3-strike failure threshold over a 5-minute window. A temporarily failing datasource is marked `"degraded"` first, then `"unhealthy"` after sustained failures. The agent's system prompt reflects this status so it can warn users.
