...
- **Action** — what was done (e.g. `update_setting`, `create_connection`, `change_role`)
- **Target** — what was affected (e.g. the setting key, connection name, or user email)
- **Details** — additional context (old/new values where applicable)

**Features:**
- **Filtering** — filter by action type, actor, or date range
