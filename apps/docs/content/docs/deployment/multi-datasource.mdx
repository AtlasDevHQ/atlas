...
### Cross-environment scope routing (1.4.5+)

When a workspace has multiple **connection groups** (environments — e.g. `us-int`, `eu`, `us-prod` all running the same schema; see [Dashboards → Environment](/guides/dashboards#environment-144)), the agent can fan a single question out across more than one member of the active group. Each chat conversation has a **routing mode** the user controls from the picker in the chat header:
