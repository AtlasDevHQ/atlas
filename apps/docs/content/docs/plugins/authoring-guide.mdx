...
The invocation is **best-effort**: a thrown error is logged by the host (with the plugin id and `workspaceId`) and the uninstall proceeds — never rely on `onUninstall` for load-bearing cleanup. Resolution: the host invokes the hook on the per-workspace instance built by its lazy loader (marketplace/SaaS installs), and on any globally-registered plugin whose `id` equals the uninstalled catalog entry's slug, catalog id, or `<slug>-<type>` (the bundled-plugin naming convention, e.g. `jira-action` for catalog slug `jira`). See `plugins/jira` for a working reference implementation that revokes Jira dynamic webhook subscriptions.
...
Called when a workspace uninstalls the plugin — before the install row and credential stores are removed, so the plugin can still authenticate against the external platform. Use it to revoke webhook subscriptions, OAuth grants, or other per-workspace external state. Best-effort: a thrown error is logged and the uninstall proceeds. See the [Uninstall Contract](#implementing-onuninstall) for details and a full example.
