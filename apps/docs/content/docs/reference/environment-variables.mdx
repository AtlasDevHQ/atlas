...
| `NEXT_PUBLIC_ATLAS_API_BASE` | `https://api.useatlas.dev` | **Build-time** var for `apps/www`: the API origin the landing-page talk-to-sales form POSTs `/api/v1/contact` to (`talk-to-sales-form.tsx`). `www` builds with NIXPACKS, so the Railway service var is inlined at build. On `www-staging` set it to `https://api.staging.useatlas.dev` — an unset value sends staging's submissions to the **prod** API |
