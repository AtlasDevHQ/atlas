...
**Timeout resolution order:** per-action `timeout` > `defaults.timeout` > `ATLAS_ACTION_TIMEOUT` env var. When no timeout is configured, actions run without a time limit.
