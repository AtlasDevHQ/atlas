...
| `SLACK_ENCRYPTION_KEY` | — | At-rest encryption key for per-Workspace Slack OAuth credentials in `chat_cache`. Required on SaaS — `ChatAdapterEnvGuardLive` asserts on it at boot. Missing the key silently drops the adapter (no Slack events land). Source: `packages/api/src/lib/effect/saas-env.ts` |
...
| `ATLAS_TEAM_PG_URL` | — | Tenant-DB connection string consumed by `bun run` operator backfill scripts (e.g. `slack_installations_to_chat_cache.ts`). Not read by the API process — only by one-shot scripts under `packages/api/src/lib/db/migrations/scripts/` |
