...
4. The first one that succeeds becomes the active backend (cached for the process lifetime)
5. If all plugins fail, Atlas falls through to the built-in detection chain (Vercel > nsjail > sidecar > just-bash)
...
- **Type mismatch** in the mutation is caught and logged -- the mutation is silently ignored
