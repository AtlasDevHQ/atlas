...
Each organization can have multiple SSO providers, each mapped to a different email domain. Domain uniqueness is enforced per organization — no two providers in the same org can claim the same domain.
...
| `idpSsoUrl` | The IdP's single sign-on URL (HTTP-POST binding) |
...
|--------|------|-------------|
| `GET` | `/providers` | List all SSO providers for the org |
...
| 409 | `conflict` | Domain already claimed by another provider in the org |
