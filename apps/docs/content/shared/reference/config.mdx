...
By default, a deployment with no process-level analytics datasource degrades `/health` — a box that simply forgot `ATLAS_DATASOURCE_URL` should look unhealthy. When a deployment *intentionally* has none (a multi-tenant SaaS region where every connection is per-workspace, or a knowledge-only self-host), declare that so `/health` reports the `datasource` component as `disabled` without degrading the rollup:

```typescript
// atlas.config.ts — this deployment deliberately has no process-level datasource
export default defineConfig({
  datasourceExpected: false,
});
```

Resolution precedence (#4854, `packages/api/src/lib/db/datasource-expectation.ts`): the `ATLAS_DATASOURCE_EXPECTED` env var overrides per-service; then the config-file `datasourceExpected` field; then the default of "expected". An unrecognized env value warns and resolves to "expected" *without* consulting the config file. `ATLAS_DEMO_DATA=true` outranks all of these — demo mode always expects its datasource.
...
The default tools are `explore` (read semantic layer files) and `executeSQL` (run validated SQL queries). Other registrable names include `searchAtlas` (Knowledge Base + [Company Atlas](/guides/atlas-temporal-model) search); the legacy names `searchBrain` and `searchKnowledge` are still accepted and normalized to `searchAtlas` with a startup warning. When the action framework is enabled, action tools are also registered. See [Actions](#actions) below.
...
The cache knobs are **runtime settings**, not `atlas.config.ts` fields — tune them from **Admin → the Cache page** (or the `ATLAS_CACHE_*` env vars) with no redeploy:

| Setting | Env var | Scope | Default | Description |
|---------|---------|-------|---------|-------------|
| Query Cache | `ATLAS_CACHE_ENABLED` | Workspace | `true` | Enable/disable query result caching |
| Cache TTL | `ATLAS_CACHE_TTL` | Workspace | `300000` | Time-to-live in milliseconds |
| Cache Max Entries | `ATLAS_CACHE_MAX_SIZE` | Platform | `1000` | Maximum cached entries (LRU eviction), process-wide |

A TTL or max-size change takes effect on the running backend immediately (hit/miss counters survive). Manual flush is available via `POST /api/v1/admin/cache/flush`. Plugins can provide an external cache backend (e.g. Redis) via the `cacheBackend` field on the plugin object.

<Callout type="warn" title="Removed: the `cache:` config block">
The `cache:` block has been **removed** from the config schema — a `cache:` block in `atlas.config.ts` now fails validation at boot. Delete the block and use the settings above (Admin → the Cache page, or the `ATLAS_CACHE_*` env vars). It was previously deprecated (ignored with a boot warning) for one release before removal.
</Callout>
...
Configure the adaptive starter-prompt resolver that powers the empty-chat surface (`GET /api/v1/starter-prompts`). Composes per-user favorites, admin-approved popular queries, and a demo-industry library into a single ordered list.

```typescript
export default defineConfig({
  starterPrompts: {
    coldWindowDays: 90,     // default: 90
    autoPromoteClicks: 3,   // default: 3
    maxFavorites: 10,       // default: 10
  },
});
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `coldWindowDays` | `number` | `90` | Window in days applied to `prompt_collections.created_at` when the library tier is populated. Also bounds the auto-promote eligibility window — suggestions whose most recent activity is older than this window do not appear in the moderation queue |
| `autoPromoteClicks` | `number` | `3` | Distinct-user click threshold that auto-promotes a learned suggestion into the admin approval queue. Clicks are counted once per user |
| `maxFavorites` | `number` | `10` | Hard cap on per-user pinned starter prompts (`POST /api/v1/starter-prompts/favorites`). Applies per user per workspace |

**Env var equivalents:**
```bash
ATLAS_STARTER_PROMPT_COLD_WINDOW_DAYS=90
ATLAS_STARTER_PROMPT_AUTO_PROMOTE_CLICKS=3
ATLAS_STARTER_PROMPT_MAX_FAVORITES=10
```
...
Declare the **Plugin Catalog** — the flat list of installable chat platforms and integration plugins your deployment offers. Implements [ADR-0002](https://github.com/AtlasDevHQ/atlas/blob/main/docs/adr/0002-catalog-seeded-from-config-at-boot.md): declarative at deploy time, idempotently seeded into the `plugin_catalog` table at boot, then DB-canonical for runtime reads.

The operator declares the catalog; customer admins install entries via `/admin/integrations`. `install_model` dispatches the install-handler family (`oauth`, `form`, `static-bot`, or `oauth-datasource`) — see [`packages/api/src/lib/integrations/install/register.ts`](https://github.com/AtlasDevHQ/atlas/blob/main/packages/api/src/lib/integrations/install/register.ts) for the registered handler set.

```typescript
// atlas.config.ts — declare a Slack OAuth chat platform + an email form integration
export default defineConfig({
  catalog: [
    {
      slug: "slack",
      ...
      install_model: "oauth",
      name: "Slack",
      description: "Connect Atlas to your Slack workspace.",
      iconUrl: "https://cdn.useatlas.dev/icons/slack.svg",
      min_plan: "starter",
    },
    {
      slug: "email",
      ...
      install_model: "form",
      name: "Email",
      description: "Send approval-gated emails via SMTP.",
      min_plan: "starter",
      // Required when install_model === "form" — non-empty.
      configSchema: [
        { key: "smtp_host", type: "string", required: true, label: "SMTP host" },
        { key: "smtp_port", type: "number", required: true, label: "Port", default: 587 },
        { key: "smtp_username", type: "string", required: true, label: "Username" },
        { key: "smtp_password", type: "string", required: true, secret: true, label: "Password" },
        { key: "from_address", type: "string", required: true, label: "From address" },
      ],
    },
  ],
});
...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `slug` | `string` | Yes | — | Stable identifier — lowercase alphanumeric with dashes (regex `^[a-z][a-z0-9-]*$`). Used as the primary lookup key in `plugin_catalog.slug` and as the dispatch key in `AdapterRegistry`. Examples: `slack`, `salesforce`, `linear-apikey` |
| `type` | `"chat" \| "integration"` | Yes | — | Admin-UI grouping. `chat` rows live on the Chat tab of `/admin/integrations`; `integration` rows live on the Actions/Integrations tab. Backend dispatch is by `install_model`, not `type` |
| `install_model` | `"oauth" \| "form" \| "static-bot" \| "oauth-datasource"` | Yes | — | Install-handler dispatch key. `oauth` runs a per-Workspace OAuth dance (Slack, Jira, Salesforce, GitHub App, Linear OAuth). `form` validates customer-supplied fields against `configSchema` (Email, Webhook, Obsidian, GitHub PAT, Linear API key). `static-bot` uses an operator-shared bot and a per-Workspace routing identifier (Teams, Discord, Google Chat, Telegram, WhatsApp). `oauth-datasource` runs a per-Workspace OAuth dance that installs a REST/OpenAPI datasource |
| `name` | `string` | No | (derived from slug) | Human-readable name shown in admin-UI cards |
| `description` | `string` | No | — | Admin-UI description copy |
| `iconUrl` | `string` (URL) | No | — | Icon URL for admin-UI cards |
| `min_plan` | `"free" \| "trial" \| "starter" \| "pro" \| "business"` | No | `"starter"` | Plan-tier gate evaluated at customer-install time via the unified `PLAN_RANK` comparator. See [Entitlements](/architecture/entitlements) for the gating model |
| `enabled` | `boolean` | No | `true` | Whether customers can install this entry today. Operators flip to `false` (in DB) for emergency-disable without a deploy; the seed preserves a DB-side `false` even if the config declares `true` |
| `saas_eligible` | `boolean` | No | `true` | Whether the entry is offered in SaaS deployments. `false` hides the row from SaaS admin-UI listings while keeping it available on self-host. Canonical case: GitHub PAT mode (per-user token tied to one employee — unsafe in B2B SaaS) |
| `configSchema` | `ConfigSchemaField[]` | Required when `install_model === "form"` (and must be non-empty); optional otherwise | — | Form-field declaration for `install_model: "form"` entries. Each field is rendered by the install modal and validated server-side. Fields flagged `secret: true` are encrypted at rest before persistence |
| `implementation_status` | `"available" \| "coming_soon"` | No | `"available"` | Whether Atlas has shipped a working install path. `coming_soon` renders the row as an inert grey card. Override per deploy via `overrideImplementationStatus`. See [ADR-0007](https://github.com/AtlasDevHQ/atlas/blob/main/docs/adr/0007-catalog-row-implementation-status.md) |

### configSchema field shape

Each entry in `configSchema` is a strict object with the following fields:

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `key` | `string` | Yes | Field name persisted in `workspace_plugins.config` |
| `type` | `"string" \| "number" \| "boolean" \| "select"` | Yes | Input type. `select` requires `options` |
| `label` | `string` | No | Display label in the install modal |
| `description` | `string` | No | Helper text below the input |
| `required` | `boolean` | No | Reject the install when missing |
| `secret` | `boolean` | No | When `true`, the value is encrypted at rest before persistence into `workspace_plugins.config` |
| `options` | `string[]` | No | Choices for `type: "select"` |
| `default` | `unknown` | No | Default value pre-filled in the install modal |

### Validation rules

- **Slug uniqueness.** Every entry must have a unique `slug` within the `catalog` array — duplicates fail Zod parsing with `catalog entries must have unique slugs — duplicate found`
...
| `datasourceExpected` | `boolean` | — (treated as expected) | Whether this deployment expects a process-level analytics datasource (#4854). Set `false` on a deployment that intentionally has none (per-workspace connections, knowledge-only) so `/health` reports the datasource component as `disabled` instead of degrading. Omitted means expected. Overridden per-service by `ATLAS_DATASOURCE_EXPECTED` ([details](#datasource-expectation)) |
...
| `starterPrompts` | `object` | — | Adaptive starter prompt configuration ([details](#starter-prompts)) |
...
| `catalog` | `CatalogEntry[]` | — | Plugin Catalog declaration — installable chat Platforms and integration plugins. Idempotently seeded into `plugin_catalog` at boot. See [ADR-0002](https://github.com/AtlasDevHQ/atlas/blob/main/docs/adr/0002-catalog-seeded-from-config-at-boot.md) |
| `overrideImplementationStatus` | `Record<string, "available" \| "coming_soon">` | — | Per-deploy override for a catalog row's `implementation_status`, keyed by catalog slug. Self-host operators use it to promote a `coming_soon` entry to `available` once they've shipped their own handler, without forking the catalog definition |
