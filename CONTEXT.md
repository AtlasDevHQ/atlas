...
- **Scope tags / scoped invalidation** — every `set()` carries a `CacheScope` (`{ orgId?, connectionId }`); the LRU keeps an `orgId → keys` side index (consistent across set / delete / capacity-eviction / TTL-expiry / flush) so `flushByOrg(orgId)` purges exactly one Workspace's entries. Org deletion and residency migration purge **per-org**, never fleet-wide — a co-tenant's warm entries survive one Workspace's teardown. Per-connection invalidation falls out by filtering an org's entries. Fleet-wide `flush()` stays for config reload + the admin flush button.
