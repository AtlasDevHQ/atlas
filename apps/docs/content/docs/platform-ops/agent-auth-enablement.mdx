...
- **Platform tier (operator master switch):** Platform console →
  **Settings** (`/platform/settings`), under **Workspace setting
  ...
  `platform_admin` role required) — no redeploy, no no-org session, no
  direct DB write. Because the key is hot-reloadable, the HTTP surface
  follows within the settings refresh TTL (~30 seconds):
  `/.well-known/agent-configuration` flips between `404` (off) and
  `200` (on). Workspace overrides are untouched.
