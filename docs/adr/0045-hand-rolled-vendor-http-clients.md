...
**Five migration sites, and they do not each take all four concerns** — `jira.ts` (deadline, narrowing, host pinning), `github.ts` (deadline, narrowing), `linear.ts` (deadline, narrowing), `salesforce.ts` (host pinning only at the time — it drove `jsforce`, not `fetch`, so it had no deadline, before or after; [#5572](https://github.com/AtlasDevHQ/atlas/issues/5572) then hand-rolled its two calls and it took the deadline and the narrowing too), and the four provider sites in `lib/email/delivery.ts` (the truncation only). `lib/tools/actions/email.ts` is untouched: it delegates to the delivery chain and re-rolls nothing.
