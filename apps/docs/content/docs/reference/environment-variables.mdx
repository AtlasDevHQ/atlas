...
| `ATLAS_HEALTH_PLUGIN_CACHE_TTL_MS` | `15000` (15s) | TTL for the plugin-liveness health cache. `0` disables caching (every call re-probes upstream). Clamped to a max of `300000` (5m); out-of-range / unparseable falls back to the default with a warn. Source: `packages/api/src/lib/plugins/registry.ts` |
