...
| **Deployment** | Docker, Railway, Vercel, Atlas Cloud (US; EU and APAC on request) | SaaS or on-premises appliance |
| **Databases** | Postgres, MySQL + plugins for BigQuery, ClickHouse, DuckDB, Snowflake, Salesforce, Elasticsearch/OpenSearch | Cloud data warehouses (Snowflake, Databricks, BigQuery, Redshift, etc.) |
...
| **Plugin system** | Plugin SDK + 24 plugins + marketplace | SpotApps marketplace + Spotter Connectors (Zendesk, Google Workspace, Slack) |
| **Chat integrations** | 8 integrations (Slack, Teams, Discord, Telegram, Google Chat, GitHub, Linear, WhatsApp) | Slack connector |
...
| **Data residency** | Hosted in the US; EU and APAC regions are built and brought online on request | Region selection in SaaS |
...
**Atlas** is built for developers and small-to-mid teams who want to add natural language data querying to their own applications. It's open-source (AGPL-3.0 core, MIT client libs), deploys in minutes, and is designed to be embedded — not to be a standalone analytics platform. [Atlas Cloud](https://app.useatlas.dev) provides a hosted SaaS option — US-hosted, with EU and APAC regions on request — with SSO/SCIM, data residency, and SLA commitments — at a fraction of enterprise platform pricing.
