...
//
// Asserted once, at its definition site, in `session-store.test.ts`: the
// override is module-scoped in `session-store.ts` and both transports only
// re-export the same function, so the guard had two identical copies here.
