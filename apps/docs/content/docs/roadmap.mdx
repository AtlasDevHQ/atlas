...
The current milestone. Self-service management and observability.

- **Admin console** — Connection CRUD with encrypted URLs at rest, action approval with bulk operations, user invite flow, plugin and settings configuration
- **Observability** — Query analytics dashboard, health dashboard with per-component checks, token usage tracking per-user and per-conversation, OpenTelemetry traces
- **Scheduled tasks v2** — Task create/edit forms, cross-task run history with filters, delivery channel configuration with status tracking and preview
