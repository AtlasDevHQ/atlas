...
For the TanStack Query approach, the sync endpoint returns data in the `data` array directly -- no tool part parsing needed. See the [Data Stream Protocol](/docs/frameworks/overview#data-stream-protocol) section for details on both response formats.
