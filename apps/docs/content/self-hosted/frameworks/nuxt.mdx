...
<Callout type="info" title="AI SDK v6 — Vue adapter">
  As of AI SDK v6, `@ai-sdk/vue` (v3+) supports the transport-based API (`DefaultChatTransport`, `sendMessage`) shown below. Make sure you install `@ai-sdk/vue@^3.0.0` and `ai@^6.0.0`. If you prefer not to use streaming, the [sync query endpoint](/self-hosted/frameworks/overview#streaming-vs-synchronous) (`POST /api/v1/query`) works with any HTTP client. The [`@useatlas/sdk`](https://www.npmjs.com/package/@useatlas/sdk) package provides a typed client for that endpoint.
</Callout>
