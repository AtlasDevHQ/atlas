...
| `atlas.agent` | `atlas.provider`, `atlas.model`, `atlas.message_count`, `finishReason`, `totalSteps`, `totalInputTokens`, `totalOutputTokens` | Full agent loop (one per `streamText` call) |
