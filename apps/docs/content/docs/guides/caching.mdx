...
3. On a **cache hit**, the stored result is returned instantly. The tool response is marked `cached: true`, and the audit log records the query's *original* execution duration (not 0ms), so slow-query analytics stay honest across hits
