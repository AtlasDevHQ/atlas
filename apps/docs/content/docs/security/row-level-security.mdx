...
RLS depends on user claims being available at query time. Choose your auth mode carefully:

- **`byot` (Bring Your Own Token)** is the primary use case — JWT claims from your identity provider drive RLS policies
- **`managed` (Better Auth)** works well — session data includes user claims
- **`api-key`** requires static claims via `ATLAS_RLS_CLAIMS` (a JSON string parsed at startup). Suitable for service-to-service scenarios with a fixed tenant context
- **`none`** is incompatible — all queries will be blocked because there is no user context to resolve claims from

### Claim resolution

Claims are resolved from the authenticated user's context using dot-delimited paths (e.g. `app_metadata.tenant`). The resolution order is:

1. Extract the full claims object from the auth context
2. Walk the dot-delimited path to find the value
3. If the value is an array, generate `IN (...)` instead of `=`
4. If the value is `null`, `undefined`, or an empty array, **block the query**

Non-string values are coerced to strings before injection. Claim values are SQL-escaped (single quotes doubled).
