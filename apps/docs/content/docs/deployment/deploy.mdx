...
- The catch-all route sets `maxDuration = 60` — increase based on your [Vercel plan](https://vercel.com/docs/functions/configuring-functions/duration)
- `@vercel/sandbox` is auto-detected via the `VERCEL` env var

### Docker

**What you get:** A Docker Compose stack with the Hono API + Postgres. Optional nsjail isolation for the explore tool.

- Images are based on `oven/bun:1.3.10`
- The semantic layer (`semantic/`) is baked into the image at build time — rebuild if you update YAMLs
- Build without nsjail for a smaller image: `docker build --build-arg INSTALL_NSJAIL=false -f Dockerfile -t atlas .`
