---
description: "Sweep npm dependency updates (Renovate is Docker-only here, so npm deps drift). Group A = all minors/patches in one PR; Group B = one PR per major."
---
...
EXCLUDE='^(@playwright/test|@axe-core/playwright|fumadocs-.*|@shikijs/.*|@useatlas/types|@typescript/native-preview|chat|@chat-adapter/.*)$'
...
- **What's in EXCLUDE and why (each is within-major-*unsafe*, not just "has a major"):** `@playwright/test` **and `@axe-core/playwright`** — the `playwright-core` split is **two-sided**, so holding one side does not prevent it (coupled-constraints table). `fumadocs-*` — the whole family, for the 16.10 lockstep (a *minor*, 16.9→16.10, breaks the docs build unless `fumadocs-openapi` goes 10→11; also capped by the `~16.9.3` tilde pin). `@shikijs/*` — pinned by whatever `shiki` core `fumadocs-core` pulls, so it is a *consequence* of the fumadocs hold (constraints table). `@useatlas/types` — workspace-published, must never have its ref bumped by a sweep (`check-published-symbols` / version-bump-ordering). `@typescript/native-preview` — pinned **exactly** by #4882 so the `type` gate stops floating; a sweep must never re-float it (#4888 taught Dependabot the same). `chat`/`@chat-adapter/*` — exact-pinned for chat-plugin×Atlas contract stability; bumping needs the contract audit (deliberate, not a sweep).
- **Packages with a pending major but a *safe* minor are NOT excluded** — `@vercel/sandbox`, `react-day-picker`, `snowflake-sdk`, `shadcn`, `esbuild`, `syncpack`, `@duckdb/node-api`, `diff`. Their minors flow into A; `bun update` holds them below the major via `^`. If a package currently has *only* a major available (e.g. `snowflake-sdk` 2→3), it simply won't appear in the sweep (no within-major update) — no exclude needed.
  - **`stripe` and `just-bash` used to be listed here and should not be** (corrected 2026-07-29, #4893). Both had a *minor* break something: `stripe` 22.2.3→22.3.2 moved the pinned `apiVersion` type, and `just-bash` 3.0.2→3.2.0 broke explore execution outright. Neither is permanently excluded — check them per sweep and expect to defer them, don't assume the minor is free.
    - **`just-bash` is now pinned EXACTLY at `3.1.0`** (#4895, 2026-07-30) — not a caret, so a fresh resolve cannot float to 3.2.0. **`3.2.0` does not change backend selection**, which is what #4895 originally suspected; selection and reporting are byte-identical. It adds a `DefenseInDepthBox` hardening layer that monkey-patches Node intrinsics and **hard-fails on bun** with `DefenseInDepthBox: critical patches failed: Module._resolveFilename`. Measured: 3.2.0 executes fine under **node**, and throws under **bun** in *both* the default and the documented capability-detecting `{ enabled: "auto" }` mode — `"auto"` does **not** detect its way around bun. The only bun-viable setting is `defenseInDepth: { enabled: false }`, i.e. switching the new defense layer off to take the bump, which buys nothing over 3.1.0. `3.0.3` and `3.1.0` are both clean (8/8 + isolation verified under bun), so the pin takes those patches and stops below the break.
    - ⚠️ **The 3.2.0 failure mode is an #4828-class reporting lie, and that is the reason to care.** `getExploreBackendType()` still returns `"just-bash"` while explore refuses every request — so `/api/health` would claim `backend: just-bash`, `isolated: false`, `status: degraded` (a working-but-unsandboxed deploy) for a deploy that runs nothing. It is **fail-CLOSED, not fail-open** — nothing executes, so it is not a sandbox-escape class — but it re-creates through a dependency exactly the boot-vs-health dishonesty #4828 fixed in the selection code. The guard that caught it is the positive control `"still reports just-bash for a pin that genuinely permits it"` in `explore-fail-closed.test.ts`; that case asserts the MARKER *does* appear, and deleting it would turn every `not.toContain(MARKER)` in the file into a tautology.
  - **`fumadocs-mdx`/`fumadocs-openapi` used to be listed here too, contradicting the constraints row** that says hold the whole family. The row wins; the family is now in EXCLUDE wholesale.
- **`(peer)`/`(optional)` rows are filtered out** — but that drops real, installed optional deps too (notably **`@vercel/sandbox`** and **`e2b`**, which appear only as `(optional)` in `@atlas/api` + `(peer)` in their plugins). Sweep those by **naming them explicitly in Step 3**, then fix the sub-workspace floor with `bun add --cwd packages/api --optional <pkg>@^<version>` — plain `bun update` does **not** rewrite an `optionalDependencies` floor. Don't let the filter silently skip a soak-track minor. Inspect the filtered rows deliberately:
  ```bash
  bun outdated --filter '*' 2>/dev/null | sed -E 's/\x1b\[[0-9;]*m//g' | sed 's/│/|/g' \
    | grep -E '^\| ' | grep -E '\(peer\)|\(optional\)' | sed -E 's/^\| +//' | sort -u
  ```
  A row whose `Update` column equals its current version has no within-range update and belongs to a deliberate bump instead — e.g. `@daytonaio/sdk` (#4889), where `^0.185.0` cannot reach `0.202.0` because **a caret on a `0.x` version pins the minor**.
...
# 3. the docs Next build (CI-only failure #3) — NOT covered by `bun run type`
rm -rf apps/docs/.next && bun run build:docs
# 4. the web Next build
rm -rf packages/web/.next && bun run build
```
**Why 3 and 4 are not optional:** `bun run type` is `tsgo --noEmit` plus `@atlas/web type` and `@atlas/www type` — **`apps/docs` is not in it at all**, and the docs build type-checks itself. A dep that only changes docs-toolchain types passes `type`, `lint` and every test, then fails in `Built image (docs)`. That is exactly how the `@shikijs/transformers` break reached CI on #4893.
...
3. **`stripe`** — a **major** silently bumps the pinned API version + touches Better-Auth Stripe plugin compat (billing); read the API-version changelog and run the billing tests *isolated*. ⚠️ **A minor can move the pinned API version too** (corrected 2026-07-29): 22.2.3 → 22.3.2 moved it from `"2026-05-27.dahlia"` to `"2026-06-24.dahlia"`, failing `type` at `packages/api/src/lib/billing/stripe-api-version.ts:26` on #4886. This page previously claimed minors don't. Check that constant against the SDK's expected literal before sweeping stripe, and treat any move as billing behaviour deserving its own PR.
...
| `@playwright/test` ↔ `@axe-core/playwright` peer (`playwright-core`) | `@axe-core/playwright`'s peer is `playwright-core: ">=1.0.0"` (resolves to the **highest** gate-eligible), while `@playwright/test`→`playwright` pins an **exact** `playwright-core`. Once a newer `playwright-core` becomes gate-eligible, bumping `@playwright/test` desyncs the two → **two `playwright-core` versions** → e2e `Page` type clash (`bun run type` fails). Main's single-version state is **inertia-only** (its lockfile predates the newer release); a fresh resolve would split it too. | **Hold BOTH `@playwright/test` and `@axe-core/playwright`** (both in EXCLUDE). **The split is two-sided:** bumping *axe* alone re-resolves its own loose `playwright-core: ">= 1.0.0"` peer up to the newest gate-eligible version while `@playwright/test` still pins its exact one — same clash, reached from the other direction. Holding only `@playwright/test` does **not** prevent it; that is what broke `type` on Dependabot's sweep #4886 (1.62.0 vs 1.60.0). A dev-only test runner and its axe helper one release behind cost nothing. The alternative is a permanent `"overrides": { "playwright-core": "<exact>" }` synced on every playwright bump; not worth it. See `reference_playwright_axe_core_peer_split` |
| `@shikijs/transformers` ↔ the `shiki` core that `fumadocs-core` pulls | `shiki` is **not** a direct dep — it arrives transitively via `fumadocs-core` (16.9.3 → `shiki@4.2.0`). `@shikijs/transformers` **is** direct, so a sweep moves it alone: 4.2.0 → 4.3.1 against a 4.2.0 core, and `CodeToHastOptions` (a shiki-core type) stops matching → `apps/docs` fails to build on `rehypeCodeDefaultOptions.transformers`. **This is a side effect of holding fumadocs** — the hold pins shiki, which pins transformers. `bun outdated` cannot see it: transformers looks like an ordinary safe minor. | In EXCLUDE as `@shikijs/*`. It moves when the fumadocs 16.10 / openapi-11 migration moves shiki — bump all three together. Verify with `bun run build:docs`, **never** `bun run type` (which does not cover `apps/docs` at all). Caught on #4893 |
| `@hono/zod-openapi` ≥1.5.0 ↔ any `OpenAPIHono` built **without** `defaultHook` | 1.5.x changed the *fallback* request-body validation response from `400` + a stringified `ZodError` (`{"success":false,"error":{"name":"ZodError",…}}`) to the router's own `defaultHook`. Any router constructed as a bare `new OpenAPIHono()` therefore flips 400→422 and changes its body shape. **This is not a validation weakening** — measured under both versions, every invalid input is still rejected and the handler never runs (`createSubscription` call count stays 0), so the SSRF guard is intact. It looked like a security break on #4894 only because 10 tests had codified the buggy fallback as expected. Root cause was **ours**: `sub-processor-subscriptions.ts` and `starter-prompts.ts` were the only two production routers missing the shared hook, while 120 assertions elsewhere already expected 422. | Fixed at the source (#4894): both routers now pass `{ defaultHook: validationHook }`, which makes 1.4.0 and 1.5.1 byte-identical, so the bump is orthogonal. **When adding an `OpenAPIHono`, always pass `defaultHook: validationHook`** — a bare one silently opts out of the API's 422 envelope. Two remaining bare routers are safe because they declare no request body (`me-connection-groups.ts`, `mode.ts`); `rest-operations.ts` has one but already declares 422. Changing a route's declared `responses` requires `bun run --filter '@atlas/api' openapi:extract` + `--filter '@atlas/docs' generate:api` or `check-openapi-drift.sh` fails |
| Any bump ↔ a **peer-differentiated duplicate** of an unrelated package | A bump can install a **second copy of the same version** of an unrelated package, because bun's store path encodes the resolved peer set (`@better-auth+core@1.6.25+<hash>`). Two copies at *identical version* but different `+hash` produce **nominal** type clashes — structurally identical types reported incompatible because their paths differ. Two measured cases (#4893): bumping `@opentelemetry/semantic-conventions` 1.41.1→1.43.0 made bun keep a nested 1.41.1 for `@better-auth/core` instead of deduping; and moving top-level `jose` to 6.2.4 split it against `@vercel/sandbox`'s **exact** `jose: "6.2.3"` pin. Both failed `agent-auth-plugin.test.ts:724` on better-call's `HookEndpointContext`. **Nothing better-auth-related was bumped in either case.** | Diagnose: `ls -d node_modules/.bun/<pkg>@*` and compare the count against a clean install of the base branch — stale dirs accumulate across install cycles, so confirm with `rm -rf node_modules && bun install --ignore-scripts` first. Then `readlink -f .../node_modules/<peer>` per copy and diff the peers; the one that differs is the culprit. Fastest signal is usually one line: `git diff origin/main -- bun.lock \| grep '"<parent>/'`. Fix by collapsing the duplicate — a top-level `override` if the consumer's declared range already admits the new version (`@better-auth/core` declares `^1.39.0`, so 1.43.0 violated nothing), or **hold the bump** if the consumer pins exactly, rather than forcing a vendor off its pin. Note a pre-existing split can be benign: `better-call` 1.3.2/1.3.7 gives 2 copies on `main` and the type gate passes — it only failed at 4. See `reference_peer_differentiated_duplicate_nominal_type_clash` |
...
- **`syncpack fix` converges to the HIGHEST version present — so a partial hold is silently undone.** Rolling a package back in root + `packages/api` does nothing if the two `create-atlas/templates/*` manifests still declare the newer version: `syncpack fix` sees the higher one and pushes every site back up. Find all sites first (`grep -rn '"<pkg>":' --include=package.json . | grep -v node_modules` — usually four for an api dep, five for `jose`) and set them **together**, then confirm `syncpack fix` reports 0 changes for that package. The templates are what bite: not workspace members, not install roots, so `bun add`/`bun update` cannot reach them and `syncpack fix` is the only thing that writes them — the same gap that made Dependabot's sweep unable to pass `drift` (#4890). **And a caret hold drifts on a fresh resolve** — `^1.4.0` re-resolves to 1.5.1 — so a hold that must survive a clean install has to be **pinned exactly**, per the `@typescript/native-preview` precedent (#4882). Cost two failed attempts in one sweep (#4893).
...
- **Transient infra flakes.** `Boot Build` / `canonical-eval` failing fast on `registry-1.docker.io ... Client.Timeout` are Docker-Hub timeouts, not your change. Re-run the job (`gh run rerun --job <id> -R AtlasDevHQ/atlas`); they're non-required checks anyway (merge gate = `ci` + `Deploy Validation` + `Image Scan` + `Analyze (JS/TS)` + `fork-pr-gate`; the `api-tests` shards and `ee-stub-build` are covered by the `ci` umbrella).
