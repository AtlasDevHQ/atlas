...
3. nsjail explicit (when `ATLAS_SANDBOX=nsjail` is set) — **hard fail**: if nsjail is unusable the API still boots, but the explore tool refuses every request rather than degrading to an unsandboxed backend. When nsjail is present but its namespaces do not work, `/api/health` reports `checks.explore.backend: "fail-closed"` and `components.sandbox.status: "down"`. Remove `ATLAS_SANDBOX` to fall through to auto-detect.
