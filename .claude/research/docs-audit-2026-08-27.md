...
- **Cross-audience links**: the content profile settled the "move vs bless" decision — none of the 14 targets except `embedding-widget` was audience-neutral in content, so bulk moves needed audience-branching passes first. `embedding-widget` moved immediately; new accidental cross-audience links now fail the docs-links gate (fixture-covered). **The shared-candidate migration then ran in the next PR**: eleven pages got `<WhenSaaS>`/`<WhenSelfHosted>` passes and moved to `content/shared/` (guides/{actions, caching, choosing-an-integration, integrations, mcp, slack, social-providers, troubleshooting}, security/{row-level-security, sql-validation}, platform-ops/encryption-key-rotation — the last three creating the `shared/security` and `shared/platform-ops` subtrees). Two stale claims were caught and fixed by the verification bar during the passes (social providers auto-enable from env at boot; the login page renders per-provider buttons). `CROSS_AUDIENCE_ALLOWED` is down to the three deliberate contrast pointers (hosted, billing-and-plans, signup).
