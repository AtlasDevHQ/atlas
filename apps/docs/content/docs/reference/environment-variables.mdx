...
| `ATLAS_REQUIRE_EMAIL_VERIFICATION` | `true` (prod) | Require email verification on signup. Default resolves from the `ATLAS_DEPLOY_ENV` profile — production `true`, staging/development `false`; an explicit value overrides the profile. Setting `false` re-enables Better Auth's `USER_ALREADY_EXISTS` signup error (an email-enumeration oracle) and is only acceptable for self-hosted single-tenant deployments without an email provider |
...
| `ATLAS_ONBOARDING_EMAILS_ENABLED` | `true` (prod) | Enable automated onboarding drip campaign for new signups. Default resolves from the `ATLAS_DEPLOY_ENV` profile — production `true`, staging/development `false`; an explicit value overrides the profile. Additionally requires `DATABASE_URL` and either `RESEND_API_KEY` or `ATLAS_SMTP_URL` |
...
| `ATLAS_DEPLOY_ENV` | `production` | Deployment shape: `production`, `staging`, or `development` (unset → `production`). Drives non-secret per-env defaults via `packages/api/src/lib/env-profile.ts` — production defaults email-verification and onboarding-emails ON; staging/development default both OFF. The per-var legacy overrides (`ATLAS_REQUIRE_EMAIL_VERIFICATION`, `ATLAS_ONBOARDING_EMAILS_ENABLED`) still win when explicitly set |
