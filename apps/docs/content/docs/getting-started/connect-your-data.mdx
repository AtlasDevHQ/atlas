...
description: Connect Atlas to PostgreSQL, MySQL, and plugin-based sources (ClickHouse, Snowflake, DuckDB, Salesforce).
---
...
<Callout type="info">
ClickHouse, Snowflake, DuckDB, and Salesforce are implemented as datasource plugins in the `plugins/` directory (e.g. `plugins/clickhouse`). PostgreSQL and MySQL are built into `@atlas/api` directly.
...
See [CLI Reference](/reference/cli#init) for all `init` flags including `--source`, `--connection`, `--csv`, and `--parquet`.
...
For multi-tenant deployments, enable [Row-Level Security](/security/row-level-security) to automatically filter queries by tenant based on user claims.
...
> The `sources` field appears when at least one datasource is registered in the ConnectionRegistry. The `explore` field reflects the active sandbox backend (sidecar, nsjail, vercel-sandbox, plugin, or just-bash as fallback).
