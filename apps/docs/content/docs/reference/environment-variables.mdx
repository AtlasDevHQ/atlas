...
See [SQL Validation Pipeline](/security/sql-validation) for how these are enforced.
...
See [Authentication](/deployment/authentication) for full setup guides.
...
See [Sandbox Architecture](/architecture/sandbox#backend-selection-priority) for the full selection priority.
...
These env vars can also be set via `atlas.config.ts` under `actions.defaults`. See the [declarative config guide](/reference/config) for details.
