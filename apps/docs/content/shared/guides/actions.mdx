...
Credentials for JIRA and GitHub can also be set per workspace under **Admin > Action Credentials**, which takes precedence over these variables.
</Callout>
</WhenSelfHosted>
...
Action targets — JIRA and GitHub today — resolve their credentials **per workspace**, from an encrypted row a workspace admin sets under **Admin > Action Credentials** (`PUT /api/v1/admin/action-credentials/{target}`).
