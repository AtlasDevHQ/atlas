...
All seventeen are hidden from the generic settings page on Atlas Cloud, so a workspace admin cannot read or write any of them. Six are **workspace-scoped** — meaning each can hold a different value per workspace, set by a platform admin: `ATLAS_BRAIN_AUDIENCE_SYNC_ENABLED`, `ATLAS_BRAIN_COVERAGE_SNAPSHOT_ENABLED`, `ATLAS_BRAIN_WAREHOUSE_CADENCE_ENABLED`, `ATLAS_BRAIN_EXTRACTION_MODEL` and the two alias auto-approval keys. The other eleven are one value for the whole deployment.
...
Query safety, SQL validation, and security-disclosure settings. See <AudienceLink saas="/security/sql-validation">SQL Validation Pipeline</AudienceLink> for how the query-safety settings are enforced.
