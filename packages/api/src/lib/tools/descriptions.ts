...
export const SEARCH_BRAIN_TOOL_DESCRIPTION = `Search the company brain — a trust-labeled read over three stores. \`tier: "fact"\` is human-reviewed; \`tier: "raw-episode"\` is its source record; \`tier: "document"\` is hosted knowledge — ${KNOWLEDGE_TRUST_FRAMING}. When \`provenance.attribution\` is \`{ "visible": false }\`, author, source id and timestamp are withheld: use the claim, say attribution is restricted; never call it anonymous, undated, unsourced; never infer the author. Retracted claims are excluded. Unextracted episodes carry \`extraction: "pending"\`. Age metadata (\`validFrom\`, \`corroborationCount\`, \`decay\`): present a stale fact's age, never assert it as current or drop it. Unranked \`tensions\` list conflicts: report every side, never pick a winner. \`unavailable\` = the search failed, not "nothing known". Example: \`{ "query": "who owns billing" }\`.
