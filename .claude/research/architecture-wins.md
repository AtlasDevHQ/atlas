...
- **A misroute is now a TS error in four distinct shapes.** Verified with a throwaway witness file: `decryptUrlSecret(opaqueValue)` (TS2345), `decryptOpaque(urlValue)` (TS2345), `const x: URLSecret = encryptOpaque(...)` (TS2322), `const y: OpaqueSecret = encryptUrlSecret(...)` (TS2322). The four-way catch covers both encrypt-side annotation (the most common misroute path — IDE auto-import) and the decrypt-side feed (the secondary path).
