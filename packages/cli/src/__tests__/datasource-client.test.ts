...
describe("datasource-client route mapping (#4044)", () => {
  ...
  it("publish → POST /api/v1/admin/publish with no archiveConnections (#4126)", async () => {
    ...
    // The fixture above is an OLDER API's response (no knowledgeDocuments and
    // no brainFacts) — the schema's `.default(0)` fills both in, pinning the
    // deploy-overlap tolerance.
