...
**Atlas** is built for developers and small-to-mid teams who want to add natural language data querying to their own applications. It's open-source (AGPL-3.0 core, MIT client libs), deploys in minutes, and is designed to be embedded — not to be a standalone analytics platform. [Atlas Cloud](https://app.useatlas.dev) provides a hosted SaaS option with US-hosted, with EU and APAC on request, SSO/SCIM, data residency, and SLA commitments — at a fraction of enterprise platform pricing.
