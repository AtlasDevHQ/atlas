...
| `install_model` | `"oauth" \| "form" \| "static-bot"` | Yes | — | Install-handler dispatch key. `oauth` runs a per-Workspace OAuth dance (Slack, Jira, Salesforce, GitHub App, Linear OAuth). `form` validates customer-supplied fields against `configSchema` (Email, Webhook, Obsidian, GitHub PAT, Linear API key). `static-bot` uses an operator-shared bot and a per-Workspace routing identifier (Teams, Discord, Google Chat, Telegram, WhatsApp) |
