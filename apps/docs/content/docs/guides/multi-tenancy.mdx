...
|----------|-------------|
| **owner** | Full control — update/delete the org, manage members, connections, semantic layer, settings |
| **admin** | Manage members, connections, semantic layer, and settings (cannot delete the org) |
| **member** | Read connections and semantic layer, create and read conversations |
...
Each organization has its own semantic layer stored in the internal database (`semantic_entities` table). The DB is the source of truth; Atlas syncs entities to disk at `semantic/.orgs/{orgId}/` for the explore tool (the agent reads files via `ls`, `cat`, and `grep`).
...
Manage org-scoped entities via the admin API:
...
export default defineConfig({
  ...
  maxTotalConnections: 100, // Hard cap across all pools (default: 100)
});
```
...
### Org members can't see each other's conversations

**This is expected behavior.** Conversations are scoped to the org via the `org_id` column. Users only see conversations created within their active organization. Switching orgs shows a different conversation history.
