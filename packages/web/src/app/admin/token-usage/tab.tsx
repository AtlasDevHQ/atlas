...
/**
 * Token Usage rendered as a tab inside `/admin/usage`. The previous
 * standalone `/admin/token-usage` route was folded in to deduplicate the
 * Monitoring nav group (the two pages reported overlapping concepts —
 * token-usage is just one slice of overall plan consumption). The old URL
 * still resolves via a redirect on `token-usage/page.tsx`.
 *
 * This is a pure body component — no `<h1>`, no page-level `<div
 * className="p-6">`. The host page (`usage/page.tsx`) provides the chrome.
 */
