...
- [Managed auth](/self-hosted/deployment/authentication#managed-auth-self-hosted-setup) enabled (`BETTER_AUTH_SECRET` set)
...
curl -X POST https://your-atlas.com/api/auth/organization/create \
  ...
  -H "Cookie: atlas.session_token=<session>" \
  -d '{
    "name": "Acme Corp",
    "slug": "acme-corp"
  }'
...
```bash
# List entities for the active org (unified DB + disk view)
curl https://your-atlas.com/api/v1/admin/semantic/entities \
  -H "Cookie: atlas.session_token=<session>"
...
<Callout type="info" title="Unified read endpoints">
`GET /api/v1/admin/semantic/entities` and `GET /api/v1/admin/semantic/entities/{name}` are the canonical read endpoints — they merge org-scoped DB rows with the disk view and apply the developer-mode overlay rules, so the admin console list and detail can't disagree on what's visible. The previous `GET /api/v1/admin/semantic/org/entities[/{name}]` routes still work for backward compatibility but are deprecated. Write operations (`PUT` / `DELETE` / `/org/import`) remain on the `/org/` path.
</Callout>
...
Cache behavior is tuned at runtime from **Admin → the Cache page** — no
redeploy needed. Self-hosted deployments can also set boot defaults via the
`ATLAS_CACHE_*` env vars (`ATLAS_CACHE_ENABLED`, `ATLAS_CACHE_TTL`,
`ATLAS_CACHE_MAX_SIZE`); a `cache:` block in `atlas.config.ts` is no longer
supported and fails validation at boot. See <AudienceLink selfHosted="/self-hosted/deployment/cache-configuration">Cache Configuration</AudienceLink> for details.
...
Role changes and member removals go through the Atlas admin API, keyed on the
target **user id**. These routes enforce the workspace last-admin invariant
atomically (you can't demote or remove the last admin/owner of a workspace), so
the native Better Auth `organization.updateMemberRole` / `organization.removeMember`
endpoints are intentionally **disabled** — they return `403 ATLAS_USE_ADMIN_API`.

```typescript
// Update a member's role (writes the per-workspace member.role).
// A platform admin acting cross-tenant passes the target's workspace as
// `organizationId`; an org admin/owner omits it (their active workspace is used).
await fetch(`/api/v1/admin/users/${userId}/role`, {
  method: "PATCH",
  headers: { "Content-Type": "application/json" },
  credentials: "include",
  body: JSON.stringify({ role: "admin", organizationId: "org-id" }),
});

// Remove a member from a workspace (does not delete their account).
await fetch(`/api/v1/admin/users/${userId}/membership`, {
  method: "DELETE",
  credentials: "include",
});
```
...
<Callout type="info">
Inviting members still uses `authClient.organization.inviteMember()` (above) — only the role-update and member-removal endpoints are routed through the guarded admin API.
</Callout>
...
**Fix:**
1. Check if entities exist for the org: `curl /api/v1/admin/semantic/entities`
...
- <AudienceLink selfHosted="/self-hosted/deployment/authentication#managed-auth-self-hosted-setup">Authentication</AudienceLink> — Set up managed auth (required for organizations)
