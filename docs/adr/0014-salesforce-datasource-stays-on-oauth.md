...
- **Installs via OAuth** — `SalesforceOAuthInstallHandler` writes a `workspace_plugins` row whose `config` carries `instance_url` / `scopes` / `status` (operator-visible, **no `url`**) and persists the access/refresh tokens, encrypted, in `integration_credentials` (ADR-0005). There is no credential form and no `url` to encrypt.
- **Is queried via the `LazyPluginLoader`** — on first tool call per workspace the lazy builder (`integrations/salesforce/lazy-builder.ts`) reads the OAuth bundle, builds a `jsforce` session from `instance_url` + `accessToken`, refreshes inline on `INVALID_SESSION_ID`, and exposes a per-workspace `querySalesforce` tool. The agent uses `querySalesforce` (SOQL), **not** `executeSQL`.
...
The result is **exactly one live Salesforce query path** (OAuth → `LazyPluginLoader` → `querySalesforce`), pinned by a regression test in `datasource-registry-bridge.test.ts` (the bridge returns `false` and never calls `createFromConfig` for `salesforce`, whether or not a salesforce plugin is registered).

## Alternatives considered

### (a) Fold Salesforce onto the bridge via a credential-form install (rejected for now)

Add a Salesforce credential-form catalog row (a `salesforce://user:pass@host?token=…` url field) so an install builds the connection through `createFromConfig` and `executeSQL` queries it, uniform with ClickHouse et al. **Rejected:** it would create a *second* Salesforce install + query path competing with the existing OAuth one (OAuth is the better auth model — no long-lived password/security-token in `config`, refresh-token rotation, per-org `instance_url`), and SOQL is queried through the dedicated `querySalesforce` tool, not the generic SQL pipeline. ADR-0007 already designated Salesforce OAuth-managed. The `createFromConfig` seam is kept dormant so this option stays open if a username/password Salesforce datasource is ever wanted, but it is not wired today.
...
- The OAuth path (handler + lazy builder + `querySalesforce`) is untouched and remains the canonical, documented way to connect Salesforce as a datasource.
