...
const SECTIONS: LegalSectionData[] = [
  ...
  },
  {
    ...
    legal: [
      "Atlas Cloud is hosted in the United States, with Europe and Asia Pacific available on request on every paid plan. Customer Data does not leave the selected region except for transactional services (billing, status email) which are processed in the United States.",
  ...
  },
  {
    ...
    title: "Security",
    legal: [
      "Atlas maintains a security program aligned with ISO 27001 and SOC 2 Type II controls. Highlights: TLS 1.2+ in transit, AES-256-GCM at rest with versioned key rotation, encrypted internal-database storage of all integration credentials and connection strings, automated vulnerability scanning of both software dependencies and container images, static application security testing on every change, secret scanning with push protection, two-factor authentication (TOTP or passkey) required for every administrator account on managed-mode sessions, and a documented incident-response runbook published in our security documentation. Per-customer KMS keys are on the public roadmap and negotiable on enterprise contracts. Atlas does not currently hold a SOC 2 Type II report or ISO 27001 certificate and does not maintain a formally documented ISMS today; formal certification is on the roadmap, as is third-party penetration testing.",
    ...
    plain:
      "TLS 1.2+ in transit, AES-256-GCM at rest with versioned key rotation, two-factor (TOTP or passkey) required for admin accounts, automated vulnerability scanning of both our dependencies and the container images we ship, and a published incident-response runbook. We do not hold SOC 2 Type II or ISO 27001 certification today, and we don’t claim controls we haven’t built — certification and third-party penetration testing are on the roadmap, not in place. Report suspected issues to security@useatlas.dev (PGP key at useatlas.dev/.well-known/atlas-security.asc).",
  },
  {
    id: "cookies",
