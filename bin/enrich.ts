...
import { getModel } from "../src/lib/providers.js";
...
function formatTableProfile(profile: TableProfile): string {
  const lines: string[] = [];
  lines.push(`Table: ${profile.table_name}`);
...
function formatAllTablesOverview(profiles: TableProfile[]): string {
  ...
  for (const p of profiles) {
    lines.push(`\nTable: ${p.table_name} (${p.row_count} rows)`);
...
export async function enrichSemanticLayer(
  ...
  await enrichGlossary(profiles, model, usage);
