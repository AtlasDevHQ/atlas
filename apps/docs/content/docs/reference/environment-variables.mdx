...
| `TURNSTILE_SECRET_KEY` | — | Cloudflare Turnstile secret for siteverify on `/api/v1/contact` and the `start_trial` MCP onboarding bootstrap. When unset, `verifyTurnstile()` fails closed (no submissions / trials accepted) |
