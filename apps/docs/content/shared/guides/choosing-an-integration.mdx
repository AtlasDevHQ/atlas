...
description: Comparison tables to help you pick the right integration surface — and, when self-hosting, the right auth mode — for your Atlas deployment.
---

import { Callout } from "fumadocs-ui/components/callout";

Atlas offers multiple integration surfaces — and, on a self-hosted deployment, a choice of authentication mode. These tables help you choose the right combination for your use case.
...
<WhenSaaS>
<Callout type="info">
In the examples below, replace `https://your-api.example.com` with the hosted API endpoint, `https://api.useatlas.dev`, and `sk-...` with an API key created from <AudienceLink saas="/guides/api-keys">Admin → API Keys</AudienceLink>.
</Callout>
</WhenSaaS>

<WhenSelfHosted>
<Callout type="info">
In the examples below, replace `https://your-api.example.com` with your Atlas API server's public URL.
</Callout>
</WhenSelfHosted>
...
<WhenSaaS>
On the hosted platform, authentication is preconfigured — every workspace runs **managed** auth. Users sign in with workspace accounts, get per-user roles (viewer, analyst, admin), and keep their own conversation history. For programmatic access — the SDK, the REST API, or an embedded widget — create an API key from <AudienceLink saas="/guides/api-keys">Admin → API Keys</AudienceLink> and send it as a Bearer token. The auth-mode choice described on the self-hosted version of this page is a deployment decision that the platform has already made for you.
</WhenSaaS>
...
- <AudienceLink selfHosted="/self-hosted/deployment/authentication">Authentication</AudienceLink> — detailed auth setup, roles, rate limiting, and audit logging
- <AudienceLink saas="/guides/api-keys">API Key Management</AudienceLink> — create and revoke keys for programmatic access
