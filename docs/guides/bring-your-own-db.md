...
Salesforce uses SOQL, not SQL. It has a separate adapter and query tool (`querySalesforce`) rather than sharing the standard `executeSQL` pipeline.
...
See `packages/api/src/lib/config.ts` for the full configuration schema.
