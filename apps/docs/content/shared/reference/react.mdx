...
```typescript
// Pre-built AtlasChat component + provider
...
Both entry points export the same `AtlasProvider`. The main entry point (`@useatlas/react`) also exports the pre-built `AtlasChat` component and `useConversations` (lower-level, takes explicit auth config). The hooks entry point (`@useatlas/react/hooks`) exports context-aware hooks (`useAtlasChat`, `useAtlasAuth`, `useAtlasConversations`) for building custom UIs with a smaller bundle.
...
### REST writes are unavailable in the widget

Allowlisted [REST datasource writes](/plugins/datasources/openapi-generic#writing--opt-in-then-confirm) require a chat surface that can render the confirm-before-write card and POST the confirmation. The widget does not, so `/api/v1/chat` requests it makes are **not offered writes**: the agent refuses a write up front with `write_confirmation_unavailable` and stages nothing ([#5498](https://github.com/AtlasDevHQ/atlas/issues/5498)). REST *reads* work normally. A custom surface that implements its own confirm card can declare the capability by sending the `x-atlas-write-confirm-ui: 1` header on its chat requests — the header is a capability declaration, not a security control; the confirm endpoint still re-validates the allowlist and burns a single-use server-signed token.
...
Wraps the SDK's `beginConnect` + `completeConnect` with a popup-or-redirect lifecycle so React embedders can drop a "Connect your AI agent" button into their app without owning OAuth bookkeeping. Exported from both `@useatlas/react` and the `@useatlas/react/hooks` subpath.
...
interface Conversation {
  ...
  connectionId: string | null;                 // Datasource (replica) SQL runs against
  connectionGroupId: string | null;            // Connection group whose content resolves
  ...
  answerStyle?: AnswerStyle | null;            // "plain-english" | "analyst" | "executive" | "conversational"; null = default
...
type ChatErrorCode =
  ...
  | "session_expired"
  | "forbidden_role"
  // Deprecated — no route emits this. The no-active-organization
  // response is `bad_request`. Retained for wire compatibility only.
  | "org_not_found"
  | "plan_limit_exceeded"
  | "trial_expired"
...
// Root entry point exports — the MCP onboarding hook + its types are also re-exported from /hooks; the rest are root-only
import type {
  UseConversationsOptions,
  UseConversationsReturn,
  FavoriteStarterPrompt,

  // Cross-environment routing (1.4.5)
...
### Cross-environment routing types

When a conversation runs against a multi-member connection group, the `executeSQL` tool result carries a `envContributions` array (one entry per group member) alongside the merged `columns` / `rows`. Custom tool renderers that consume the wire type directly should import these from `@useatlas/react`:

| Type | Description |
|------|-------------|
| `ConnectionContribution` | `{ connectionId, rowCount, durationMs, error }` per env. Read `error` to render partial-failure badges |
...
| `PythonResultCard` | The `executePython` result card (stdout, artifacts, progress). Also exports `PythonResultCardProps`, `PythonProgressData` |
...
Type your own `executeSQL` / `explore` / `executePython` renderers against the same wire shapes `AtlasChat` uses:
...
|--|--|--|
| **Provider** | `AtlasProvider` (same component, re-exported from both) | `AtlasChat` provides `AtlasContext` internally — no separate wrapper needed |
