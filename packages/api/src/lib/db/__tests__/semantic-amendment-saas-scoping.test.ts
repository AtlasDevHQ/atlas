...
/**
 * Rows the insert-time conflict SELECT returns for the current test. Empty for
 * every scoping/decide-seam test below; the rejection-memory describes (moved
 * here from semantic-amendment-rejection-guard.test.ts) seed it.
 */
...
function makeStubPool() {
  return {
    query: async (sql: string, params?: unknown[]) => {
      ...
      // `conflictRows`, which is empty unless a test seeds it.
...
/**
 * ── Moved from semantic-amendment-rejection-guard.test.ts ──
 *
 * Insert-enforced rejection memory + pending dedup for semantic amendments
 * (#4507). That file's own header said it used "the same harness as
 * semantic-amendment-saas-scoping.test.ts", which it did, down to a second copy
 * of the captured-SQL stub pool — so it lives here now against the one copy.
 *
