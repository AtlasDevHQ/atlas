...
`POST /api/chat` streams the agent's response in real time using Server-Sent Events (SSE). This is what the web UI uses. The response includes tool calls, intermediate results, and the final answer as they happen.
...
The response streams as SSE events. The `x-conversation-id` header contains the conversation ID for follow-up messages.
...
Conversation endpoints require an internal database (`DATABASE_URL`). They return `404 not_available` if no internal database is configured.
...
```json
{
  ...
  "version": "0.0.6",
  "components": {
    "datasource": { "status": "healthy" },
    "internalDb": { "status": "healthy" },
    "llmProvider": { "status": "healthy", "provider": "anthropic", "model": "claude-sonnet-4-20250514" },
    "sandbox": { "status": "healthy", "backend": "sidecar" },
