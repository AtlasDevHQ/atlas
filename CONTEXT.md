...
- **Company Atlas** — what Atlas has learned about the customer's business, held as **reviewed facts** in the Atlas substrate (ADR-0036). Its admin surface is `/admin/brain`, its own sidebar group.
  ...
  **It yields to the warehouse.** A reviewed fact is authoritative for its class but loses to tier 1 — the analytics Datasource resolved through the semantic layer — in any overlap (`TRUST_TIERS`). "The Atlas knows things your database doesn't" is the correct pitch; "the Atlas overrides your database" is not.
  ...
  _Avoid_: **"Brain Facts"** as a product label — `brain_facts` is the table, and until #5066 it was wearing a product name in the sidebar. Say **Company Atlas** for the surface and the concept; **reviewed fact** (or plainly, *fact*) for a single row. Also avoid **"Company Brain"** in any copy a customer reads (ADR-0038), while leaving every developer surface alone: the table name, the API path (`/api/v1/admin/brain-facts`, OpenAPI tag "Admin — Brain Facts"), the `/admin/brain` route, and the `searchBrain` tool name all stay as they are — the rule is about product copy. The two ingest catalog rows above (Zoom, Outlook) no longer read "Company Brain" in their **`name` and `description`** (#5082) — and the rename is migration `0201_brain_catalog_rows_company_atlas.sql`, not a constant edit, because the seeder is `ON CONFLICT DO NOTHING` and editing `seed-builtin-knowledge-catalog.ts` alone renames nothing a region already holds. **Two stored strings still say "brain" and are KNOWN, not drift**: each of those rows' `config_schema` carries `"Optional. A human description of this brain source."`, which is customer-read helper text on the install form, frozen by the same `ON CONFLICT DO NOTHING`. Rewriting a string inside a JSONB array is a materially less safe statement than 0201's two guarded column updates, so it is deliberately deferred to #5240; a test in `seed-builtin-knowledge-catalog.test.ts` pins the current wording so the next editor is told rather than surprised. The general rule this leaves behind: **changing a field on a built-in catalog row that regions already hold takes a migration**. `ON CONFLICT DO NOTHING` freezes *every* column of an existing row, so enforcement is a chosen subset: for the **knowledge** catalog, a copy lock over all fourteen rows' `name`, `description`, `saasEligible` and `autoInstall`, plus an individual pin on the two exempt `config_schema` strings above. The built-in **datasource** catalog (`seed-builtin-datasource-catalog.ts`) seeds customer-read copy through the identical unqualified `ON CONFLICT DO NOTHING` and carries no lock at all. Also avoid calling it a pillar, a knowledge base, or "the memory" (**Session Memory** is a different, per-conversation thing under Intelligence).
