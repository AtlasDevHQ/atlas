...
- [ ] **Encrypted at rest** — Bearer secrets in the internal DB (connection URLs, `workspace_model_config.api_key_encrypted`, every integration credential column — bot tokens, API keys, email/sandbox JSON blobs) go through `encryptUrl` (URLs) or `encryptSecret` (everything else) before write. Integration credentials currently dual-write plaintext + `*_encrypted` for back-compat during the F-41 soak; the read path prefers the encrypted column via `pickDecryptedSecret`. Never add a new credential column without an `_encrypted` sibling. Key derivation: `ATLAS_ENCRYPTION_KEY` → `BETTER_AUTH_SECRET` (F-47 / #1820 rotation path pending)
