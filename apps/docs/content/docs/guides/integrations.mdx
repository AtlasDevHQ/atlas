...
Workspace admins see a **Connect to Discord** button when the platform has the Discord bot install configured (`DISCORD_BOT_TOKEN` + `DISCORD_CLIENT_ID`). If not, the card falls back to the BYOT credentials form. The card only shows **Not Available** when neither the platform bot install nor BYOT is possible.
...
| `starter` | Paid Starter and above. Slack ships at this tier.              |
| `pro`     | Pro and above.                                                 |
| `business`| Business / premium integrations (Salesforce ships here).       |
...
plan-tier check for Atlas's own dogfood / per-region operator orgs.
The flag is operator-only — never exposed in customer-facing UI or
billing surfaces.
