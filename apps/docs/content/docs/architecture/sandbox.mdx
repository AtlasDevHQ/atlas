...
The sidecar backend enforces a **10-second command timeout** (`DEFAULT_TIMEOUT_MS = 10_000`) for shell commands. The HTTP fetch uses a total abort signal of **15 seconds** (10s command timeout + 5s HTTP overhead) to account for network latency and response serialization. Shell commands that exceed the timeout return exit code 124 (matching GNU `timeout(1)` convention).

Python execution via the sidecar uses a longer default timeout of **30 seconds** (`PYTHON_DEFAULT_TIMEOUT_MS = 30_000`) with a maximum of **120 seconds**. The HTTP abort signal adds 10 seconds of overhead on top of the execution timeout.
