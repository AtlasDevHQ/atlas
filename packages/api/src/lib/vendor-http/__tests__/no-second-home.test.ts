 ...
 *
 * ⚠️ Scoped to `lib/tools/actions/` on purpose. The ~10 `lib/brain/ingest`
 * connectors adopt the spine opportunistically when next touched (#5569), so
 * widening this glob would turn a deliberate non-migration into a red suite.
 * When they have adopted, widen it in the same commit — not before.
 */
...
describe("lib/tools/actions has no second home for a spine concern", () => {
  it("⭐ defines isAbortError nowhere outside the spine", () => {
    ...
    const offenders = actionClientSources()
      .filter(({ source }) => /function\s+isAbortError\b/.test(source))
      .map(({ file }) => file);
  ...
  it("does not hand-roll a deadline around a vendor fetch", () => {
    ...
    const offenders = actionClientSources()
      .filter(
        ({ source }) =>
          /new AbortController\(\)/.test(source) && /setTimeout\([^)]*abort/.test(source),
      )
      .map(({ file }) => file);
    expect(offenders).toEqual([]);
  });
