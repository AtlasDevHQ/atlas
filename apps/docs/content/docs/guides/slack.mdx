...
For distributing Atlas to multiple Slack workspaces. Requires an [internal database](/docs/reference/environment-variables) (`DATABASE_URL`) to store per-workspace bot tokens.
...
When the [action framework](/docs/guides/actions) is enabled and a query produces pending actions, Atlas posts approval buttons visible only to the requesting user. Clicking **Approve** or **Deny** triggers `POST /api/slack/interactions`.
