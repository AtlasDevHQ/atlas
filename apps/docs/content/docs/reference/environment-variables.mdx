...
| `ATLAS_REQUIRE_EMAIL_VERIFICATION` | Profile-driven (`production`: `true`; `staging`/`development`: `false`) | Require email verification on signup. The default follows the [`ATLAS_DEPLOY_ENV`](#runtime) profile; setting this var explicitly always wins (`false`/`0`/`no`/`off`, case-insensitive → off; anything else → on). Turning it off re-enables Better Auth's `USER_ALREADY_EXISTS` signup error (an email-enumeration oracle) and is only acceptable for self-hosted single-tenant deployments without an email provider |
...
| `ATLAS_ONBOARDING_EMAILS_ENABLED` | Profile-driven (`production`: `true`; `staging`/`development`: `false`) | Enable the automated onboarding drip campaign for new signups. When set, only the literal `true` enables it (any other value disables — the override grammar isn't widened). When unset, the [`ATLAS_DEPLOY_ENV`](#runtime) profile default applies (`production` → on; `staging`/`development` → off). Also requires `DATABASE_URL` and either `RESEND_API_KEY` or `ATLAS_SMTP_URL` |
...
| `ATLAS_DEPLOY_ENV` | `production` | Deployment-shape discriminator: `production`, `staging`, or `development`. Drives the typed non-secret runtime defaults in `env-profile.ts` — currently `ATLAS_REQUIRE_EMAIL_VERIFICATION` and `ATLAS_ONBOARDING_EMAILS_ENABLED` (both `true` under `production`, `false` under `staging`/`development`). Unset (or an unrecognized value) resolves to `production`, preserving the historical self-hosted defaults. Each profile field still has a legacy per-var override that wins when set explicitly. Distinct from [`ATLAS_DEPLOY_MODE`](#enterprise) (`saas`/`self-hosted`), which gates enterprise features rather than runtime defaults |
...
| `ATLAS_DASHBOARD_DRAFTS_ENABLED` | `true` | Per-user dashboard drafts (1.4.6). When enabled, admin mutations to a bound dashboard write to the caller's draft and the **Publish** flow promotes them via an atomic three-way merge against the persisted baseline. Set to the literal string `false` to fall back to the pre-1.4.6 direct-write model — draft fetch returns `503` and the chat-bound editor degrades to a read-only viewer. See [Dashboards → Chat as editor](/guides/dashboards#chat-as-editor) |
| `ATLAS_INTERNAL_SCREENSHOT_COOKIE` | — | Service cookie value used by the 1.4.6 `screenshotDashboard` vision tool (#2367). The headless Chromium worker calls back into `/admin/dashboards/:id/screenshot` and must authenticate as the requesting user; without this cookie the screenshot request is rejected and the agent falls back to its last-good description. Set this on **both** the API and web services when running behind a reverse proxy. See [Dashboards → Chat as editor](/guides/dashboards#chat-as-editor-146) |
