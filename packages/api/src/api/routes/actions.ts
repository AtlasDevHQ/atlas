...
const redispatchActionRoute = createRoute({
  ...
  description:
    "Executes an action whose row is stuck at `approved` — the approval landed, but nothing ran. Since the executor registry is keyed by action type and populated at boot, the only way to reach that state is an instance that does not have the action's type loaded (actions disabled on this deploy, or a plugin declaring the type that is not wired). List the backlog with `GET /?status=approved`. " +
