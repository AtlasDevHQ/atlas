...
> **Prerequisites:** A running Atlas API server. See [Bring Your Own Frontend](/self-hosted/frameworks/overview) for architecture and common setup.
...
No AI SDK packages are required. If you prefer the `useChat` hook, you can install `@ai-sdk/react` and follow the [React Vite guide](/self-hosted/frameworks/react-vite) -- it works the same in TanStack Start.
...
If you use the `@ai-sdk/react` `useChat` approach, see the [React/Vite guide](/self-hosted/frameworks/react-vite#5-rendering-tool-calls) for a complete `ToolPart` implementation -- it works identically in TanStack Start. The data shapes are the same across all frameworks:
...
For the TanStack Query approach, the sync endpoint returns data in the `data` array directly -- no tool part parsing needed. See the [Data Stream Protocol](/self-hosted/frameworks/overview#tool-call-parts-ai-sdk-v6) section for details on both response formats.
