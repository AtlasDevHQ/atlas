...
- **Request-body validation on two endpoints now returns `422`, not `400` (#4894).** `POST /api/v1/sub-processor-subscriptions`, `POST /api/v1/starter-prompts/favorites`, and `PATCH /api/v1/starter-prompts/favorites/:id` previously answered a malformed body with `400` and a raw stringified `ZodError` (`{"success":false,"error":{"name":"ZodError","message":"[…]"}}`). They now return `422` with the same structured envelope every other validation failure in the API already used — `{ "error": "validation_error", "message": "Invalid request body", "details": [ …one entry per issue… ] }`. **Nothing about what is accepted has changed:** every input rejected before is still rejected, and the SSRF guard on the sub-processor webhook URL (loopback / RFC1918 / link-local / metadata-service / non-https) is unaffected. These two routers were simply the only ones in the API built without the shared `defaultHook`, so they alone leaked the upstream default instead of Atlas's envelope. **If you check for `400` on these three routes, check for `422`.** The genuine business-logic `400`s on `POST /favorites` — favorite cap exceeded, unusable text, no active workspace — are unchanged and still `400`.

### Removed

- **Notebook surface retired ([ADR-0035](docs/adr/0035-retire-the-notebook-surface.md), #4587).** The notebook — a cell-based analysis document painted over a chat transcript — is removed end-to-end: the `/notebook` route and its components, the fork/branch endpoints (`POST /:id/fork`, `DELETE`/`PATCH /:id/branches/:branchId`), the chat→notebook conversion (`POST /:id/convert-to-notebook`), the notebook-state persistence route (`PATCH /:id/notebook-state`), and the "Share as Report" (`/report/[token]`) path. Its audit found the cell curation was a display-only projection that had silently diverged from the persisted/shared truth (deletes that didn't delete, shares that shipped the raw transcript); post-dashboards elevation ([ADR-0029](docs/adr/0029-dashboards-draft-first-editing.md)/[ADR-0034](docs/adr/0034-dashboard-draft-cache-single-edit-mechanism.md)), dashboards own the agent-built, curated, shareable-artifact job on a sounder model. **Your data is preserved:** existing notebook conversations are converted to ordinary web chats with their full message history intact (fork children become standalone chats); nothing is deleted. Branching exploration is dropped with no successor. Chat conversation sharing is unaffected. The `notebook_state` column and the `"notebook"` `Surface` value are dropped in a follow-up release (two-phase migration, #4588).
