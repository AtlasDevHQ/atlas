...
Wire everything together with `createPlugin()`, which validates config via a Zod schema and returns a factory function. (For plugins that don't need runtime configuration, use `definePlugin()` instead -- see [`createPlugin` vs `definePlugin`](#createplugin-vs-defineplugin) below.)
...
<Callout type="warn">
`parserDialect` and `forbiddenPatterns` are validated at plugin registration time but **not yet consumed at query time**. Wiring these into the core SQL validation pipeline is tracked in [#15](https://github.com/AtlasDevHQ/atlas/issues/15). Until that lands, use a custom `validate` function if you need to enforce dialect-specific validation rules at runtime.
</Callout>
