...
- **A workspace-admin off-switch for raw SQL** (gate-1 analog) — **BUILT. Shipped 2026-07-01** ([#4095](https://github.com/AtlasDevHQ/atlas/issues/4095), PR [#4152](https://github.com/AtlasDevHQ/atlas/pull/4152), milestone `v0.0.36 — CLI & MCP Hardening`). A workspace admin can disable raw `executeSQL` over the CLI and MCP; the default is enabled, so rollout changed no behaviour, and the agent loop / chat / NL `atlas query` are never gated by it. It slotted into the gate-1 seam as shaped.
  - ⚠️ **This bullet said "deferred… Not built now" until #5350, and a note added 2026-08-18 reasoned about whether to un-defer it — six weeks after it had shipped.** The stale text is worth recording rather than quietly deleting, because of what it cost: an ADR is read as the standing account of a decision, so a security control that EXISTS was documented as absent, and the later note then argued the deferral's rationale against a with-customers threat model that no longer needed arguing. The original deferral reasoning — Atlas pre-customer, raw SQL already contained by member-floor + whitelist + RLS + approval — is preserved below as the record of why it was deferred *before* it was built.
  - Original deferral rationale (2026-06-29, superseded by the build): shaped to slot into the gate-1 seam; not built at the time because Atlas was pre-customer and raw SQL was already contained by member-floor + whitelist + RLS + approval; default would be enabled. The 2026-08-18 note observed that the "Atlas is pre-customer" half of that rationale had expired with the clean-break window (`docs/contexts/deployment-posture/CONTEXT.md`) — correct in itself, and moot, since the control was already shipped.
