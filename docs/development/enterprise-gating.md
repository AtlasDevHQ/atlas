...
- **Web:** `useDeployMode()` only. Its resolution can be a **guess** (hostname fallback while loading / on fetch error / `enabled: false`) — a component that swaps whole views or chooses *write vocabularies* by mode must handle `loading`/`error` instead of committing to the guess (#3378).
