...
Both entry points export the same `AtlasProvider`. The main entry point (`@useatlas/react`) also exports the pre-built `AtlasChat` component. The hooks entry point (`@useatlas/react/hooks`) exports individual hooks for building custom UIs with a smaller bundle.
