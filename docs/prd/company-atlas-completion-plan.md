...
- **Status lives in GitHub and — since 2026-08-24 — on the [Finish Conditions board](https://github.com/orgs/AtlasDevHQ/projects/3), which is keyed by *condition*.** Every row below points at an issue number or is marked **UNFILED**. When a lane's issues close, the lane closes; nothing here needs editing to stay true. This document is keyed by neither: it is keyed by **lane**, which is why it needs a reconciliation pass when the others move and it does not.
...
⭐ **RESOLVED 2026-08-30 — Layer 2 shipped standalone, by maintainer decision at [#5469](https://github.com/AtlasDevHQ/atlas/issues/5469).** With one candidate carrier left and the expiry biting harder, the maintainer reversed ADR-0038's "never as a standalone rename PR" clause rather than wait: the tool is `searchAtlas`, the wire tiers are `attested` / `on-record` (`document` unchanged), and ADR-0038 carries a dated amendment recording the reversal, the chosen names, and the corrected blast radius (the published `@useatlas/react` widget binds the enum, not only MCP clients). The expiry is defused; nothing about Lane D remains open.
