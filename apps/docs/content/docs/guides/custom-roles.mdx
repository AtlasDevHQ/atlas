...
The `admin:*` flags **refine** the admin gate rather than replace it: those routes also require an org role of `admin`, `owner` or `platform_admin`, so a flag alone does not open them. `query*` and `dashboards:*` are enforced on their own, which is what lets a non-admin role such as `analyst` carry them.
