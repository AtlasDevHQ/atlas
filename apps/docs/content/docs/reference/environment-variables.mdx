...
| Variable | Default | Description |
|----------|---------|-------------|
| `ATLAS_ORG_ID` | — | Organization ID for org-scoped semantic layer selection. When set, the CLI reads/writes to `semantic/.orgs/{orgId}/` instead of `semantic/`. Can also be passed via `--org` flag |
...
For additional configuration (e.g. `maxEntitiesPerQuery`), use [`atlas.config.ts`](/reference/config#semantic-index).
