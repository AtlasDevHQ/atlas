...
Buttons are rendered as platform-native interactive components (Block Kit buttons on Slack, Adaptive Card actions on Teams, Discord buttons). On platforms that don't support buttons (GitHub, Linear, WhatsApp), the card includes a text fallback with instructions:
...
| Action buttons | Yes | Yes | Yes | Yes | Partial |
| Modals | Yes | No | No | No | No |
| Dropdowns | Yes | No | Yes | No | No |
