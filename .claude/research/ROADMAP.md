...
- [ ] `MutationErrorSurface` phase 2 carve-outs (#1624) — SCIM row-pin (#1675), billing PlanShell combinedError (#1676). Parent #1624 closes when these ship
- [ ] Type-design hardening for `<MutationErrorSurface>` — branded `FeatureName` + empty-message invariant (#1652)
- [ ] Abuse cluster type-design hardening — brand/discriminated-union for `AbuseInstance` (#1684), `Percentage`/`Ratio` branded numerics (#1685)
- [ ] Bugs filed during agent review — `AbuseDetail` diagnostic channel (#1682), `getAbuseEvents` JSON.parse catch radius (#1683), `PLAN_MRR` stale keys MRR = 0 (#1680), `backups.status` DB CHECK (#1679)
...
- [ ] Tech debt: module-scoped warn-set reset between tests (#1674)
