...
| `ATLAS_MCP_MAX_SESSIONS` | `100` | Max concurrent hosted-MCP sessions per API instance. Counts both registered sessions and in-flight initializations. Hitting the cap returns `503 too_many_sessions`. Sessions are in-process; behind a multi-replica load balancer, sticky routing is required so frames stay on the same instance |
