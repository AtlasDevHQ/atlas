...
- **Active users** — unique users who triggered events this period
...
If usage data cannot be read (e.g., metering database is temporarily unavailable), requests are **allowed** with a warning that usage tracking may be inaccurate. Enforcement is best-effort — Atlas prefers availability over strict enforcement.
...
- Usage events are fire-and-forget with a circuit breaker. After 5 consecutive write failures, events are dropped until the database recovers
