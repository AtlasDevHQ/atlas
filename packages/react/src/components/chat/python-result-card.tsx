...
/**
 * Progress event from the server's streaming Python execution — the UI-prop
 * shape the host feeds this card. It MIRRORS the API wire event
 * `SidecarProgressEvent` (`packages/api/src/lib/sidecar-types.ts`) but is
 * field-renamed (`content`/`chart` here vs the wire's `data`); the host owns
 * the wire→prop mapping. Kept local (not `@useatlas/types`) because it is a
 * render-prop contract, not a wire type — but the two unions are parallel, so
 * a new wire variant must be reflected here by hand.
 */
...
export interface PythonResultCardProps {
  part: unknown;
  /**
   * Streaming progress from the server's Python execution — hosts that bridge
   * the progress stream (e.g. @atlas/web's chat page) pass the events for the
   * card's invocation; omitted, the card shows a plain loading state until the
   * tool completes.
   */
  progressEvents?: PythonProgressData[];
  /**
...
export function PythonResultCard(props: PythonResultCardProps) {
  return (
    <ResultCardErrorBoundary label="Python">
      <PythonResultCardInner {...props} />
    </ResultCardErrorBoundary>
  );
}

function PythonResultCardInner({ part, progressEvents, dark }: PythonResultCardProps) {
      ...
      <div className="my-2 overflow-hidden rounded-lg border border-emerald-200 bg-emerald-50/50 dark:border-emerald-800/50 dark:bg-emerald-950/20">
        <div className="flex items-center gap-2 px-3 py-2 text-xs">
          ...
          <span className="flex-1 truncate text-zinc-500 dark:text-zinc-400">
            {String((args.explanation ?? "") as string | number | boolean)}
          </span>
        </div>
    ...
    return (
      <div className="my-2 overflow-hidden rounded-lg border border-red-300 bg-red-50 dark:border-red-900/50 dark:bg-red-950/20">
        ...
        <pre className="border-t border-red-200 px-3 py-2 text-xs whitespace-pre-wrap text-red-600 dark:border-red-900/50 dark:text-red-300">
          {String((result.error ?? "Unknown error") as string | number | boolean)}
        </pre>
        {!!result.output && (
          <pre className="border-t border-red-200 px-3 py-2 text-xs whitespace-pre-wrap text-red-500 dark:border-red-900/50 dark:text-red-400">
            {String(result.output as string | number | boolean)}
          </pre>
        )}
      </div>
    );
  ...
  const output = result.output ? String(result.output as string | number | boolean) : null;
  ...
  return (
    <ResultCardBase
      ...
      title={String((args.explanation ?? "Python result") as string | number | boolean)}
    ...
    >
      ...
      {hasRechartsCharts &&
        rechartsCharts.map((chart, i) => (
          <RechartsChartSection key={i} chart={chart} dark={isDark} />
        ))}
...
function ChartImage({ chart, index }: { chart: PythonChart; index: number }) {
  ...
  return (
    // oxlint-disable-next-line @next/next/no-img-element -- base64 data URL; @useatlas/react is framework-agnostic, no next/image
    <img
...
function RechartsChartSection({ chart, dark }: { chart: RechartsChartConfig; dark: boolean }) {
  ...
  const rows: string[][] = chart.data.map((row) =>
    headers.map((key) => (row[key] == null ? "" : String(row[key] as string | number | boolean))),
  );
  ...
  const detectionResult: ChartDetectionResult = {
    ...
    data: chart.data.map((row) => {
      ...
      for (const key of headers) {
        const val = row[key];
        out[key] = typeof val === "number" ? val : String((val ?? "") as string | number | boolean);
      }
      return out;
    }),
  };
