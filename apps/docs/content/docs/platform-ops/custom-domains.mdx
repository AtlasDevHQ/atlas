...
description: Configure custom domains for Business-plan workspaces so users access Atlas at your own URL.
---
...
Workspaces below the Business plan see an upgrade prompt instead of the domain form. Self-hosted deployments (which default to the "free" tier) can use custom domains without plan restrictions.
