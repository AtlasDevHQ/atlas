...
/**
 * Admin Action Log rendered as a tab inside `/admin/audit`. The previous
 * standalone `/admin/action-log` route was folded in to collapse two audit
 * surfaces (queries vs. admin mutations) under one nav entry. The old URL
 * still resolves via a redirect on `action-log/page.tsx`.
 *
 * Pure body component — no `<h1>` and no outer `<div className="p-6">`.
 * The host audit page provides chrome and tab framing.
 */
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Admin mutations performed in this workspace. {total > 0 && `${total} total entries.`}
        </p>
        <Button
          variant="outline"
          size="sm"
