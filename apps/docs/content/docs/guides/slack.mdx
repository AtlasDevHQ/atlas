...
Direct users to `https://your-api-host/api/v1/slack/install`. This redirects to Slack's OAuth authorize page. After approval, the callback persists the bot token under `chat_cache:slack:installation:<teamId>` — the single store shared with the `@chat-adapter/slack` multi-workspace lookup path.
