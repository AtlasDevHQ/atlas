...
- **`ops smoke-crm`** verifies the demo→Twenty lead-capture pipeline end to end. Run ad-hoc by an operator and as the post-deploy Staging Smoke gate (.github/workflows/staging-smoke.yml (deleted)) — not per-PR CI. Its optional `--wipe-twenty` phase clears the Twenty workspace and is double-gated by `ATLAS_SMOKE_WIPE_OK=1`.
