...
- [Operator Debugging](#operator-debugging) — `atlas doctor`, startup errors, connection issues, sandbox problems, debug logs
...
<Callout type="warn" title="Self-Hosted Operators">
  The sections below are for operators managing their own Atlas deployment. On [app.useatlas.dev](https://app.useatlas.dev), infrastructure diagnostics are handled by the Atlas platform team. If you encounter persistent issues on the hosted platform, contact support.
</Callout>
...
For user-facing descriptions, see [User Troubleshooting](#sql-validation-errors) above. The operator-specific details below cover remediation steps that require config access.
