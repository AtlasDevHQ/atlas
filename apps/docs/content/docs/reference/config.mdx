...
When set to `"auto"` (or omitted), Atlas detects the mode from environment variables. See [Authentication](/deployment/authentication) for details.
...
```typescript
import clickhouse from "@useatlas/clickhouse";
import snowflake from "@useatlas/snowflake";
