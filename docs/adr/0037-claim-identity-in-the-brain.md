...
**The consequence to state plainly, because it reads as a regression:** with `passthroughEntityResolver` shipped as the default, an entity-valued object (`Ada / reports to / Grace` vs `Alan`) has **no** comparable value on either side and **never supersedes**. Only parseable values do, plus resolved entity ids — for which [#5031](https://github.com/AtlasDevHQ/atlas/issues/5031) built the batched seam, and which arrive the day a workspace injects a real store rather than the day that issue merged. That is the abstain band working, not a gap — the pair still carries its advisory tension edge and a human still arbitrates at the review gate.
