...
mutate "$DOC" "$DOC_BACKUP" 's/All seventeen are hidden/All fourteen are hidden/'
check fail "a stale hidden-count is caught" \
  'but the table lists'
restore_doc

mutate "$DOC" "$DOC_BACKUP" 's/All seventeen are hidden from the generic settings page/All of them are concealed from the generic settings page/'
...
mutate "$DOC" "$DOC_BACKUP" 's/All seventeen are hidden from the generic settings page/All keys are hidden from the generic settings page/'
...
mutate "$DOC" "$DOC_BACKUP" 's/Six are \*\*workspace-scoped\*\*/Six of them are **workspace-scoped**/'
...
mutate "$DOC" "$DOC_BACKUP" 's/ Six are \*\*workspace-scoped\*\* — meaning each can hold a different value per workspace, set by a platform admin: `ATLAS_BRAIN_AUDIENCE_SYNC_ENABLED`, `ATLAS_BRAIN_COVERAGE_SNAPSHOT_ENABLED`, `ATLAS_BRAIN_WAREHOUSE_CADENCE_ENABLED`, `ATLAS_BRAIN_EXTRACTION_MODEL` and the two alias auto-approval keys.//'
...
mutate "$DOC" "$DOC_BACKUP" 's/Six are \*\*workspace-scoped\*\*/Five are **workspace-scoped**/'
...
# still verified — `6` is the true number, so this must pass for the right
# reason, which the next fixture pins by making the digit WRONG.
mutate "$DOC" "$DOC_BACKUP" 's/Six are \*\*workspace-scoped\*\*/6 are **workspace-scoped**/'
check pass "W-DIGITS: a digit count is read, not silently skipped"
restore_doc

mutate "$DOC" "$DOC_BACKUP" 's/Six are \*\*workspace-scoped\*\*/7 are **workspace-scoped**/'
