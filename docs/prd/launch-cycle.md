...
**The discipline**, inherited from the destination and scoped to the sentence: it names no table, tool, column or vendor. The rest of this page is an operating page, not a destination, so it names issues, files and queries on purpose — a gate that does not name its query is a gate that cannot be read.
...
- **They read Hacker News**, which is the channel the launch day is built around (#5612); the MCP directories, r/dataengineering and X follow the same day, and Product Hunt a week later
...
The demo is the sentence's three commitments, shown: beat 5 is commitment 1 (it does not guess), beat 4 is commitment 2 (it does not decide who is right), beat 3 is commitment 3 (nothing is authoritative without a person's name on it). No admin console, no plugin list, no six chat platforms. The corpus also holds a warehouse-overlapping claim — a misremembered December 2024 GMV that the live rows outrank — which is available to the conversations and the docs but is not in the sixty seconds; five beats are enough, and Surveyed-outranks-Attested is the second screen's argument, not the first.
...
**Read two weeks after the Show HN posts.** The read timestamp is the post's timestamp (recorded in the day's log below, #5612) plus exactly fourteen days, to the minute, and that one timestamp is the upper bound of every windowed measure below. Read then, not when convenient; a gate read late is a gate read with hindsight.

Each measure names the exact source that produces its number. The person reading it runs the query and writes the result.
...
| **Anonymous MCP demo sessions** | 100+ | under 30 | Against the `us` region's internal DB (the only serving region — `eu` and `apac` are parked): `SELECT count(*) FROM demo_anonymous_sessions WHERE created_at >= '<post timestamp>' AND created_at < '<read timestamp>';` — **the table does not exist today; it lands with #5604**, which adopts this table name and a `created_at` column so this row is a query and not a guess. The day's log (#5612) reads referrers from the same table so it can say where the sessions came from |
| **Real conversations held** | 10+ | under 3 | The maintainer's Microsoft 365 (Outlook) calendar, on a dedicated calendar named **`Atlas launch conversations`** — one event per conversation, titled with the person's name and company, in the window from the post timestamp to the read date. The count is the number of events in that window. Each is mirrored in the log below with a dated verbatim sentence; the calendar is the count, the log is the record |
| **Design partners committed** | 2+ | 0 | A countersigned design-partner agreement, filed in the private repository **`AtlasDevHQ/design-partners`** — which does not exist today and is created on the first signing — one directory per partner named for the company, holding the signed PDF and a `README.md` stating the next step and its date. The count is the number of directories holding a countersigned agreement. A verbal yes, an email, or an intent to sign is zero |
...
Append-only, dated. Nothing above this line changes after sign-off except by a dated amendment here. This section is the named source #5611, #5612 and #5613 write to; the calendar and the repository are the counts, this is the record.
...
### Prepared answers (#5612)

*Not yet written.* One answer each, written before the day, for the five things that will come up: AGPL plus a commercial `ee/`; "isn't this just RAG over Slack"; "why not Glean"; the human gate as friction; the name colliding with Ariga's Atlas. Plus the objection the conversations raised most, which the launch post answers first.
