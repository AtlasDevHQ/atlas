...
No authentication is required to view a shared conversation. The public endpoint has its own rate limiter to prevent abuse.
...
- **Rate limited** — the public endpoint has a per-IP rate limiter (separate from the authenticated rate limiter) to prevent scraping.
