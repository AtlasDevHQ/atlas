...
The `adapters:` config field was removed in [#2650](https://github.com/AtlasDevHQ/atlas/pull/2650). Adapter activation is driven by `catalog` rows + per-Platform env vars; `chatPlugin()` itself takes no credentials. Google Chat's service-account install handler landed in [#2754](https://github.com/AtlasDevHQ/atlas/pull/2754) — the catalog row carries `install_model: "static-bot"`, and the service-account JSON + Pub/Sub topic are read from env vars. See the [chat plugin docs](/plugins/interactions/chat#configuration) for the canonical shape.
</Callout>
...
Both required vars must be present for the adapter to activate; there is no Application Default Credentials (ADC) path through Atlas's install flow — activation requires the inline service-account JSON.
