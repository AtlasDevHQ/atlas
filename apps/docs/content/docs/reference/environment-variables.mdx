...
| `TURNSTILE_SECRET_KEY` | — | Cloudflare Turnstile secret for siteverify on `/api/v1/contact` and the `start_trial` MCP onboarding bootstrap. When unset, `verifyTurnstile()` fails closed (no submissions / trials accepted). **Boot-required on SaaS** (#3795): `TurnstileGuardLive` fails boot when this is unset in `ATLAS_DEPLOY_MODE=saas`, so a missing secret surfaces as a loud boot failure rather than a silent 100% signup outage. Self-hosted leaves it opt-in |
