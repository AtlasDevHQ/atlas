...
<Callout type="warn">
**`@useatlas/teams` is deprecated.** Use [`@useatlas/chat`](/plugins/interactions/chat) with the Teams adapter instead.
The Chat SDK bridge plugin provides the same Teams functionality plus multi-platform support (Slack, Discord, etc.) and built-in state management. See the [migration guide](#migrating-to-useatlas-chat) below.
</Callout>
