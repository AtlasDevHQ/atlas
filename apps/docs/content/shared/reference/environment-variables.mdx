...
Ingest, extraction, and access-membership knobs for [company-brain sources](/guides/brain-sources). Like the Knowledge Base knobs above these are **settings-registry keys** — a **platform admin** can change them in Admin → Settings without a redeploy; the env var form below is honored on self-hosted deployments. Precedence is `workspace > platform > env > default`.

All nine are hidden from the generic settings page on Atlas Cloud, so a workspace admin cannot read or write any of them. `ATLAS_BRAIN_AUDIENCE_SYNC_ENABLED` is the one key that is **workspace-scoped** — meaning it can hold a different value per workspace, set by a platform admin — while the other eight are one value for the whole deployment.
