...
describe("migrateAuthTables", () => {
  it("runs internal DB migration when DATABASE_URL is set", async () => {
    ...
    // migrateInternalDB: 45 original + 14 org_id columns + 4 semantic_entities + 3 learned_patterns + 4 prompt DDL + 7 seed + 5 query_suggestions = 82 queries
    // + 1 loadSavedConnections SELECT + 1 loadPluginSettings SELECT = 84 total
    expect(queries.length).toBe(84);
