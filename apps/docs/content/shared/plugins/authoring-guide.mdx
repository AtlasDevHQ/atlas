...
```typescript
import { createFileTransportPythonBackend, installPythonPackages } from "@useatlas/plugin-sdk";
...
createPython(options) {
  return createFileTransportPythonBackend(options, {
    ...
    async createSession() {
      const sandbox = await mySdk.create();
      await installPythonPackages(sessionFor(sandbox), config.pythonPackages, "MyProvider", log);
      return sessionFor(sandbox); // PythonSandboxSession over the provider SDK
    },
  });
},
```
...
- Types: `PluginPythonOptions`, `PluginPythonBackend` (`exec`, optional `execStream` with `PluginPythonProgressEvent`, optional `close`), `PluginPythonResult`, `PluginPythonData`, `PluginPythonChart`, `PluginRechartsChart`, `PythonSandboxSession`, `PythonBackendAdapter`.
