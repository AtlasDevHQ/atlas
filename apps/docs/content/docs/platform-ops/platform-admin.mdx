...
- **The admin action log is anonymized, not deleted.** Its rows include the record of the purge itself, so deleting them would erase the evidence that the erasure happened. The purge instead NULLs `actor_id`, `actor_email` and `ip_address` and stamps `anonymized_at`. The action sequence remains auditable; the identifiers do not survive.
