...
> the prose below is that file's `## Learned query patterns` section verbatim — only the relative links are repathed for the new depth — and it is no longer there.
