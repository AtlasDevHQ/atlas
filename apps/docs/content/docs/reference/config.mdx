...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `coldWindowDays` | `number` | `90` | Window in days applied to `prompt_collections.created_at` when the library tier is populated. Also bounds the approval queue for learned-popular prompts in later slices |
