...
export default defineConfig({
  datasources: {
    default: { url: process.env.ATLAS_DATASOURCE_URL! },  // Primary Postgres
  },
  plugins: [
...
// atlas.config.ts
import { defineConfig } from "@atlas/api/lib/config";
import { e2bPlugin } from "@useatlas/e2b";
import { daytonaPlugin } from "@useatlas/daytona";

export default defineConfig({
  plugins: [
    e2bPlugin({ apiKey: process.env.E2B_API_KEY!, priority: 90 }),
    daytonaPlugin({ endpoint: process.env.DAYTONA_URL!, priority: 80 }),
  ],
});
```
