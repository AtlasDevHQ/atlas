...
describe("createBoundDashboardTools", () => {
  ...
  // draft-write happy paths (with a userId) live in the "bound-dashboard tools
  // — drafts" section at the foot of this file; the draft-path invalidation +
  // text-card cases below use
...
// ===========================================================================
// Formerly bound-dashboard-drafts.test.ts — drafts integration (#2364).
//
// Drafts are unconditional (#4324): the first mutation forks a draft from
// published, subsequent mutations stay on the draft, two users editing the same
// dashboard each get their own draft row, and the same user in two tabs
...
// dashboard_user_drafts table, without standing up a real Postgres. It always
// shared this file's mock-pool shape and helpers; the fixtures it needs at a
// different timestamp precision keep their own `drafts*` names.
// ===========================================================================
const draftsDashboardRow = {
  id: "dash-1",
  org_id: "org-1",
  owner_id: "user-1",
  title: "Demo",
...
describe("bound-dashboard tools — drafts", () => {
  ...
  describe("drafts path", () => {
    ...
    it("updateDashboardMeta with userId routes through dashboard_user_drafts", async () => {
      enableInternalDB();
      setResults(
        { rows: [draftsDashboardRow] }, // getDashboard
        { rows: [draftsCardRow] },
        { rows: [] }, // loadDraft empty
    ...
    it("two addCard calls in the same user reuse the same draft row (no extra inserts)", async () => {
      ...
      const existingRow = {
        user_id: "user-1",
        dashboard_id: "dash-1",
        draft: draftsBaselineSnap,
        baseline: draftsBaselineSnap,
        published_baseline_at: "2026-05-17T00:00:00.000Z",
        created_at: "2026-05-17T00:00:00.000Z",
        updated_at: "2026-05-17T00:00:00.000Z",
      };
