...
- **Right panel** — Detail view with a **Pretty** / **YAML** / **History** toggle
...
- **Automatic type detection** — When you enter a column name in a dimension's SQL field and tab out, the type dropdown auto-selects the matching type (e.g., `varchar` → `string`, `timestamp` → `timestamp`, `integer` → `number`)
...
| `POST` | `/api/v1/admin/semantic/entities/:name/rollback` | Rollback to a previous version |
