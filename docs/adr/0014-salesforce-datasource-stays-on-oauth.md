...
- **The inert `salesforcePlugin({})` registration is removed** from `deploy/api/atlas.config.ts` and `deploy/api-staging/atlas.config.ts` (import + array entry), with a comment at both the import block and the skip site explaining why, pointing here. _(The separate `deploy/api-staging/atlas.config.ts` was later retired in [#3958](https://github.com/AtlasDevHQ/atlas/issues/3958) — api-staging now runs the shared `deploy/api/atlas.config.ts`.)_
