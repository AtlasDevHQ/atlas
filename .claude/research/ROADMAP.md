...
All six slices have landed, so v0.0.2 maps 1:1 to PRD #2868. Two `ready-for-human` follow-ups surfaced during slice 6 gate the tag: [#3034](https://github.com/AtlasDevHQ/atlas/issues/3034) (security — `probeSpec` leaks the customer credential to third-party spec hosts) and [#3035](https://github.com/AtlasDevHQ/atlas/issues/3035) (data-candidate installs can't express read-only POST, so Notion search is unreachable on a default install). The net-new spec-lifecycle subsystem (refresh interval, auto re-discovery, drift diff, breaking-change signal, shared cache) was split into [`v0.0.3 — Spec Lifecycle`](https://github.com/AtlasDevHQ/atlas/milestone/58) per [#3013](https://github.com/AtlasDevHQ/atlas/issues/3013) to keep this milestone inside its PRD contract.
