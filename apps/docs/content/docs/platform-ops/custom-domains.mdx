...
description: Configure custom domains for Pro and Business plan workspaces so users access Atlas at your own URL.
---
...
- Active Pro or Business plan on [app.useatlas.dev](https://app.useatlas.dev) (or self-hosted "free" tier)
...
Workspace admins on Pro or Business plans can manage their own custom domain at **Admin Console → Configuration → Custom Domain**. One domain per workspace (MVP).
...
Workspaces on the trial or starter plan see an upgrade prompt instead of the domain form. Self-hosted deployments (which default to the "free" tier) can use custom domains without plan restrictions.
...
Require `admin` role and active organization. Pro or Business plan required for adding a domain (`POST /api/v1/admin/domain`).
