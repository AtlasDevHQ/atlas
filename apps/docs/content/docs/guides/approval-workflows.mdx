...
Approvals are scoped to the connection's **environment group** (called a "connection group" in the schema; see the [multi-environment guide](/semantic-layer/multi-env)). When an admin approves a query, the approval covers any member of that group running the same query — operators don't have to re-approve once for each replica of a federated dataset.
