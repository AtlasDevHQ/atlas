...
export async function migrateInternalDB(): Promise<void> {
  ...
  await pool.query(`ALTER TABLE conversations ADD COLUMN IF NOT EXISTS share_token VARCHAR(64) UNIQUE, ADD COLUMN IF NOT EXISTS share_expires_at TIMESTAMPTZ;`);
