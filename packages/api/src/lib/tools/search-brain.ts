...
- \`tensions\` is the fact's conflict cluster, listed in both directions and deliberately unranked — each visible counterpart carries its own claim and provenance, so surface both sides with their evidence and never pick a winner; recency and corroboration are context for the reader, not a verdict. A counterpart with a non-null \`invalidatedAt\` is a rival that has since been RETRACTED — it is listed because it is why the claim was contested, so report it as a withdrawn rival, never as a live contradiction. A \`{ "visible": false, "withheldCount": N }\` entry means N conflicting claims exist that you cannot see — treat the claim as contested, never as settled
