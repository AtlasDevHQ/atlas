...
When using [multiple datasources](/docs/reference/config), each source can have its own `entities/` subdirectory under `semantic/{source}/`. Entities can also specify a `connection` field to indicate which datasource they belong to.
...
The semantic layer also powers the [table whitelist](/docs/security/sql-validation) -- queries can only reference tables that have an entity YAML file.
