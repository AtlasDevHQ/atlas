...
- [x] **1.4.4 — Multi-environment semantic layer** (#45, 11 + 17 audit + #2429 + 2026-05-16 dogfood follow-on) — connection groups foundation + admin merge-into-group wizard + Phase 4 archive cascade, group-scoped semantic / dashboards / scheduled tasks / approvals / PII, group-aware chat routing + per-turn override, application-layer FK gate for cross-org `connection_group_id` writes (#2424), legacy `connection_id` dropped (#2346 + 0069), `@useatlas/types` → 0.1.x. PRD #2336, audit #2407, mop-up #2442. Path-A real route-level e2e deferred to #2441. **Dogfood follow-on**: chat / notebook empty-state primitives (#2451 / #2452 / #2457), admin data-layer IA reshape PRD #2458 (4/5 shipped via #2471 / #2472 / #2475 / #2479), SaaS plan + trial onboarding PRD #2464 (all 4 slices shipped — PRD closed), `/admin/connections` cache-poisoning root cause (#2444 / PR #2478). **Final slice #2463 (retire `/admin/schema-diff`) + manual verification #2443 gating GH milestone close.**
