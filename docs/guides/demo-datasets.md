...
| Tables | 3 | 62 | 52 |
| Rows | ~330 | ~500K | ~480K |
| Database | Postgres | Postgres only | Postgres only |
...
Requires PostgreSQL (uses `GENERATE_SERIES` for data generation).
...
See [cybersec-seed-design.md](./cybersec-seed-design.md) for the full table index, data volumes, and design rationale.
...
The e-commerce dataset includes the same four real-world tech debt patterns as the cybersec demo:

#### 1. Missing FK Constraints

Nineteen `*_id` columns reference other tables but lack `FOREIGN KEY` constraints:
...
| `inventory_levels.variant_id` | `product_variants.id` |
| `payments.order_id` | `orders.id` |
| `utm_tracking.customer_id` | `customers.id` |
| `page_views.customer_id` | `customers.id` |
| `cart_events.customer_id` | `customers.id` |
| `review_helpfulness.review_id` | `product_reviews.id` |
| `review_helpfulness.customer_id` | `customers.id` |
...
| `gift_card_transactions.order_id` | `orders.id` |
| `shipments.carrier_id` | `shipping_carriers.id` |
| `search_queries.customer_id` | `customers.id` |
| `search_queries.clicked_product_id` | `products.id` |

Additionally, ~1.5% of payments reference nonexistent orders (orphaned from deleted test orders).

#### 2. Abandoned Tables

Four tables match legacy/temp naming patterns and have no inbound foreign keys:

- `old_orders_v1` — pre-migration 2020 order data with different column names
- `temp_product_import_2023` — one-time CSV import artifact
- `legacy_analytics_events` — old event tracking, replaced in 2024
- `payment_methods_backup` — backup from Stripe-to-Adyen migration

#### 3. Inconsistent Enums

Several text columns have case-inconsistent values:

- `customers.acquisition_source`: 'Google', 'google', 'GOOGLE', 'Organic', 'organic'
- `loyalty_accounts.tier`: 'Gold', 'gold', 'GOLD', 'Silver', 'silver'
- `returns.reason`: 'Defective', 'defective', 'DEFECTIVE', 'Wrong Item', 'wrong_item'

#### 4. Denormalized Tables

Five reporting/cache tables duplicate data from other tables:

- `orders_denormalized` — pre-joined orders + customer + product info
- `daily_sales_summary` — daily rollup
- `monthly_revenue_summary` — monthly aggregates
- `product_performance_cache` — pre-computed product metrics
- `customer_ltv_cache` — lifetime value estimates

Additionally, `orders.customer_email` and `customer_segment_assignments.segment_name` are denormalized columns.

### Schema Evolution Artifacts

The dataset includes ten schema evolution instances where old and new columns coexist (five shown below; see [ecommerce-seed-design.md](./ecommerce-seed-design.md) for the full list):
...
See [ecommerce-seed-design.md](./ecommerce-seed-design.md) for the full table index, data volumes, and design rationale.
