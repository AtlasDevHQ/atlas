...
- **every position uses the `subject` columns** — Both the read column and the key column, since the destructure supplies both. The closure subquery still interpolates `position`, so this isolates the COLUMN pair rather than the whole statement.
