...
**Solution:** New `packages/schemas/src/residency.ts` holds the four primary entity schemas (`RegionPickerItemSchema`, `RegionStatusSchema`, `WorkspaceRegionSchema`, `RegionMigrationSchema`) plus the three composite response shapes (`RegionsResponseSchema`, `AssignmentsResponseSchema`, `MigrationStatusResponseSchema`). Every schema uses `satisfies z.ZodType<T>` against the `@useatlas/types` interface, so a field rename breaks the file at compile time. `status` pins to `MIGRATION_STATUSES` from `@useatlas/types` — a new migration status now propagates to both the route OpenAPI contract and the web parse without a second edit. The route file (`admin-residency.ts`) drops its two local schemas and imports `RegionMigrationSchema` / `MigrationStatusResponseSchema` from `@useatlas/schemas`. The web file (`admin-schemas.ts`) drops all seven local schemas and re-exports them from `@useatlas/schemas` at the top, matching the phase-3 pattern established in commit `ad78cfe0`.

**Impact:**
- **17 new parse-boundary tests** across the residency module: happy-path parses for each entity, round-trip (parse → serialize → parse) preservation, enum strict rejection of unknown statuses, `all MIGRATION_STATUSES values parse` exhaustive coverage, canonical-tuple literal assertion (fails loud if someone reorders `MIGRATION_STATUSES`), structural rejection of missing keys, composite response shape parses. Schemas package test count: 113 → 130.
...
- **Seven `as z.ZodType<T>` casts replaced with `satisfies z.ZodType<T>`** — a field rename in `@useatlas/types` now breaks `residency.ts` at compile time instead of silently drifting.
