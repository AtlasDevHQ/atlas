...
Run the region's migrations, then clear the residue with the orphaned-workspace sweep above. **Do not try to purge again** — the organization record is already gone, so the endpoint answers `404`.
