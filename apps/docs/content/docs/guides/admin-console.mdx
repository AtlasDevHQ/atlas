...
**Cross-connection diff.** When the workspace has multiple connection groups (1.4.4), the drift drawer is scoped to one group at a time. Pick the target group from the drawer header — the schema diff is computed against that group's primary member. To audit drift across all groups, run `atlas diff` from the CLI; the admin drawer is optimised for the day-to-day "the semantic layer caught up to last sprint's migration" workflow rather than a cross-environment audit.

**API.** The drift-detection API (`GET /api/v1/admin/semantic/diff?connection=<id>&group=<id>`) returns structured JSON with `newTables`, `removedTables`, `tableDiffs`, `unchangedCount`, and a `summary` object. See [Schema Evolution](/deployment/schema-evolution) for the underlying drift contract.

<Callout type="info">
**If you're looking for `/admin/schema-diff`** — the page was retired in 1.4.4. Drift is now inline on `/admin/semantic` per the walkthrough above. The `GET /api/v1/admin/schema-diff` endpoint still exists for back-compat but the UI route redirects to `/admin/semantic`.
</Callout>
