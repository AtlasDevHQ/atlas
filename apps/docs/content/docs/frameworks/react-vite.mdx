...
- **`state`** -- check for `"output-available"` to know when the result is ready. Before that, show a loading indicator.

See the [Data Stream Protocol](/docs/frameworks/overview#data-stream-protocol) section in the overview for the full part structure.
