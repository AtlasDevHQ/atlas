...
| **Enterprise features** | SSO, SCIM, custom roles, IP allowlists, approval workflows, audit retention | None |
| **Backend architecture** | Effect.ts for key subsystems (structured concurrency, typed errors, circuit breaking) | Standard Python |
...
Atlas has a typed Plugin SDK with 20+ official plugins covering datasource adapters, sandbox backends, interaction channels (Slack, Teams, Discord, Telegram, Google Chat, GitHub, Linear, WhatsApp via Chat SDK, and MCP), and action triggers (email, JIRA, webhooks). You can build custom plugins with `bun create @useatlas/plugin`.
...
Atlas ships with enterprise features out of the box: SSO (SAML/OIDC) and SCIM provisioning, custom role definitions, IP allowlists, approval workflows for sensitive operations, configurable audit log retention with CSV export, and data residency controls. The backend uses Effect.ts for structured concurrency, typed errors, graceful shutdown, and circuit breaking.
