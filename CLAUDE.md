...
> ⚠️ **The hosted SaaS and staging were shut down on 2026-09-17 ([ADR-0047](docs/adr/0047-hosted-saas-shut-down.md)).** Every Railway service, database and bucket is deleted, and nothing under `useatlas.dev` serves. There are no deploy targets, so guidance in this file, `.claude/rules/**` and `docs/development/**` about prod, staging, regions, parked regions, `railway ssh` or releasing to prod is **historical**, not procedure. Never write a present-tense claim about a live deployment.
...
`docs/adr/` holds 47 system-wide decisions. **The split is done (#5302)**: every context
