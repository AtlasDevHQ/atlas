...
|---|---|---|---|
| Teams | yes | #2752 / #3142 | ✓ verified — static-bot install via `TeamsStaticBotInstallHandler`; per-Workspace `tenant_id` (Microsoft Entra ID tenant GUID, lowercased) in `workspace_plugins.config`; no per-Workspace credential store (operator-shared `TEAMS_APP_ID` + `TEAMS_APP_PASSWORD`, MultiTenant mode); reachability verified via Microsoft OIDC discovery at install time; `@chat-adapter/teams` builder + `/webhooks/teams` receive route + executeQuery Teams branch (resolves `channelData.tenant.id` → workspace) wired in #3142; legacy `/api/v1/teams/{install,callback}` OAuth dance retired (410 Gone) |
