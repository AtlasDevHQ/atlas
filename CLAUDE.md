...
> ⚠️ **Three regions are BUILT; one is SERVING.** `eu` and `apac` are **parked** — `selectable: false` + `requestable: true`, API services scaled to zero — since #5582 (2026-09-01), because two idle always-on processes were 43% of the bill. Procedure both directions, and the seven steps un-parking actually takes: [docs/development/parked-regions.md](docs/development/parked-regions.md). **So never write "all three prod regions" in the present tense** — a prod verification claiming three was run against one. And note the region DBs stay online holding their data, so `atlas-operator --region eu|apac` still answers: **the DB replying is not the region being live.**
...
`.claude/commands/` was eight runbooks that were purely **operational** (`release`,
`publish`, `deploy`, `ci`, `verify-mcp-cli`, `verify-prod-signup`, `dev`, `deps-update`) —
sequences against external systems that fail loudly, which no finding implicated.
`/reset`, `/sitrep` and `/tidy` were restored on 2026-08-19 under the same test, bringing
