...
**Actions not appearing** -- Verify `ATLAS_ACTIONS_ENABLED=true`, authentication is configured, and `DATABASE_URL` is set.

**Approval stuck** -- Check that the approving user has the required role. In `admin-only` mode, the requester cannot self-approve.

See [Troubleshooting](/docs/guides/troubleshooting#action-framework) for more diagnostic steps.
