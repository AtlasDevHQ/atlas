...
- Actions require an internal database (`DATABASE_URL`) for the action log
...
- In `admin-only` mode, the requesting user cannot approve their own action (separation of duties)
...
- If `ATLAS_AUTH_MODE=none` with RLS enabled, all queries are blocked (no user context to resolve claims). This is by design -- see [Row-Level Security](/docs/security/row-level-security#auth-mode-compatibility)
