...
## Platform-Tier Settings

Platform admins can write the **platform tier** of workspace-scoped settings from the console ([#4669](https://github.com/AtlasDevHQ/atlas/issues/4669)): `PUT`/`DELETE /api/v1/admin/settings/{key}?tier=platform` sets or clears a deployment-wide value that every workspace inherits unless it has set its own override (the registry precedence is workspace > platform > env > default). The settings `GET` response exposes `platformValue`/`platformSource` so a workspace admin can see when their effective value is platform-inherited.
...
**Always check `complete`.** When it is `false`, the purge ran but did not finish: `skippedTables` names the work that did not run — relations absent from this region's schema, plus the deletes and writes gated behind them. `complete` is also `false` when the Stripe teardown left a warning.

Read `message` rather than inferring from the names, because the two kinds of skipped work have opposite consequences:

- a skipped **DELETE** reported `0` rows, which reads exactly like "there were none" — that data was **not** deleted and survives;
- a skipped **WRITE** has no count at all. The one that occurs is the [#3468](https://github.com/AtlasDevHQ/atlas/issues/3468) tombstone (`stripe_purged_subscriptions`), and its absence means late Stripe cancellation webhooks can **regrow** `stripe_webhook_events` rows the purge did clear.
