...
### Only a surface that can confirm is offered writes

The confirm flow only works on a chat surface that can actually render the banner and POST the confirmation — today, the Atlas web app. So the write half of `executeRestOperation` is gated on a **fail-closed capability declaration** ([#5498](https://github.com/AtlasDevHQ/atlas/issues/5498)): a chat request is offered writes only when it carries the `x-atlas-write-confirm-ui: 1` header, declaring its surface renders the confirm-before-write card and can finish the flow. On any surface that does not declare it — the embeddable `@useatlas/react` widget, the SDK, Slack and other chat platforms, MCP, scheduled runs — every write is refused up front with `write_confirmation_unavailable` and **nothing is staged**; the agent is told writes are not available in that chat and never offers to confirm one. Reads are untouched.

The two refusals are ordered by who fixes them: `writes_disabled` means the operation isn't allowlisted (an admin fixes the datasource config); `write_confirmation_unavailable` means the operation is allowed but the surface can't confirm it (run it from the Atlas web app, or build a confirm card into your custom surface and send the header). The header is a capability declaration, not a security control — the confirm endpoint's server-side allowlist re-validation and single-use token are what enforce the human-in-the-loop step.
