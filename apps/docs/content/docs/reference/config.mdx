...
export default defineConfig({
  ...
  residency: {
    regions: {
      "us-east": { url: process.env.US_EAST_DB_URL! },
      "eu-west": { url: process.env.EU_WEST_DB_URL! },
    },
...
Each region value is a `RegionConfig` with the same fields as a datasource entry (e.g. `url`, `schema`).
