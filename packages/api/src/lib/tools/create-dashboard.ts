...
KPI / SCORECARD CARDS: a \`kpi\` card is a big-number scorecard — the first thing a reader looks at. Lead a dashboard with 2-3 KPI cards summarizing the top metrics (revenue, active users, conversion), then put the trend charts below them. A KPI card's \`sql\` returns a SINGLE row: \`chartConfig.categoryColumn\` names the label column and \`chartConfig.valueColumns[0]\` the metric column. Add \`chartConfig.kpi\` to control it:
...
Example: { title: "Revenue", sql: "SELECT 'Revenue' AS label, SUM(amount) AS total FROM orders WHERE created_at >= :date_from", chartConfig: { type: "kpi", categoryColumn: "label", valueColumns: ["total"], kpi: { valueFormat: "currency", comparisonSql: "SELECT SUM(amount) AS total FROM orders WHERE created_at >= :prev_from AND created_at < :date_from", comparisonLabel: "vs. prior period" } } }.
