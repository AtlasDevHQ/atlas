...
Ingest, extraction, and access-membership knobs for [company-brain sources](/guides/brain-sources). Like the Knowledge Base knobs above these are **settings-registry keys** — changeable in Admin → Settings without a redeploy, with the env var form honored on self-hosted deployments. Precedence is `workspace > platform > env > default`, so on Atlas Cloud these are set in the admin console rather than by redeploy. All are platform-scoped except `ATLAS_BRAIN_AUDIENCE_SYNC_ENABLED`, which is the one key a workspace can override for itself.
...
| `ATLAS_BRAIN_GRANT_SWEEP_INTERVAL_HOURS` | `24` | How often Atlas scans brain facts and episodes for access grants that name nobody. Such rows are invisible to every reader and to the review queue; the sweep only counts and logs them and never changes anything. Applies at restart; values below 0.05 hours are clamped up and values above ~596 hours clamped down |
