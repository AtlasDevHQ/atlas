...
- **Shards are balanced by measured duration**, not round-robin file index — `--timings=scripts/test-timings.json`. On the committed data that is a 1.96x → 1.00x skew across four shards. The file is advisory: if it goes stale, sharding stays correct and just gets less balanced. Do **not** regenerate it locally — a developer box measures the wrong thing twice over (#5383). It is regenerated on runners by `test-timings-refresh.yml`, which opens a PR from `ci/test-timings-refresh`: monthly on a schedule, by hand (`gh workflow run test-timings-refresh.yml --ref main`), and whenever `ci.yml`'s `shard-balance` job measures the heaviest shard's median past 1.5x the lightest's over the last ten green `main` runs.
...
PRs, `--all` on push to `main`, sharded six ways in CI); it globs the directory,
so a new spec is covered the moment it lands. A CI shard owns ROWS, not specs:
every shard verifies every selected spec, `mutate.ts --shard I/N` measures the
rows at positions ≡ I-1 (mod N) within each and reads the rest back from the
committed table, so the comparison is still whole-file and the long pole is no
longer the heaviest single spec.
