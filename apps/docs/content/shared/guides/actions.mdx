...
Credentials resolve per workspace first (`PUT /api/v1/admin/action-credentials/linear`) and fall back to the two environment variables above only on self-hosted deployments. A workspace row that is missing a required field is an error rather than a partial fall-through to the environment.
