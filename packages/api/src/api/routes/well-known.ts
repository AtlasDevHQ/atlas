 ...
 * canonical SaaS regional `api*.useatlas.dev` hosts, the doc
 * advertises the brand-mirror `mcp*.useatlas.dev/mcp` instead so
 * clients reading discovery bind tokens to the brand audience rather
 * than the regional infra. The issuer keeps accepting BOTH
 * (`resolveOAuthValidAudiences`) so a token bound to the regional
 * host still verifies; this function intentionally stops naming the
 * regional surface so new clients walk forward, not back. An operator
 * already running with `ATLAS_PUBLIC_API_URL=https://mcp.useatlas.dev`
 * sees the brand advertised verbatim — `brandUseatlasHost` is
 * asymmetric and returns null for brand inputs, so the `?? trimmed`
 * fallback emits the brand surface as-is.
 */
function buildResourceUri(req: Request): string {
  ...
  return `${brandUseatlasHost(trimmed) ?? trimmed}/mcp`;
}
