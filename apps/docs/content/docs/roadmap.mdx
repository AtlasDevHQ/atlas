...
- **Admin console** — Connection CRUD with encrypted URLs at rest, action approval with bulk operations, user management (invite flow next), plugin and settings configuration
- **Observability** — Query analytics dashboard, health dashboard with per-component checks, token usage tracking, OpenTelemetry traces
- **Scheduled tasks v2** — Task create/edit forms, cross-task run history with filters, delivery channel management
