---
title: Authentication
description: Configure Atlas authentication — from no auth in development to enterprise SSO in production.
---

import { Callout } from "fumadocs-ui/components/callout";

<Callout type="info" title="Using app.useatlas.dev?">
  On the hosted SaaS platform, authentication is pre-configured. Users sign in with email/password or SSO. API access uses Bearer tokens from your workspace settings. The configuration below (environment variables, `ATLAS_AUTH_MODE`, `BETTER_AUTH_SECRET`) is for **self-hosted operators** deploying their own Atlas instance. If you are a SaaS user looking to integrate with Atlas, see [Client Usage (Simple Key)](#client-usage-simple-key) and [BYOT (Bring Your Own Token)](#byot-bring-your-own-token) for client-side usage.
</Callout>

Atlas supports four authentication modes. Auth is optional — Atlas works with no auth for local development.

| Mode | Best for | How it works |
|------|----------|--------------|
| **None** | Local dev, internal tools | No authentication. All requests are anonymous |
| **Simple Key** | Headless integrations, CLI, SDKs | Single shared secret via `Authorization: Bearer <key>` |
| **Managed** | Multi-user web apps | Email/password with sessions. Built on [Better Auth](https://www.better-auth.com/) |
| **BYOT** | Enterprise SSO (Auth0, Clerk, Okta) | Stateless JWT verification against your identity provider's JWKS endpoint |

## Operator Setup (Self-Hosted)

The following sections cover how operators configure authentication for their Atlas deployment. If you are a SaaS user on app.useatlas.dev, skip to [Roles](#roles) or [Rate Limiting](#rate-limiting).

### Auto-detection

When `ATLAS_AUTH_MODE` is not set, Atlas auto-detects the mode from environment variables:

| Priority | Variable present | Mode selected |
|----------|-----------------|---------------|
| 1 | `ATLAS_AUTH_JWKS_URL` | BYOT |
| 2 | `BETTER_AUTH_SECRET` | Managed |
| 3 | `ATLAS_API_KEY` | Simple Key |
| 4 | None of the above | None |

Set `ATLAS_AUTH_MODE` explicitly to override auto-detection: `none`, `api-key` (alias: `simple-key`), `managed`, or `byot`.

### No Auth

The default for local development. All requests pass through without authentication.

```bash
# No auth variables needed — just start the server
bun run dev
```

No user identity is available. Rate limiting still works (keyed by IP). Audit logs record queries without user attribution.

> Do not use no-auth mode in production unless Atlas is behind a VPN or reverse proxy that handles authentication externally.

### Simple Key

A single shared secret for API access. Best for headless integrations, CLI tools, and automation.

#### Simple key setup

```bash
# .env
ATLAS_API_KEY=your-secret-key-here
ATLAS_API_KEY_ROLE=admin      # Optional: member, admin (default), owner
```

### Managed Auth (Self-Hosted Setup)

Full user management with email/password login, sessions, and role-based access. Built on [Better Auth](https://www.better-auth.com/). Requires an internal Postgres database (`DATABASE_URL`). Supports [social login providers](/self-hosted/guides/social-providers) (Google, GitHub, Microsoft) alongside email/password.

<Callout type="info">
  On [app.useatlas.dev](https://app.useatlas.dev), managed auth is pre-configured. The setup below is for self-hosted operators only.
</Callout>

#### Managed auth setup

```bash
# .env
BETTER_AUTH_SECRET=your-random-secret-at-least-32-characters-long
DATABASE_URL=postgresql://user:pass@host:5432/atlas

# Optional
BETTER_AUTH_URL=https://api.example.com        # Auto-detected on Vercel
BETTER_AUTH_TRUSTED_ORIGINS=https://app.example.com,https://admin.example.com
ATLAS_ADMIN_EMAIL=admin@example.com            # First admin account
ATLAS_CORS_ORIGIN=https://app.example.com      # Required for cross-origin cookies
```

#### Bootstrap admin

On first deployment:
- If `ATLAS_ADMIN_EMAIL` is set, the user matching that email gets `admin` role on signup
- If `ATLAS_ADMIN_EMAIL` is not set, the first user to sign up gets `admin` automatically
- Subsequent signups get the default `member` role (set by the `role` user field's `defaultValue`; the Better Auth admin plugin was removed in #3159)

#### Dev admin seeding

When `ATLAS_ADMIN_EMAIL` is set and no users exist in the database, Atlas automatically creates a dev admin account on startup:

- **Email**: the value of `ATLAS_ADMIN_EMAIL`
- **Password**: `atlas-dev`
- **Role**: `admin`

The account is flagged with `password_change_required` -- the user must change this password after first login. This is intended for initial setup only. If any users already exist, seeding is skipped (idempotent).

#### Password reset

Atlas ships a self-serve password reset flow for managed auth. Users click **Forgot password?** on `/login`, enter their email at `/forgot-password`, click the link in the recovery email, and choose a new password at `/reset-password`. The link expires in one hour and can be used only once. On a successful reset Atlas revokes every other active session for that user — recovery is the wrong moment to keep stale sessions live.

The recovery email is sent through whichever delivery backend you have configured for Atlas — see the [Email integration setup](/self-hosted/guides/integrations#email) for the workspace-level provider, or wire up `ATLAS_EMAIL_PROVIDER` / `RESEND_API_KEY` at the platform level. Resolution order:

1. Per-org override (a workspace admin configured Resend, SendGrid, Postmark, SMTP, or SES under Admin > Email Provider)
2. Platform-level provider (`ATLAS_EMAIL_PROVIDER` + the matching key in your settings store)
3. SMTP webhook bridge (`ATLAS_SMTP_URL`)
4. `RESEND_API_KEY` env var
5. Log-only fallback — the verification token is still issued, but no email leaves the box

The **Forgot password?** affordance on `/login` is shown only when the deployment can deliver auth email (resolution levels 2 through 4). On a self-hosted instance with no provider configured, the link is hidden so users aren't sent to a recovery flow that goes nowhere. The Better Auth password-reset endpoints are always live regardless — `/api/auth/request-password-reset` and `/api/auth/reset-password` work unconditionally so an admin can drive the flow manually if needed.

The endpoints are rate-limited at the Better Auth layer (5 attempts per minute per IP for both `/request-password-reset` and `/reset-password`); these limits are pinned and cannot be loosened by environment variables — relaxing them would re-open a flooding surface.

## User Authentication (All Deployments)

The sections below apply to all Atlas users — both SaaS and self-hosted — covering how to authenticate API requests and understand roles.

### Client usage (Simple Key)

Send the key in the `Authorization` header (preferred) or `X-API-Key` header:

```bash
# Authorization header
curl -H "Authorization: Bearer your-secret-key-here" \
  http://localhost:3001/api/v1/query \
  -d '{"question": "How many users?"}'

# X-API-Key header (alternative)
curl -H "X-API-Key: your-secret-key-here" \
  http://localhost:3001/api/v1/query \
  -d '{"question": "How many users?"}'
```

#### Security details

- Keys are compared using constant-time SHA-256 hash comparison (timing-safe)
- User IDs in logs are derived from a hash of the key — the raw key is never logged
- A single key maps to a single user identity and role

#### RLS with Simple Key

When using [Row-Level Security](/reference/environment-variables#row-level-security) with simple-key auth, provide static claims:

```bash
ATLAS_RLS_CLAIMS='{"tenant_id": "acme-corp"}'
```

### How sessions work (All Deployments)

1. User signs in via `POST /api/auth/sign-in/email`
2. Better Auth creates a session and sets an `atlas.session_token` cookie (the name's prefix is set by `ATLAS_COOKIE_PREFIX`, default `atlas`; production over HTTPS prefixes it with `__Secure-`)
3. Subsequent requests include the cookie automatically (browser) or send `Authorization: Bearer <session-token>` (API clients)
4. Sessions last 7 days. On any request where the session is at least 24 hours old, the expiry is automatically renewed

Users can log out via `POST /api/auth/sign-out` (clears the session cookie). Sessions are also revoked when the session expires (7 days) or if the user is deleted by an admin.

> **Cookie cache:** Better Auth is configured with a **30-second** signed cookie cache (`cookieCache.maxAge`, default 30s, clamped 5–300s via `ATLAS_SESSION_COOKIE_CACHE_MAX_AGE_SEC`). **Role changes and session revocations may take up to that long to take effect.** The cache avoids a database lookup on every request — raising the value trades revocation speed for fewer lookups. See [Session Timeouts](/reference/environment-variables#session-timeouts).

#### Cross-origin deployment (Self-Hosted)

When the API and frontend are on different origins (e.g. `api.example.com` and `app.example.com`):

```bash
ATLAS_CORS_ORIGIN=https://app.example.com
BETTER_AUTH_TRUSTED_ORIGINS=https://app.example.com
BETTER_AUTH_URL=https://api.example.com
```

`ATLAS_CORS_ORIGIN` controls the HTTP `Access-Control-Allow-Origin` header (which origins can make API requests). `BETTER_AUTH_TRUSTED_ORIGINS` controls CSRF protection (which origins can submit auth forms). In most cross-origin setups, set both to the same value.

> **Both variables are required together.** If you set `ATLAS_CORS_ORIGIN` but not `BETTER_AUTH_TRUSTED_ORIGINS`, CORS will pass but CSRF protection will reject auth requests. If you set `BETTER_AUTH_TRUSTED_ORIGINS` but not `ATLAS_CORS_ORIGIN`, the browser will block cross-origin requests entirely. Always set both for cross-origin managed auth.

> **Host-only session cookies:** The session cookie is **host-only** — scoped to the API host that issued it (`api.example.com`), with no parent-domain (`.example.com`) attribute. When the app and API are on different subdomains of the same registrable domain (e.g. `app.example.com` and `api.example.com`), the browser still sends the cookie on credentialed requests because they are *same-site* (cross-origin): the cookie rides `credentials: include` + the CORS allowlist with `SameSite=Lax` — no `SameSite=None` and no shared cookie domain are needed. Host-only scoping is deliberate: a session minted by one API host never transits another (the foundation for per-region data residency). No manual cookie configuration is needed.

### Frontend integration (All Deployments)

The Atlas web UI includes a built-in auth client. For custom frontends, use the Better Auth React client:

```typescript
import { createAuthClient } from "better-auth/react";

const auth = createAuthClient({
  baseURL: "https://api.example.com",
});

// Sign up
await auth.signUp.email({
  email: "user@example.com",
  password: "secure-password",
  name: "User Name",
});

// Sign in
await auth.signIn.email({
  email: "user@example.com",
  password: "secure-password",
});
```

### BYOT (Bring Your Own Token)

Stateless JWT verification against an external identity provider. Best for enterprises with existing SSO (Auth0, Clerk, Okta, Supabase Auth, etc.). BYOT works on both SaaS and self-hosted deployments — on app.useatlas.dev, configure your JWKS endpoint in **Admin &gt; Authentication**.

#### BYOT setup (Self-Hosted)

```bash
# .env
ATLAS_AUTH_JWKS_URL=https://your-idp.com/.well-known/jwks.json
ATLAS_AUTH_ISSUER=https://your-idp.com/                        # Required
ATLAS_AUTH_AUDIENCE=your-atlas-api          # Optional but recommended
ATLAS_AUTH_ROLE_CLAIM=app_metadata.role     # Optional, defaults to "role" then "atlas_role"
```

> `ATLAS_AUTH_ISSUER` is **required** for BYOT mode. Atlas will return a startup error if `ATLAS_AUTH_JWKS_URL` is set without `ATLAS_AUTH_ISSUER`.

If `ATLAS_AUTH_AUDIENCE` is set, audience validation is enforced — JWTs without a matching `aud` claim are rejected. If unset, audience validation is skipped entirely.

### How BYOT works (All Deployments)

1. Client obtains a JWT from your identity provider (Auth0, Clerk, etc.)
2. Client sends `Authorization: Bearer <jwt>` to Atlas
3. Atlas verifies the JWT signature against the JWKS endpoint
4. Atlas validates `iss` (issuer) and `aud` (audience) claims
5. Atlas extracts user ID from `sub` claim and role from the configured claim path

#### Provider examples

**Auth0:**

```bash
ATLAS_AUTH_JWKS_URL=https://your-tenant.auth0.com/.well-known/jwks.json
ATLAS_AUTH_ISSUER=https://your-tenant.auth0.com/
ATLAS_AUTH_AUDIENCE=https://api.example.com
ATLAS_AUTH_ROLE_CLAIM=https://example.com/roles
```

**Clerk:**

```bash
ATLAS_AUTH_JWKS_URL=https://your-app.clerk.accounts.dev/.well-known/jwks.json
ATLAS_AUTH_ISSUER=https://your-app.clerk.accounts.dev
ATLAS_AUTH_ROLE_CLAIM=metadata.role
```

**Supabase:**

```bash
ATLAS_AUTH_JWKS_URL=https://your-project.supabase.co/auth/v1/.well-known/jwks.json
ATLAS_AUTH_ISSUER=https://your-project.supabase.co/auth/v1
ATLAS_AUTH_ROLE_CLAIM=app_metadata.role
```

#### Role claim extraction

The `ATLAS_AUTH_ROLE_CLAIM` supports dot-delimited paths for nested JWT structures:

```bash
# Top-level claim
ATLAS_AUTH_ROLE_CLAIM=role

# Nested claim
ATLAS_AUTH_ROLE_CLAIM=app_metadata.role

# Deeply nested
ATLAS_AUTH_ROLE_CLAIM=https://example.com/claims.atlas.role
```

If not set, Atlas checks `role` then `atlas_role` at the top level. Valid roles: `member`, `admin`, `owner`.

#### RLS with BYOT

The full JWT payload is available as user claims for [Row-Level Security](/reference/environment-variables#row-level-security). This enables tenant isolation based on JWT claims:

```bash
ATLAS_RLS_ENABLED=true
ATLAS_RLS_COLUMN=tenant_id
ATLAS_RLS_CLAIM=org_id          # Extracted from JWT payload
```

> **`ATLAS_AUTH_ROLE_CLAIM` vs `ATLAS_RLS_CLAIM`:** These serve different purposes. `ATLAS_AUTH_ROLE_CLAIM` extracts the user's role (`member`/`admin`/`owner`) for permission checks. `ATLAS_RLS_CLAIM` extracts a value (e.g. a tenant ID) that gets injected as a WHERE clause filter on every query for row-level security.

## Roles

Atlas has three workspace roles with increasing privileges, plus a global `platform_admin` operator role:

| Role | Level | Can query | Can approve actions | Can approve admin actions |
|------|-------|-----------|---------------------|---------------------------|
| `member` | 0 | Yes | No | No |
| `admin` | 1 | Yes | Yes | No |
| `owner` | 2 | Yes | Yes | Yes |

`platform_admin` (level 3) is a global, cross-workspace operator role — it sits above `owner` and is used for platform-operator surfaces, not per-workspace access. "Can approve actions" maps to the `manual` approval mode (minimum role `admin`); "Can approve admin actions" maps to the `admin-only` mode (minimum role `owner`).

Default roles by auth mode:
- **Simple Key**: `admin` (override with `ATLAS_API_KEY_ROLE`)
- **Managed**: `member` for new signups (first signup or `ATLAS_ADMIN_EMAIL` match gets `admin`)
- **BYOT**: Extracted from JWT claim (defaults to `member` if missing)

## Rate Limiting

Rate limiting is per-user, sliding-window (60-second window).

```bash
ATLAS_RATE_LIMIT_RPM=60         # 60 requests per minute per user
ATLAS_TRUST_PROXY=true          # Required behind reverse proxy to get real IPs
```

When rate limited, the API returns HTTP 429 with a `Retry-After` header.

> **Proxy deployments:** Without `ATLAS_TRUST_PROXY=true`, the `X-Forwarded-For` and `X-Real-IP` headers are ignored. Behind a reverse proxy (Railway, Vercel, nginx, etc.), all requests will appear to come from the proxy's IP, causing all users to share a single rate-limit bucket. Always set `ATLAS_TRUST_PROXY=true` when deploying behind a proxy.

> Rate limits apply to API requests, not agent steps. One chat request can trigger up to 25 internal agent steps.

> Rate limit state is held in-memory. It resets when the process restarts and is not shared across horizontally scaled instances. For distributed rate limiting, use an external solution (e.g. a reverse proxy or API gateway).

## Audit Logging

Every SQL query is logged with user attribution, regardless of auth mode.

**Pino logs** (always active): Structured JSON logs with user ID, query, duration, and success/failure. SQL truncated to 500 chars.

**Internal database** (when `DATABASE_URL` is set): Persistent `audit_log` table with full query text (truncated to 2000 chars), user identity, timing, and error details. Auto-migrated on startup.

Sensitive patterns (passwords, connection strings, credentials) are automatically scrubbed from both log destinations before writing.

Set `ATLAS_LOG_LEVEL=debug` to see per-step agent actions in the logs.

---

## Related

- [Multi-Tenancy](/guides/multi-tenancy) — isolate semantic layers, connections, and caches per organization
- [Social Login Providers](/self-hosted/guides/social-providers) — configure Google, GitHub, and Microsoft sign-in
- [Choosing an Integration](/self-hosted/guides/choosing-an-integration) — compare auth modes side-by-side with setup examples
- [Row-Level Security](/self-hosted/security/row-level-security) — automatic data isolation per tenant using JWT claims
- [Actions Framework](/self-hosted/guides/actions) — approval-gated write operations (requires auth)
- [Troubleshooting](/self-hosted/guides/troubleshooting#auth-issues-operator) — common auth errors and fixes
