...
For most people, scaffolding a new Atlas app is the right starting point:

```bash
bun create atlas-agent my-app
cd my-app
# edit .env and set your LLM API key (the scaffold writes a placeholder)
bun run dev
# Open http://localhost:3000
```

The scaffold creates a runnable Atlas project. It defaults to PostgreSQL + Anthropic — pass `--defaults` for non-interactive setup, or follow the prompts to pick MySQL / OpenAI / Bedrock / Ollama / Gateway. Add `--demo` to seed the canonical NovaMart e-commerce dataset (52 tables, ~480K rows) and its semantic layer; otherwise you point it at your own database and generate the semantic layer with `bun run atlas -- init`.

Once `bun run dev` is up, jump to [Ask your first question](#ask-your-first-question) below.

<Callout type="info" title="Need to contribute to Atlas itself?">
  If you want to work on Atlas's codebase (PRs, plugin development on the monorepo's internal packages, debugging the agent loop directly), follow the [monorepo setup](#alternative-clone-the-monorepo) further down instead — that's the path for contributors, not for using Atlas as a product.
</Callout>
...
- **[Bun](https://bun.sh/) v1.3.13** — check with `bun --version`
...
Restart your terminal after installing. Verify with `bun --version` — you need v1.3.13.
</Accordion>
</Accordions>

## Alternative: clone the monorepo

This longer path is for contributing to Atlas itself or working from the canonical monorepo. Most users should follow [The fast path](#the-fast-path) above instead.
...
- **Postgres** with the canonical NovaMart e-commerce demo dataset pre-seeded (52 tables, ~480K rows)
- **Sandbox sidecar** for isolated code execution

See [Demo Dataset](/getting-started/demo-datasets) for the schema overview and canonical questions.
...
Profiling postgres database...
  ...
  [1/13] Profiling customers...
  [2/13] Profiling products...
  [3/13] Profiling orders...
  ...

Done! Semantic layer for "default" is at ./semantic/
```

Check the files: `ls semantic/entities/` should list 13 entities including `orders.yml`, `products.yml`, `customers.yml`, `payments.yml`, `shipments.yml`, and `returns.yml`.
</Callout>
...
- **[Deploy to production](/self-hosted/deployment/deploy)** — Railway, Vercel, or Docker
- **[Set up authentication](/self-hosted/deployment/authentication)** — API key, managed auth, or bring-your-own-token
- **[Choose an integration](/self-hosted/guides/choosing-an-integration)** — not sure whether to use the widget, SDK, or API? Compare all options
- **[Embed in your app](/self-hosted/guides/embedding-widget)** — add Atlas as a chat widget in any web app
- **[Integrate with Slack](/self-hosted/guides/slack)** — query data from Slack channels
- **[Use as MCP server](/self-hosted/guides/mcp)** — connect Atlas to Claude Desktop or Cursor
- **[Troubleshooting](/self-hosted/guides/troubleshooting)** — full error reference and debug logging
