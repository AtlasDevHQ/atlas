...
> **Amendment (2026-07-25, #4774):** §1's "never authoritative" is **scoped, not withdrawn**, by [ADR-0036](./0036-atlas-as-company-brain.md) § The knowledge substrate (T3). A tier-2 **reviewed fact** — a row in the brain substrate, *not* a knowledge document — is authoritative **for its class**, yielding to the warehouse in any overlap (`TRUST_TIERS` in `packages/api/src/lib/brain/types.ts`; tier 1 has no representation in the brain tables — warehouse facts resolve live through the semantic layer and are `executeSQL`'s). What holds a fact is the draft→published **review gate** §4 built, reused by a distinct subsystem — not descriptiveness. **This ADR is not superseded and the KB pillar is unchanged:** knowledge documents stay descriptive-only, never run verbatim, never extend the whitelist, never gate the agent, and are left *outside* that trust ordering rather than at the bottom of it (`BrainDocumentResult.trustTier` is `null`; `packages/types/src/brain.ts` carries why. A fused read is relevance-ordered and uses the tier only to break ties between equally-scored rows, where documents do come last — presentation order, not a truth claim). What §1's moat sentence no longer claims is that *descriptive vs. authoritative* is the only thing that can separate one context surface from another: a reviewed fact is separated by a review gate instead. §4's hard-boundary assertions are unchanged by any of this — they prove a knowledge document reaches neither the whitelist, nor a pinned metric, nor glossary gating, and that is still the enforced moat.
...
> **Amendment (2026-07-06, #4376):** The connector deferral above is closed. PRD #4375 (Confluence Cloud + Notion — two real vendors at once) is the "second example" this section anticipated; [ADR-0030](./0030-knowledge-sync-connector-seam.md) names the **Knowledge Sync Connector** seam, splits the ingest seam at document level, widens `IngestSource` (publish guard unchanged — connectors structurally cannot publish, per §4), and adds the high-water-mark + reconciliation engine. One prediction above did not survive contact: the connectors shipped as **API-token form installs** (vendor tokens in the dedicated `knowledge_sync_credentials` table), not OAuth installs with `INTEGRATION_TABLES` credentials. Everything else in this section stands.

> **Amendment (2026-07-25, #4774):** §5's `searchKnowledge` deferral is closed too, and the tool has since been renamed. It in fact shipped inside this ADR's own milestone (#4210), then was renamed **`searchBrain`** when [ADR-0036](./0036-atlas-as-company-brain.md)'s T7 (#4773) made hosted documents one of three fused stores rather than the whole tool; the old spelling survives only as a deprecation-window config alias (`RENAMED_TOOLS` in `lib/tools/registry.ts`). Read §5's `searchKnowledge` as today's `searchBrain`. Embeddings are still deferred — M1's fused read is FTS-first.
