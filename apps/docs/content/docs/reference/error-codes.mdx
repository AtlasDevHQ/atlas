...
| `provider_rate_limit` | 429 | AI provider rate limited | Upstream LLM provider (Anthropic, OpenAI, etc.) throttling requests | Wait and retry with backoff. Consider upgrading your provider plan |
...
| `provider_unreachable` | 502 | Cannot reach the AI provider | Network issue between Atlas and the LLM provider, or provider outage | Check provider status page. Retry after a short delay |
...
| `provider_auth_error` | 502 | AI provider authentication failed | Invalid or expired LLM provider API key (e.g., `ANTHROPIC_API_KEY`) | Verify your provider API key. Regenerate it if expired |
...
The `@useatlas/sdk` adds three SDK-specific codes to the `AtlasErrorCode` type. These are never returned by the server — they represent client-side conditions detected by the SDK itself.

| Code | Description | Retryable |
|------|-------------|-----------|
| `network_error` | `fetch()` threw an error (connection refused, DNS failure, stream interrupted) | Yes |
| `invalid_response` | Server returned a 2xx status but the body wasn't valid JSON | No |
| `unknown_error` | Server returned an error response with an unrecognized error code | No |
| `not_available` | Feature not available (e.g., conversation history without `DATABASE_URL`) | No |
...
```typescript
import { AtlasError, createAtlasClient } from "@useatlas/sdk";
...
async function queryWithRetry(
  question: string,
  maxRetries = 3,
): Promise<Awaited<ReturnType<typeof atlas.query>>> {
