...
export const PURGE_TABLE_DECISIONS = {
  ...
  admin_action_log: { decision: "anonymized", reason: "The operator accountability trail — INCLUDING the record of this purge. Deleting the org's rows would erase the evidence that the erasure happened, so instead the purge NULLs actor_id/actor_email/ip_address and stamps anonymized_at, reusing the exact scrub F-36 (migration 0035) built for the right-to-erasure contract and relaxed these columns to nullable for. The org_id and the action sequence survive; the identifiers do not. The pre-existing F-36 endpoint is keyed on `actor_id = userId` and is NOT reached by a workspace purge, which is why the purge does its own scrub rather than delegating." },
