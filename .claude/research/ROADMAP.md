...
Ordering recommendation: B → D (eval safety net) → C (governance) → E (distribution + foundation). Theme A complete (3/3); D fully closed (D1 Phase 1 + 2). Risk items A2 + B2 + B1 + D1 done. Next risk-class item is B3 (load-test hosted MCP, [#2070](https://github.com/AtlasDevHQ/atlas/issues/2070)) — `ATLAS_MCP_MAX_SESSIONS=100` is still somebody's gut, not a measurement, even with the N=10 concurrent-session smoke check Part B added. Alongside D2 (tool description audit, [#2075](https://github.com/AtlasDevHQ/atlas/issues/2075)) on the eval-quality lane.
