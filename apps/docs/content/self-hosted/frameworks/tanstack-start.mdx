...
If you use the `@ai-sdk/react` `useChat` approach, see the [React/Vite guide](/self-hosted/frameworks/react-vite#rendering-tool-calls) for a complete `ToolPart` implementation -- it works identically in TanStack Start. The data shapes are the same across all frameworks:
