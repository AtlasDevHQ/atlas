...
Atlas Cloud sandboxes execute in the US (Vercel's `iad1` region) regardless of the workspace's data-residency region — connect your own provider below to control where `explore` executes (locality follows your provider account's region configuration; Python coverage is per-provider, see [Python Tool Coverage](#python-tool-coverage)). See [Sandbox Execution Locality](/platform-ops/data-residency#sandbox-execution-locality) for what transits and the isolation controls.
