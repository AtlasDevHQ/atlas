...
| `ATLAS_DURABILITY_RETENTION_DAYS` | `30` | How long terminal (`done`/`failed`) agent-run checkpoints are retained before the scheduler retention sweep deletes them. Non-terminal runs are never swept. Working-memory rows ride the same sweep — a swept session takes its memory with it |
| `ATLAS_DURABILITY_RESUME_LEASE_SECONDS` | `300` | TTL of the single-resumer lease a crash-resume holds on an interrupted turn (#3747). A second concurrent resume of the same run is rejected while the lease is live; the lease self-heals once it lapses. Default 300s — deliberately longer than one agent turn's wall-clock budget |
| `ATLAS_DURABILITY_MAX_PARK_MINUTES` | `1440` (24h) | How long an agent turn may stay **parked** awaiting a human approval decision before the sweep fails it (#3748). Default 1440 minutes (24h), matching the approval-request expiry default |
