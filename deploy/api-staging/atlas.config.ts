/**
 * Atlas SaaS *staging* configuration — api.staging.useatlas.dev
 *
 * Loaded by the `api-staging` Railway service (staging slice 8/22, #2912).
 * Structurally a mirror of deploy/api/atlas.config.ts — same plugin set,
 * same catalog, same proactive runtime wiring, same agent/sandbox/pool/
 * scheduler/cache settings. It differs from prod ONLY in its region
 * identity and the URLs that name it:
 *
 *   - Region discriminator: the api-staging service sets ATLAS_API_REGION=staging
 *     (the EXISTING residency env var — NOT a new one; see the PRD
 *     "Region-keying and deploy mode" section). `residency.defaultRegion`
 ...
 *     never routes to a prod regional DB.
 *   - No prod-specific hostnames leak in: app.useatlas.dev →
 *     app.staging.useatlas.dev. Operator URLs (Twenty CRM baseUrl, etc.)
 ...
 * (HITL). Until then the config is inert; nothing loads it.
 */
...
export default defineConfig({
  ...
  plugins: [
    chatPlugin({
      ...
      proactive: {
        ...
        // Staging web origin. Primary is the per-service env var; the
        // fallback is the staging hostname (NOT prod's app.useatlas.dev) so
        // a missing env var can't point staging deep-links at production.
        linkUrl:
          process.env.ATLAS_PUBLIC_WEB_URL ?? "https://app.staging.useatlas.dev",
  ...
  // ── Data Residency ──────────────────────────────────────────────
  // Single-region soak (PRD story 17). Unlike the prod config — which
  // declares us/eu/apac plus a tolerated `staging` arm — this file declares
  // ONLY `staging`. The api-staging service sets ATLAS_API_REGION=staging,
  // which `getApiRegion()` (residency/misrouting.ts) reads first; if that
  // env var is ever unset, `defaultRegion: "staging"` keeps the config
  // self-identifying as staging. Either way the RegionGuardLive boot guard
  // (saas-guards.ts) finds the claimed "staging" region declared here with a
  // valid postgres `databaseUrl`, so boot doesn't hard-fail.
  //
  // No us/eu/apac arms: staging is excluded from the residency router and
