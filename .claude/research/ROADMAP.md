...
- [ ] `MutationErrorSurface` phase 2 remainder (~9 pages) (#1624 parent + #1649 tracker) — remaining chunks: compliance, prompts, roles, semantic, semantic/improve, sessions `revokeError`, + SCIM row-pin carve-out + billing PlanShell `combinedError`
- [ ] Type-design hardening for `<MutationErrorSurface>` — branded `FeatureName` + empty-message invariant (#1652)
- [x] Remaining abuse follow-ups: factory + computed helpers for `AbuseInstance` invariants (#1644), extract `errorRatePct` branch (#1638), integration test for `getAbuseDetail` (#1639) — bundled in PR #1681. Agent-review follow-ups filed: AbuseDetail DB-failure diagnostic channel (#1682), JSON.parse catch radius in getAbuseEvents (#1683), brand/discriminated-union for AbuseInstance (#1684), Percentage/Ratio branded numerics (#1685)
