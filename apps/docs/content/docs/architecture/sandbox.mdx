...
When a workspace has a [REST Datasource](/docs/datasources/rest) connected, the Python sandbox's **network policy** is the layer-0 security boundary for any future in-sandbox REST work: it bounds *which hosts the sandbox can reach at all*, before any operation-level check runs. How that boundary is enforced differs by backend, and the difference is operationally important.
