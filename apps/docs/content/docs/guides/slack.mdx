---
title: Slack Integration
description: Query your data from Slack with slash commands and threaded follow-ups.
---

import { Callout } from "fumadocs-ui/components/callout";

Atlas integrates with Slack via a slash command (`/atlas`) and threaded follow-ups. Ask questions in any channel, get answers with formatted data tables, and continue the conversation in a thread.

<WhenSaaS>
<Callout type="info" title="Looking for the proactive chat admin surface?">
Operators configuring proactive mode (kill switches, monthly quota, sensitivity, public-dataset HITL, activation announcement) — see the [Proactive Chat admin guide](/guides/proactive-chat).
</Callout>

## Install

On [app.useatlas.dev](https://app.useatlas.dev), the Atlas Slack app is pre-configured — there is no Slack app to create and no environment variables to set. A workspace admin installs it from **Admin > Integrations**: click **Connect** on the Slack card, approve the OAuth grant in Slack, and you land back on the Integrations page with the Slack card showing the connected workspace name, install timestamp, and the user who completed the install. For custom Slack app setup (e.g., your own branding), contact support.
</WhenSaaS>

<WhenSelfHosted>
<Callout title="Self-hosted prerequisites">
- Atlas API server running and accessible over HTTPS (required by Slack)
- A [Slack workspace](https://api.slack.com/apps) with permissions to create apps
- `SLACK_SIGNING_SECRET` from your Slack app's Basic Information page
- For multi-workspace OAuth: internal database (`DATABASE_URL`)
</Callout>

Self-hosted deployments bring their own Slack app — you create it once, point it at your Atlas API, and configure the credentials via environment variables.

## Setup Modes

| Mode | Env Vars | Best For |
|------|----------|----------|
| Single-workspace | `SLACK_BOT_TOKEN` + `SLACK_SIGNING_SECRET` | One Slack workspace |
| Multi-workspace (OAuth) | `SLACK_CLIENT_ID` + `SLACK_CLIENT_SECRET` + `SLACK_SIGNING_SECRET` | Distributing to multiple workspaces |

Both modes require `SLACK_SIGNING_SECRET` for request signature verification.

---

## Single-Workspace Setup (Self-Hosted)

The simplest setup -- no OAuth flow, no database storage for tokens.

### 1. Create a Slack App

Go to [api.slack.com/apps](https://api.slack.com/apps) and create a new app from scratch.

### 2. Configure Scopes

Under **OAuth & Permissions**, add these **Bot Token Scopes**:

- `commands` -- Register slash commands
- `chat:write` -- Post messages
- `app_mentions:read` -- Read mentions (for thread follow-ups)

### 3. Create the Slash Command

Under **Slash Commands**, create a new command:

- **Command:** `/atlas`
- **Request URL:** `https://your-api-host/api/plugins/chat-interaction/webhooks/slack`
- **Description:** Ask a data question

### 4. Enable Events

Under **Event Subscriptions**:

- **Request URL:** `https://your-api-host/api/plugins/chat-interaction/webhooks/slack`
- **Subscribe to bot events:** `message.channels`, `message.groups`

### 5. Enable Interactivity

Under **Interactivity & Shortcuts**:

- **Request URL:** `https://your-api-host/api/plugins/chat-interaction/webhooks/slack`

<Callout type="info" title="One webhook URL, three Slack app fields">
The Slack app's Slash Commands, Event Subscriptions, and Interactivity & Shortcuts Request URLs all point at the **same** `/api/plugins/chat-interaction/webhooks/slack` endpoint. That's the standard Bolt contract — the chat-plugin's Bolt receiver dispatches slash commands, events, and block-action interactions off a single URL.
</Callout>

### 6. Install and Configure

Install the app to your workspace, then copy the **Bot User OAuth Token** and **Signing Secret**:

```bash
# .env — copy these from your Slack app's settings page
SLACK_BOT_TOKEN=xoxb-your-bot-token        # OAuth & Permissions → Bot User OAuth Token
SLACK_SIGNING_SECRET=your-signing-secret    # Basic Information → Signing Secret
```

---

## Multi-Workspace OAuth Setup (Self-Hosted)

For distributing Atlas to multiple Slack workspaces. Requires an [internal database](/reference/environment-variables) (`DATABASE_URL`) to store per-workspace bot tokens.

### 1. Configure OAuth

In your Slack app settings, under **OAuth & Permissions**:

- **Redirect URL:** `https://your-api-host/api/v1/integrations/slack/callback`

### 2. Set Environment Variables

```bash
# .env — OAuth credentials from your Slack app settings
SLACK_CLIENT_ID=your-client-id
SLACK_CLIENT_SECRET=your-client-secret
SLACK_SIGNING_SECRET=your-signing-secret
SLACK_ENCRYPTION_KEY=                 # 32 raw bytes (hex64 or base64) — encrypts
                                      # per-workspace bot tokens at rest. Optional
                                      # for self-hosted single-user; required for
                                      # multi-workspace production.
DATABASE_URL=postgresql://...         # Required — stores per-workspace bot tokens
```

### 3. Install Flow

The supported path is the admin UI: a Workspace admin opens **`/admin/integrations`** and clicks **Connect** on the Slack card. The button redirects to Slack's OAuth authorize page; after approval, the callback persists the bot token under `chat_cache:slack:installation:<teamId>` — the single store shared with the `@chat-adapter/slack` multi-workspace lookup path. The admin lands back on `/admin/integrations` with a success toast and the Slack card now showing the connected workspace name, install timestamp, and the user who completed the install.

For headless installs (CI provisioning, SDK consumers), the install endpoint is still callable directly at `https://your-api-host/api/v1/integrations/slack/install`. A browser-based callback failure (invalid state, upstream Slack error) redirects to `/admin/integrations?error=slack&reason=...` with an actionable toast; JSON-Accept callers receive the original 400/502 for machine-readable debugging.
</WhenSelfHosted>

---

## Workspace Disclosure [#4-workspace-disclosure]

Before approving the OAuth grant, give the installing admin this disclosure so they can present it to their org:

<Callout type="info" title="Atlas privacy posture">
**Atlas reads messages in channels you opt in to proactive mode.**

By default Atlas only responds to direct mentions (`@atlas`), slash commands (`/atlas`), and DMs. If a workspace admin enables Atlas's proactive mode and adds a channel to the proactive allowlist, Atlas will read messages in that channel to detect data questions and may offer to answer them. Atlas does not monitor channels that have not been explicitly opted in.

Users can DM Atlas with `unsubscribe` to opt out of proactive answers across the workspace. Channel members can also pause proactive mode in a channel for 24 hours with `@atlas pause`.

When proactive mode is first enabled for a workspace, Atlas posts a one-time announcement to the admin-configured announcement channel containing this same disclosure so end-users see it at the point of contact.
</Callout>

---

## How It Works

### Slash Command

```
/atlas What was last month's revenue?
```

1. Slack sends the command to `POST /api/plugins/chat-interaction/webhooks/slack`
2. Atlas verifies the request signature (HMAC-SHA256, 5-minute timestamp window)
3. Atlas immediately acknowledges within 3 seconds (Slack requirement)
4. In the background, Atlas posts a "Thinking..." message, runs the agent, and updates the message with the formatted response

### Thread Follow-ups

When a user replies in a thread started by Atlas:

1. Atlas receives the message via the Events API
2. Loads the conversation history from the original thread
3. Runs the agent with the full conversation context
4. Posts the response in the same thread

This provides multi-turn conversations within Slack threads.

### Action Approvals

When the [action framework](/guides/actions) is enabled and a query produces pending actions, Atlas posts approval buttons visible only to the requesting user. Clicking **Approve** or **Deny** triggers `POST /api/plugins/chat-interaction/webhooks/slack` (the same endpoint that handles slash commands and events — Bolt routes by payload type).

### Query approval rules

If your workspace has <AudienceLink saas="/guides/approval-workflows">approval rules</AudienceLink> configured (e.g. "queries against `customer_pii` require approval"), a Slack query that matches a rule will not execute. Atlas replaces the "Thinking…" message with a clear approval-required notice naming the rule, and queues the request in the admin console. An admin must approve it from the web app before it can run — there is no per-message approval workflow inside Slack itself. This is the same gate that runs from the chat UI; it can't be bypassed by routing through Slack.

---

## Message Formatting

Atlas formats responses using Slack Block Kit:

- **Answer text** -- The agent's narrative response
- **SQL** -- The executed query in a code block
- **Data table** -- Column-aligned text table (max 20 rows displayed, with a "Showing X of Y" note)
- **Metadata** -- Steps taken, tokens used

Block Kit limits: max 50 blocks per message, max 3,000 characters per text block.

---

<WhenSelfHosted>
## Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `SLACK_SIGNING_SECRET` | Yes | Request signature verification |
| `SLACK_BOT_TOKEN` | Single-workspace | Bot OAuth token |
| `SLACK_CLIENT_ID` | Multi-workspace | OAuth app client ID |
| `SLACK_CLIENT_SECRET` | Multi-workspace | OAuth app client secret |
| `SLACK_ENCRYPTION_KEY` | Multi-workspace (production) | 32 raw bytes (hex64 or base64) — encrypts per-workspace bot tokens at rest |

See [Environment Variables](/reference/environment-variables) for the full reference.

---
</WhenSelfHosted>

## Deployment Notes

- Slash commands have a **3-second response deadline** -- Atlas acknowledges immediately and processes asynchronously
- Rate limiting is per Slack user per team for commands, and per team for thread follow-ups
- Error messages sent to Slack are scrubbed to prevent leaking connection strings, API keys, or stack traces

<WhenSelfHosted>
- The API must be accessible over **HTTPS** for Slack to deliver events and commands
- Without an internal database (`DATABASE_URL`), thread conversation mappings and OAuth tokens cannot be persisted. Single-workspace mode with `SLACK_BOT_TOKEN` still works, but thread follow-ups won't have conversation history
</WhenSelfHosted>

---

## Troubleshooting

<WhenSaaS>
If Atlas stops responding in Slack, check **Admin > Integrations** — the Slack card should show your workspace as connected. If it doesn't, run **Connect** again from the card; otherwise contact support.
</WhenSaaS>

<WhenSelfHosted>
**Events not being delivered** -- Verify the Request URL in your Slack app settings is HTTPS and publicly reachable. Slack retries failed deliveries but will disable the subscription after repeated failures.

**OAuth callback fails** -- Ensure the Redirect URL in your Slack app settings matches `https://your-api-host/api/v1/integrations/slack/callback` exactly. Check that `DATABASE_URL` is set (required for storing OAuth tokens).

**Bot doesn't respond in threads** -- The bot needs `message.channels` and `message.groups` event subscriptions. Also verify `DATABASE_URL` is set for conversation history persistence.
</WhenSelfHosted>

See [Troubleshooting](/guides/troubleshooting) for general diagnostic steps.
