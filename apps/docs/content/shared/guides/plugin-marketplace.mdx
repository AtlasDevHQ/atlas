...
- [Managed auth](/self-hosted/deployment/authentication#managed-auth-self-hosted-setup) enabled
...
Navigate to **Platform → Plugin Registry** (`/platform/plugin-registry`). The page lists every plugin loaded in your workspace (from `atlas.config.ts` and, on SaaS, any plugins installed by a platform admin on your behalf). Each row shows the plugin's name, type badge, enable/disable state, and a toggle to expand configuration and health details.
