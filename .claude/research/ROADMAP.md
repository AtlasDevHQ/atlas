...
- [x] `@useatlas/react` package (#218, PR #245) — extract `AtlasChat` + `AtlasUIProvider` into standalone React component package (`packages/react/`). Bundle with tsup, zero Next.js dependency. Props: `apiUrl`, `apiKey`, `theme`, `position`. Publish to npm
  - [ ] Headless hooks mode (#225) — export `useAtlasChat`, `useAtlasAuth`, `useAtlasTheme` without UI
  - [ ] Custom tool renderers API (#226) — `toolRenderers` prop for embedder-defined result rendering
...
- [x] BigQuery datasource plugin (#220, PR #250) — `@useatlas/bigquery`. `@google-cloud/bigquery` client, `node-sql-parser` BigQuery dialect, standard plugin structure following existing datasource plugins in `plugins/`. Most-requested missing source — unlocks GCP shops
  - [ ] Cost estimation (#229) — dry-run query to estimate bytes scanned / approximate cost before execution
- [x] Conversation sharing (#221, PR #249) — shareable links with optional expiry and permission controls (anyone with link / org-only). New `shared_conversations` table in internal DB. Public `/shared/:id` route renders read-only conversation view. People share interesting query results — this turns launch users into distribution
  - [ ] OG meta tags and social preview (#230) — title, description, branded OG image for shared links
  - [ ] Share expiry controls and org-scoped permissions (#231) — configurable TTL (1h–never), org-only mode
  - [ ] Embed-as-iframe mode (#232) — `/shared/:token/embed` minimal read-only view for external embedding
...
- [x] Onboarding hardening (#222, PR #259) — `bun create atlas-agent --demo` must work flawlessly end-to-end in under 60 seconds. Add E2E test covering full scaffolding flow. Fix any friction points discovered during testing
  - [ ] Post-scaffold `atlas doctor` auto-run (#233) — validate setup + report total elapsed time
- [ ] README overhaul (#223) — demo GIF/video at top, "What is Atlas → Try it in 60s → Why it's different" structure, widget embed example, comparison links. The README is the landing page for GitHub visitors
  - [ ] Animated terminal recording (#234) — SVG recording of atlas init → query flow
- [ ] Landing page refresh (#238) — useatlas.dev update with widget demo section, competitive positioning ("Open-source alternative to ThoughtSpot"), updated feature grid reflecting 0.1–0.4 shipped work
  - [ ] Live interactive widget on landing page (#241) — visitors can query the demo dataset directly
- [ ] Launch content (#224) — draft Show HN post, introductory blog post, "Atlas vs WrenAI vs Vanna" comparison page for docs site. Pre-write in `.claude/research/launch/`
  - [ ] Comparison page on docs site (#235) — feature matrix vs WrenAI, Vanna, Metabase
