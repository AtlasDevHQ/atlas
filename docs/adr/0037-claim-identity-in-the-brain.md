...
- **Every statement the producer causes is bounded in the DATABASE, not in JS.** `boundedTransaction` issues `SET LOCAL statement_timeout` and `SET LOCAL lock_timeout` around the read *and* around each proposal. The trigger is `await`ed inside the extraction drain's `concurrency: 1` loop with no per-tick timeout, so a call that never SETTLES — an internal database that is reachable and not answering — would stop the whole brain-extraction fiber forever, with no error to catch and no dead fiber to log. `correction.ts`'s `proposeUnderDeadline` race was weighed and declined for the property its own docstring records: *`Promise.race` does not CANCEL the query*, so it abandons a statement that goes on holding one of five pooled connections. Read `ALIAS_PROPOSAL_CANDIDATE_CAP` for the matching correction: `LIMIT` bounds the QUEUE and not the scan, because it applies after `GROUP BY`.
