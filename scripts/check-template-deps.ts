...
/**
 * Floor for the anti-vacuity check below. Both templates walked ~1,280 source
 * files when this landed; 500 is well clear of normal churn and still catches
 * an absent or half-generated tree.
 */
...
function checkTemplate(template: string): Missing[] {
  ...
  // Anti-vacuity. With --skip-prepare this gate trusts that
  // check-template-drift.sh has already generated the template source; if that
  // step is reordered away, or someone runs this on a fresh checkout, the
  // gitignored `src/` is simply absent and the walk returns almost nothing.
  // The gate would then report "every import is declared" having checked
  // nothing at all — a green that verified nothing, which is worse than a red.
  // Same reasoning as scripts/check-type-program-not-vacuous.sh.
  if (!existsSync(join(templateDir, "src"))) {
    throw new Error(
      `${template}: create-atlas/templates/${template}/src is missing. It is ` +
        `gitignored and generated by prepare-templates.sh — run without ` +
        `--skip-prepare, or run check-template-drift.sh first.`,
    );
  }
  if (files.length < MIN_SOURCE_FILES) {
    throw new Error(
      `${template}: only ${files.length} source files found (expected at least ` +
        `${MIN_SOURCE_FILES}). The template tree looks incomplete, so this check ` +
        `would pass without having verified anything.`,
    );
  }
