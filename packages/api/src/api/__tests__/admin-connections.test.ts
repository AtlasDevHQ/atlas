...
// Billing enforcement (#3433) — per-test arm selection via
// mockCheckResourceLimit. Default `allowed: true` matches what the real
// `checkResourceLimit` returns under this harness (`getWorkspaceDetails`
// resolves null → no cap), so the CRUD suites below are unaffected.
