...
4. **Rate limit** — a per-`(workspace, datasource, operation)` token bucket, **60 calls/min** by default, overridable per install via `rate_limit_per_minute`. Throttles runaway agent loops on a paid upstream (`rate_limited`, with a retry hint).
5. **Timeout** — each request is capped at `ATLAS_OPENAPI_TIMEOUT` (default **30s**). A per-install timeout above the cap is rejected.
