...
| `ATLAS_ONBOARDING_EMAILS_ENABLED` | Profile-driven (`production`: `true`; `staging`/`development`: `false`) | Enable the automated onboarding drip campaign for new signups. The default follows the [`ATLAS_DEPLOY_ENV`](#runtime) profile; setting this var explicitly always wins, but only the literal `true` enables it (any other value, including unset on a non-prod profile, disables). Also requires `DATABASE_URL` and either `RESEND_API_KEY` or `ATLAS_SMTP_URL` |
