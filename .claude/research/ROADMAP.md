...
- [ ] Bucket 3 — Dashboards
  ...
  - [x] 3a list (`/dashboards`, #1866 → superseded by #1895, PR #1896) — **completed by deletion**. The standalone list page was mostly a launcher; with #1867's freeform tile grid the dashboard *is* the home, so we killed the surface instead of redesigning it. `/dashboards` is now a server-side redirect to the most-recently-updated dashboard (id-asc tiebreak); empty workspaces fall through to an inline create-first-dashboard state. A new `<DashboardSwitcher>` chevron next to the title on the detail topbar opens to a sorted dropdown with current marker, "+ New dashboard", and "View all" actions; the View All modal preserves the old card-grid pattern (hover trash + create dialog) for occasional governance/audit. Pre-fetched after-delete redirect lands the user on the next-most-recent dashboard or the empty state. Multi-agent review surfaced + fixed inline: cross-origin pre-delete fetch, switcher silent-error rendering, modal silent-DELETE-failure, NewDashboardDialog onCreated default-to-navigation. Shared `defaultOnDashboardCreated(router)` helper is the explicit policy now.
