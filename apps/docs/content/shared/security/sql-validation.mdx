...
<Callout title="Looking for the overview?">
  This page is the deep technical reference. For the prospect-facing summary of
  how Atlas keeps your database safe — validation, read-only connections,
  sandboxing, and encrypted credentials — see the
  [Security overview](https://www.useatlas.dev/security).
</Callout>
...
<WhenSaaS>
On Atlas Cloud the table whitelist is platform-scoped and always on — it cannot be disabled from the workspace admin console.
</WhenSaaS>
...
Review query patterns in Admin > Audit Log — see [Error handling](#error-handling) for what gets recorded.

### For developers

If you're building plugins or custom tools, see [Plugin hooks](#plugin-hooks) above for how `beforeQuery`/`afterQuery` hooks and custom validators interact with the pipeline. Custom validators **completely replace** layers 0-3, so your validator must provide equivalent safety guarantees.
...
- <AudienceLink saas="/deployment/schema-evolution">Schema Evolution</AudienceLink> — Keeping the semantic layer in sync with database changes
- [Environment Variables](/reference/environment-variables#security) — `ATLAS_ROW_LIMIT`, `ATLAS_QUERY_TIMEOUT`, and `ATLAS_TABLE_WHITELIST`
