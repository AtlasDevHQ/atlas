...
- **the alias decide transaction** (`lib/brain/vocabulary-decide.ts`, #5023) — the *identity* arm's sanctioned writer, and the only carve-out here that is a **pre-registration**: the file writes no gated column today. The gate's own remediation text already names it (*"Re-keying because the vocabulary changed? That is the alias-approval decide transaction"*), and ADR-0037 §7 puts the drift re-key inside that transaction, in TypeScript at request time rather than in another migration. The entry lands with the seam rather than with the re-key (#5024) for the reason the `_cmp` arms were gated ahead of their schema: the guard should never be the thing lagging, and a carve-out argued when the writer arrives is one argued under deadline. **Its cost, recorded because the allowlist has no per-column granularity:** an allowlist entry exempts a *file*, so this one also exempts the decide seam for `status`, `visible_to` and `valid_to` — columns it has no business writing. What holds that in place is the module's own scope (it writes `brain_vocabulary_*`, never `brain_facts`) plus this register; if #5024 ever needs one of the other three, that is a new argument, not an inherited one.
