...
Ordering recommendation: B → D (eval safety net) → C (governance) → E (distribution + foundation). Theme A complete (3/3); **Theme B fully closed** (B1 + B2 + B3 done as of 2026-05-07); D1 + D2 closed. Next risk-class item is **D3 (MCP prompts library exposes canonical eval questions, [#2076](https://github.com/AtlasDevHQ/atlas/issues/2076))** — same lane as D2 (eval surface), and the canonical questions are the best showcase the prompt panel could carry. Theme C (governance) follows once D is done.
