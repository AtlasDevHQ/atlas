...
`ATLAS_MCP_MAX_SESSIONS=100` is the documented default ([env vars](/reference/environment-variables#mcp-transport)). The number reflected operational feel — Railway's `numReplicas: 1` constraint plus the in-memory session map plus a "feels safe" guess. Issue [#2070](https://github.com/AtlasDevHQ/atlas/issues/2070) asked the obvious follow-up: **has anyone actually measured it?** And: where does the curve break?
...
All three drive the wire protocol directly (no SDK dependency). Setup, env vars, and result-reading are documented in the [README](https://github.com/AtlasDevHQ/atlas/blob/main/eval/load-tests/mcp/README.md). A manual `workflow_dispatch` runner — [.github/workflows/load-test-mcp.yml (deleted)](https://github.com/AtlasDevHQ/atlas/blob/main/.github/workflows/load-test-mcp.yml) ([#2129](https://github.com/AtlasDevHQ/atlas/issues/2129)) — runs the same scripts on demand, posts a markdown summary to the run's step summary, and uploads `summary.json` + `.log` files as a workflow artifact (30-day retention). The numbers on this page are still operator-curated until a tracking-issue or external-dashboard pipeline lands.
