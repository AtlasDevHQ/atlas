...
- **Glossary disambiguation** — `revenue`, `status`, `price` are `status: ambiguous` and must trigger the disambiguation contract documented in [the typed MCP tools](/self-hosted/guides/mcp#searchglossary), never resolve to a single mapping
...
| Trigger | Behavior |
|---|---|
| Pull request to `main` | Informational. `continue-on-error: true` — a regression posts the `X/N passing` line in the run log but doesn't block merge. Mirrors the [Lighthouse budget](/self-hosted/contributing/ci#lighthouse-budget-advisory) first-month policy. |
