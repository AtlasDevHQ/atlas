...
| `rateLimit` | number | No | Per-datasource rate limit |
...
Define RLS policies that inject WHERE clauses on every query based on user JWT claims.
...
This injects `WHERE tenant_id = '<user's org_id claim>'` on every query.
...
export default defineConfig({
  rls: {
    column: "tenant_id",
    claim: "org_id",
    table: "orders", // Only apply to the orders table
  },
});
```

Equivalent env vars: `ATLAS_RLS_ENABLED=true`, `ATLAS_RLS_COLUMN=tenant_id`, `ATLAS_RLS_CLAIM=org_id`.
