...
[markdown-tree adapter](#the-markdown-tree-adapter) the second, and three
file-based importers — [Mintlify](#the-mintlify-importer) (a `docs.json` nav
filter), [MkDocs](#the-mkdocs-importer) (a `mkdocs.yml` `nav:` filter), and
[Docusaurus](#the-docusaurus-importer) (a `sidebars` filter) — the markdown-tree
adapter plus a per-framework nav filter. (Confluence shipped as a *server-side*
...
`createMkDocsSource` (#4392) points the markdown-tree adapter at a MkDocs
project and derives a `filter` predicate from `mkdocs.yml`'s `nav:` tree —
`mkdocs.yml`, falling back to `mkdocs.yaml`. The walk is rooted at the
resolved `docs_dir` (default `docs/`), so archive paths carry no `docs/`
prefix leak. Pages under `docs_dir` but absent from `nav:` are declined and
counted in `skipped.filtered`.

```ts
import { buildOkfBundle, createMkDocsSource } from "@atlas/okf-bundle";
import { load } from "js-yaml";

const { source, filter, nav } = await createMkDocsSource({
  root: "my-mkdocs-site", // where mkdocs.yml lives
  parseYaml: load,
});
const { bytes, stats } = await buildOkfBundle(source, { prefix: "docs", filter });
// nav.configPath — which config won; nav.docsDir — the resolved docs_dir
```

The config is parsed structurally as unknown YAML (via the injected
`parseYaml`; runtime deps stay `fflate`-only), exploiting the one invariant
across every MkDocs nav shape (bare path, `Title: path.md`,
`Section: [ …nested… ]`): **a page path only ever appears as a string that is a
bare list element or the value of a single-key title mapping**. Titles (mapping
keys) and external-link values (`Community: https://…`) are never pages. A
config with **no `nav:`** is MkDocs's auto-discovery mode — the whole `docs_dir`
collects unfiltered. A missing/malformed/non-mapping config, a non-list `nav:`,
a non-string `docs_dir`, or a `nav:` resolving to zero pages throws
`NavManifestError` at generation time.

## The Docusaurus importer

`createDocusaurusSource` (#4392) points the markdown-tree adapter at a
Docusaurus docs directory and derives a `filter` predicate from the site's
`sidebars` config. A `sidebars.js`/`sidebars.ts` module must be **evaluated**
(entries can be computed at load time), so — exactly like `parseYaml` — the
caller loads it and injects the plain object; this package owns the resolver
(the sidebar walk, `type: autogenerated` directory expansion, and fail-loud
accounting).

```ts
import { buildOkfBundle, createDocusaurusSource } from "@atlas/okf-bundle";
import { load } from "js-yaml";

const sidebars = (await import("./sidebars.js")).default; // or require / JSON.parse
const { source, filter, sidebar } = await createDocusaurusSource({
  root: "my-docusaurus-site/docs", // the docs directory
  parseYaml: load,
  sidebars,
});
const { bytes, stats } = await buildOkfBundle(source, { prefix: "docs", filter });
// sidebar.pages — the reachable doc-id set; stats.skipped.filtered — off-sidebar docs
```

The resolver walks Docusaurus's item shapes structurally: a doc is named only
by a bare id string, a `type: doc`/`ref` item's `id`, a `type: category` item's
`link` (when it's a `doc`), or membership of a `type: autogenerated` directory
(expanded against the docs tree — membership only, and the deterministic packer
renders it byte-stable regardless of order). Category labels, `link`/`html` items, and
`generated-index` category links name no source doc. `.mdx` module lines are
stripped by the base adapter's fence-aware strip. A non-object `sidebars` value
or a sidebar resolving to zero docs throws `NavManifestError` at generation
time.
