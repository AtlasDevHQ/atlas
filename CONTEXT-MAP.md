...
## Status: the split is underway — 4 of 18 extracted

⚠️ **Read the *Governed by* column before following any path below.** It says, per context,
whether that context has its own file yet. Four do; the other fourteen are still sections of
the root [`CONTEXT.md`](CONTEXT.md), which shrank from ~85 KB to ~62 KB as they left.
