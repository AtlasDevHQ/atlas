 ...
 * 5xx behavior, bearer-auth header construction, the
 * first-source-preservation logic (parameterized on existing-Person state),
 * and `parseRetryAfterMs` (formerly `retry-after.test.ts`, merged here — it
 * drove the same module, `../src/client`).
 *
...
// ---------------------------------------------------------------------------
//  parseRetryAfterMs (moved from retry-after.test.ts)
//
//  RFC 9110 §10.2.3 covers two valid forms: delta-seconds and HTTP-date. Both
//  must round-trip cleanly through the parser, and any garbage / negative
//  value must collapse to undefined (so a misbehaving upstream can't ask us to
//  retry in the past — which would either be a no-op or, worse, produce a
//  negative interval the outbox would store as a Postgres-rejected timestamp).
// ---------------------------------------------------------------------------
