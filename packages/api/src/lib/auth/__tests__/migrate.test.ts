...
describe("migrateAuthTables", () => {
  it("runs internal DB migration when DATABASE_URL is set", async () => {
    ...
    // + 1 loadSavedConnections SELECT = 38 total
  ...
  it("only runs once (idempotent guard)", async () => {
    ...
    // Internal DB migration runs once (37 queries) + 1 loadSavedConnections SELECT + 1 ALTER TABLE for password_change_required (managed mode)
