...
| `invoice.payment_succeeded` / `invoice.paid` | **Automatically un-suspends** a workspace blocked or suspended by a payment failure, restores access, and sends a recovery-confirmation email |
...
2. **`unpaid`** → chat and queries are blocked with a "fix your payment" message; an email is sent. (This reuses the same `suspended` workspace state as step 3 — there is no separate billing-block status.)
3. **3+ failed retries** → the workspace is suspended and a final-notice email is sent.
4. **Payment succeeds** (`invoice.payment_succeeded` / `invoice.paid`, or the subscription returns to `active`) → the workspace is automatically un-suspended and a recovery-confirmation email is sent.
