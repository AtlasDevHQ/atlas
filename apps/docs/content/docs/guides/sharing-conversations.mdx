...
> **Note:** The SDK currently uses default settings (no expiry, public access). To specify expiry or share mode, use the REST API directly.
...
| `org` | Only authenticated users can view the conversation |

Organization-scoped shares require the viewer to be authenticated. Unauthenticated viewers see a "Sign in required" message.
...
| `429` | `rate_limited` | Too many requests (60 per minute per IP) |
| `500` | `internal_error` | A server error occurred |
...
- **Org-scoped access** — links with `shareMode: "org"` require the viewer to be authenticated. Unauthenticated requests receive `401`. Note: this checks for any valid authentication, not organization membership. In `auth: "none"` deployments, org-scoped sharing has no effect.
