...
- The catch-all route sets `maxDuration = 300` (5 minutes) — this requires the **Pro plan**. Hobby plan maximum is 60 seconds; set `maxDuration = 60` if you're on Hobby. See [Vercel plan limits](https://vercel.com/docs/functions/configuring-functions/duration)
...
- The Dockerfile defaults to `INSTALL_NSJAIL=true` — nsjail is included in the image by default. Build without it for a smaller image: `docker build --build-arg INSTALL_NSJAIL=false -f Dockerfile -t atlas .`
