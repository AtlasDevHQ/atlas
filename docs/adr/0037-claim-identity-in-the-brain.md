...
- **The importer's `_cmp` destination is typed `ComparableValue`, and `regionPortableComparable` is the module's only export that produces one from a stored value.** Both halves are needed: the type stops `fact.subjectCmp ?? null`, and deleting the sibling entry points stops the caller who reaches past the function — #5032's brand-the-output lesson, at a destination rather than a parameter.
