...
- **Dark-launch US-only (flip EU/APAC `selectable: false`), ship regional identity post-launch.** Rejected by the maintainer: build it right while pre-customer — retrofitting identity-regionalization after real EU customers exist is the migration that actually hurts; the clean-break window (docs/contexts/deployment-posture/CONTEXT.md — 🔒 window CLOSED 2026-08-18; this decision stands, the authority is spent) is the cheap time.
...
- **`@useatlas/types` / config stay the source of truth for the region map.** `deploy/api/atlas.config.ts` `residency.regions[region].apiUrl` is what the picker, the front-door, and the cookie fast-path consume; the staging-stays-local routing *intent* and `isRegionSelectable` are unaffected (staging stays `selectable: false`, `"local"`). The `DEPLOY_REGION_ROUTING` table itself is removed by §2 along with its sole consumer `resolveRegionDatabaseUrl` — the policy lives on in the deploy config's `selectable: false`, not the deleted constant.
...
## Amendment (2026-07-16, #4223): known exception — agent sandbox execution runs in the US

"The process is the region" has one known exception at launch: **the agent's sandboxed tool execution**. On Atlas Cloud, `explore` and `executePython` run in Vercel Sandbox, and Vercel Sandbox provisions exclusively in `iad1` (US East) with no region primitive — verified conclusively 2026-07-16 against `@vercel/sandbox` 2.7.0, the then-latest release (no create-time region parameter; `region` exists only as a read-only instance getter; the deployed SDK, 2.2.1, has the same absence), Vercel's docs (sandboxes "automatically provision in `iad1`"), the absence of any project- or team-level placement setting, and nothing publicly roadmapped. All three regional API processes share one Vercel team/project (`deploy/api/atlas.config.ts`), so EU/APAC workspaces' sandbox payloads execute in the US.

**What transits is customer data, not metadata:** the semantic-layer files copied into the sandbox for `explore` embed sampled column values from the customer warehouse; the mounted mode root also carries the hosted Knowledge Base document mirror (ADR-0028), so workspace-authored KB content transits too; and `executePython` writes actual SQL query-result rows into the sandbox. Counterweights: the sandbox's network policy is deny-all (for Python, deny-all by default — egress may open only to an installed REST datasource's own hosts, credential-free, per #2927), explore pools one sandbox across tool calls while Python creates a fresh one per call — neither is ever snapshotted (`persistent: false`; nothing persists at Vercel once a sandbox stops), and the DPA sub-processor annex has disclosed the US processing since 2026-07-10 (Vercel entry; wording widened 2026-07-16 to name knowledge-base content explicitly) — the legal floor is covered; this amendment brings the product promise in line.

**Decision (maintainer-approved 2026-07-16, #4223): ship v0.1.0 with a documented carve-out**, not a migration against the launch clock and not tool-gating for EU/APAC workspaces (considered, rejected). The exception is stated plainly on every surface that makes the residency promise — this ADR, the [data-residency docs](../../apps/docs/content/docs/platform-ops/data-residency.mdx), and the www `/security` page — rather than silently contradicting them.

**Remediation tracks (spun out of #4223):**
- [#4665](https://github.com/AtlasDevHQ/atlas/issues/4665) — finish Python support on the e2b/daytona BYOC sandbox providers. An engaged BYOC sandbox routes above the platform `vercel-sandbox` pin and fails closed **for `explore`** — though Atlas passes no region parameter to the BYOC plugins (they carry only API credentials), so in-region execution today depends on the provider account's own region configuration, not on anything Atlas pins; `executePython` on e2b/daytona falls through to the US platform sandbox by design until `createPython` lands (`providerSupportsPython`, `lib/sandbox/runtime.ts`) — the docs state this rather than implying fail-closed.
- [#4666](https://github.com/AtlasDevHQ/atlas/issues/4666) — evaluate a per-region **platform** sandbox provider post-launch (Daytona `target:`, Modal `region=` incl. `ap`, Cloudflare `jurisdiction:` all ship region primitives) to end the carve-out for everyone.
