...
- **Plugin Catalog** — the runtime registry of plugins / integrations available on a deployment. Backed by the `plugin_catalog` table. Seeded from `atlas.config.ts` at boot (see [ADR-0002](./docs/adr/0002-catalog-seeded-from-config-at-boot.md)). Holds `min_plan`, `enabled`, `config_schema` per entry. Ops can flip `enabled` for emergency disable. **Operator-curated only**: every catalog write path is operator-authored, enforced at the write seam by `assertOperatorCatalogWrite` (`lib/plugins/catalog-provenance.ts`, #4174) — third-party/community plugin submission is gated on plugin-execution isolation (#4099).
