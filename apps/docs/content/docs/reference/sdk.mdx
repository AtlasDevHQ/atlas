...
All API errors are thrown as `AtlasError` instances. See [Error Codes](/reference/error-codes) for the full catalog of every error code, HTTP status, and retry guidance. See [Rate Limiting & Retry](/guides/rate-limiting) for backoff patterns and handling 429 responses.
...
| `provider_auth_error` | 502 | No | LLM provider auth failed |
| `provider_rate_limit` | 429 | Yes | LLM provider rate limited |
| `provider_timeout` | 504 | Yes | LLM provider timed out |
| `provider_unreachable` | 502 | Yes | Could not reach the AI provider |
| `provider_error` | 502 | Yes | LLM provider error |
| `internal_error` | 500 | Yes | Server error (includes `requestId` for log correlation) |
| `network_error` | — | — | Could not reach the API (SDK-only) |
| `invalid_response` | — | — | Response could not be parsed (SDK-only) |
| `unknown_error` | — | — | Unrecognized error (SDK-only) |
