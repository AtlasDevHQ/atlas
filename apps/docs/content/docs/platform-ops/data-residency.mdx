...
<Callout type="warn" title="Stays behind does not mean retained">
  Everything in the "stays" rows above remains only for the 7-day grace period. Once the source-region cleanup runs, the workspace's leftover rows are **deleted** from the source region — the migration bundle is the complete set of what survives. Export anything else you need before the grace period ends.
</Callout>
