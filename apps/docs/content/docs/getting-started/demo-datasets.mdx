...
<Callout type="info" title="Self-hosted only">
  Demo datasets are for self-hosted local development and evaluation. On [app.useatlas.dev](https://app.useatlas.dev), the onboarding wizard offers a demo dataset option when you create your workspace — no CLI needed.
</Callout>
