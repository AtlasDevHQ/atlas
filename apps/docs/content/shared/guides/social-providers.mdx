...
Atlas's managed auth (Better Auth) supports social login with **Google**, **GitHub**, and **Microsoft** — no plugin required.

<WhenSaaS>
<Callout type="info" title="Atlas Cloud">
  On [app.useatlas.dev](https://app.useatlas.dev), social login is operated by the Atlas platform — **Sign in with Google, GitHub, or Microsoft** is available on the sign-in page with no setup. See [Signup](/guides/signup) for the hosted signup flow.
</Callout>

## Social Login on Atlas Cloud

There is nothing to configure: the sign-in page at [app.useatlas.dev](https://app.useatlas.dev) shows a button for each available provider. Two behaviors are worth knowing:

- **Account linking** — When you sign in with a social provider using the same verified email as an existing Atlas account, the accounts are linked and you can sign in with either method. Google and Microsoft return verified emails by default; for GitHub, your primary email must be verified on GitHub for linking to work.
- **GitHub email errors** — GitHub sign-in fails if your GitHub account has no verified primary email. Go to [GitHub email settings](https://github.com/settings/emails), verify an email, and set a primary.

The rest of this page — creating OAuth apps, callback URLs, environment variables — is for self-hosted deployments, where you operate the providers yourself.
</WhenSaaS>

<WhenSelfHosted>
Enabling a provider on a self-hosted deployment takes two steps: create OAuth credentials with the provider, then set the matching environment variables. Atlas enables each provider automatically at boot when both its client ID and client secret are set — no code changes needed. This guide walks through each provider.
...
Restart Atlas — providers are read once at boot, and Google is enabled automatically when both variables are set.
</Step>
...
Restart Atlas — GitHub is enabled automatically when both variables are set.
</Step>
...
Restart Atlas — Microsoft is enabled automatically when both client ID and secret are set.

**`MICROSOFT_TENANT_ID` values:**
...
Enable any combination of providers by setting each one's pair of environment variables — every provider with both its client ID and client secret set is enabled at boot:

```bash
# .env — all three enabled at once
GOOGLE_CLIENT_ID=123456789.apps.googleusercontent.com
GOOGLE_CLIENT_SECRET=GOCSPX-...
GITHUB_CLIENT_ID=Iv1.abc123def456
GITHUB_CLIENT_SECRET=your-github-client-secret
...
The Atlas web UI detects configured providers automatically — the login page shows a sign-in button for each enabled provider, no frontend changes needed. If you're building a custom frontend, use the Better Auth client:
...
**Symptom:** A user signs in via social provider but gets the default `member` role instead of `admin`.
...
- Otherwise → the default `member` role (the `role` field's `defaultValue`)
...
See [Cross-origin deployment](/self-hosted/deployment/authentication#cross-origin-deployment-self-hosted) for details.
...
- <AudienceLink selfHosted="/self-hosted/deployment/authentication">Authentication</AudienceLink> — full auth mode reference including managed auth setup
- <AudienceLink saas="/guides/admin-console">Admin Console</AudienceLink> — manage users and roles
- <AudienceLink saas="/security/row-level-security" selfHosted="/self-hosted/deployment/authentication#rls-with-byot">Row-Level Security</AudienceLink> — tenant isolation with JWT claims
- [Troubleshooting](/guides/troubleshooting#auth-issues-operator) — general auth troubleshooting
