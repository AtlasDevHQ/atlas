...
3. Choose **Multi Tenant** for the bot type (or **Single Tenant** if restricting to one Azure AD tenant)
...
| `appPassword` | `string` | Yes | Microsoft App Password (client secret) from Azure AD. |
| `tenantId` | `string` | No | Restrict the bot to a specific Azure AD tenant. Tokens from other tenants are rejected. |
...
3. The plugin verifies the JWT in the Authorization header against Azure AD's public keys
...
Ensure `TEAMS_APP_ID` and `TEAMS_APP_PASSWORD` match the Azure Bot registration credentials. If using tenant restriction, verify the tenant ID matches the Azure AD tenant where Teams is configured.
