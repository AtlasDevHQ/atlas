...
### Cross-environment scope routing

When a workspace has multiple **connection groups** (environments — e.g. `us-int`, `eu`, `us-prod` all running the same schema; see [Dashboards → Environment](/guides/dashboards#environment)), the agent can fan a single question out across more than one member of the active group. Each chat conversation has a **routing mode** the user controls from the picker in the chat header:

| Mode | Behavior |
|------|----------|
| **Auto** | The agent decides — picks one environment when the question is environment-specific ("how many EU orders today?") and fans out across all members for fleet-wide questions ("compare order volume across environments"). Default. |
| **Pin** | Every `executeSQL` call targets the conversation's pinned environment. Use when you want stable, single-source results — e.g. building a dashboard card that will be published. |
| **All envs** | Every `executeSQL` call fans out across all group members and merges the rows under an `environment` discriminator column. Audit rows for each child query roll up to a parent row via `query_audit.parent_audit_id`. |

The `executeSQL` tool exposes a `scope` parameter (`auto` / `pin` / `all`) that the agent fills based on the mode. Results carry an `envContributions` wire field with per-environment row counts and any partial-failure errors, so a fan-out that succeeds on 2 of 3 environments returns the merged rows and surfaces the third as a degraded warning rather than failing the whole turn.
...
- **Exclude a datasource** — drop one or more REST datasources from a conversation. The agent still runs SQL and queries every *other* in-scope REST datasource, but the excluded ones are invisible to it for that chat. Use when a connected API is noisy or irrelevant to the question at hand.
- **REST-only focus** — focus a conversation on a single REST datasource. This **suspends `executeSQL`** (and SQL-authoring tools like `createDashboard`) for the conversation: the agent answers only via `executeRestOperation` against the focused datasource and is told SQL is unavailable. Clearing the focus re-enables SQL and returns the conversation to its prior scope.

Both axes are **per-conversation and authoritative**: scope is restored exactly when you reopen a conversation, and a workspace-level sticky preference seeds new chats without ever overwriting a conversation's saved scope. SQL routing mode (`auto` / `pin` / `all`) and the REST exclude-set apply together; a REST-only focus overrides both until it is cleared.
