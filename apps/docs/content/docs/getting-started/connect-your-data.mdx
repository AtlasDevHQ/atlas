...
<Callout type="info">
If you're working in the Atlas monorepo, the driver is already bundled in `@atlas/api`. These install commands are only needed for standalone or scaffolded projects (e.g. those created with `bun create atlas-agent`).
</Callout>
...
Every query goes through seven layers of validation before execution (see [SQL Validation Pipeline](/docs/security/sql-validation) for full details):

0. **Empty check** -- Reject empty or whitespace-only queries
1. **Regex mutation guard** -- Quick reject of DML/DDL keywords (INSERT, UPDATE, DELETE, DROP, etc.)
2. **AST parse** -- `node-sql-parser` in PostgreSQL/MySQL mode (auto-detected) verifies a single SELECT statement. Unparseable queries are rejected
3. **Table whitelist** -- All referenced tables must exist in `semantic/entities/*.yml`. CTE names are excluded from the check
4. **RLS injection** -- When [Row-Level Security](/docs/reference/environment-variables#row-level-security) is enabled, WHERE clauses are injected based on the authenticated user's claims. Applied after validation and plugin hooks so plugins cannot strip RLS conditions
