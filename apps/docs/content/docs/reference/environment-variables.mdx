...
<Callout type="warn">
Some providers need a **complete** credential set, not just one key. `bedrock` requires `AWS_ACCESS_KEY_ID` **and** `AWS_SECRET_ACCESS_KEY` (the region resolves through the standard AWS credential chain — set `AWS_REGION` unless your deploy supplies it another way); `openai-compatible` requires `OPENAI_COMPATIBLE_BASE_URL`. On **SaaS** (`ATLAS_DEPLOY_MODE=saas`), the provider-key boot guard (`ProviderKeyGuardLive`) rejects an incomplete set at startup rather than letting boot pass and the first chat fail; self-hosted surfaces the same gap as a per-request `503`. See #3178, #3200.
</Callout>
